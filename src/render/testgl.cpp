...
int HandleEvent(SDL_Event *tEvent)
{
    ...
    switch( tEvent->type ) {
    // case SDL_ACTIVEEVENT: // removed in SDL3
        /* See what happened */
        printf( "app %s ", tEvent->active.gain ? "gained" : "lost" );
        if ( tEvent->active.state & SDL_WINDOW_OCCLUDED ) {
            printf( "active " );
...
int RunGLTest( int argc, char* argv[] )
{
    ...
    // SDL3: SDL_SetVideoMode removed; testgl is a legacy test utility.
    if ( false /* SDL_SetVideoMode stub */ ) {
    ...
    // SDL3: SDL_WM_SetCaption removed; use SDL_SetWindowTitle(window, "SDL GL test");
