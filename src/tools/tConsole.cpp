...
static std::ofstream * st_LogFile();
...
void tConsole::PrintLine(tString const & line, int repetitions)
{
    ...
    {
        if ( std::ofstream * log = st_LogFile() )
        {
            *log << tColoredString::RemoveColors( line );
            log->flush();
        }
...
// Windowed builds have no console to print to - on Windows nothing is
// attached at all - so everything the game has to say about itself was going
// nowhere and a player reporting a problem had nothing to send. A copy goes to
// a file beside the configuration, overwritten each run so it always describes
// the session being asked about.
static std::ofstream * st_LogFile()
{
    static std::ofstream * file = NULL;
    if ( !file )
    {
        // the var directory is not known during the very first lines, so keep
        // trying rather than giving up on the whole session
        tString path = rc_InternalPath( "retrocycles.log" );
        if ( path.Len() > 1 )
        {
            file = new std::ofstream( (const char *)path, std::ios::trunc );
