...
// Writes out the rules currently in force. Only the settings a server pushes
// are taken, so the file describes the game mode and carries nothing personal
// like names or bindings. Run it while connected, then load the file to play
// the same mode offline.
static void sg_ExportGameSettings( std::istream & s )
{
    tString filename;
    s >> filename;
    if ( filename.Len() <= 1 )
        filename = "server_settings.cfg";

    tString path = tDirectories::Var().GetWritePath( filename );
    std::ofstream out( (const char *)path );
    if ( !out.is_open() )
    {
        con << "Could not write " << path << "\n";
        return;
    }

    out << "# Game rules captured from the active session.\n";
    out << "# Load with: INCLUDE " << filename << "\n\n";
    ...
    for ( tConfItemBase::tConfItemMap::const_iterator i = items.begin(); i != items.end(); ++i )
    {
        ...
        // Network controlled settings are exactly the ones that define a mode.
        if ( !dynamic_cast< nConfItemBase * >( item ) )
            continue;
    ...
    out.close();
    con << "Exported " << count << " settings to " << path << "\n";
}

static tConfItemFunc sg_ExportGameSettingsConf( "EXPORT_SETTINGS", &sg_ExportGameSettings );
...
// The counter this used to switch was drawn by a widget that no longer exists;
// the setting survived it and did nothing at all. Kept only so that a config
// written by an older build loads without complaining about an unknown command.
...
int sg_modAmbientParticlesMin = 10;
...
static void sg_NoclipControlsMenu() {
    ...
    new uMenuItemModKey(&sub, "Fast Fly Key",       "Hold to fly quickly", sg_noclipKeyFast);
    new uMenuItemReal(&sub, "Fast Fly Factor", "How much faster the fast key flies", sg_noclipFastFactor, 1.0, 12.0, 0.5);
    new uMenuItemModKey(&sub, "Level Flight Key",   "Toggle: forward stays level however far you look up or down", sg_noclipKeyLevel);

    extern int sg_noclipKeySphere;
    extern REAL sg_noclipSphereRadius;

    new uMenuItemModKey(&sub, "Lock On Key", "Toggle: circle the followed player, mouse walks the camera round them", sg_noclipKeySphere);
    new uMenuItemReal(&sub, "Lock On Distance", "How far out the camera sits when locked on", sg_noclipSphereRadius, 4.0, 400.0, 5.0);

    extern int sg_noclipKeyMark, sg_noclipKeyMarkNext, sg_noclipKeyMarkPrev, sg_noclipKeyMarkDrop;
    extern bool sg_waypointPanel;

    new uMenuItemModKey(&sub, "Save Waypoint Key",   "Write down where the camera is standing", sg_noclipKeyMark);
    new uMenuItemModKey(&sub, "Next Waypoint Key",   "Fly to the next one along", sg_noclipKeyMarkNext);
    new uMenuItemModKey(&sub, "Prev Waypoint Key",   "Fly to the previous one", sg_noclipKeyMarkPrev);
    new uMenuItemModKey(&sub, "Delete Waypoint Key", "Forget the one you are standing on", sg_noclipKeyMarkDrop);
    new uMenuItemToggle(&sub, "Waypoint Panel",      "Show the list while the free camera is flying", sg_waypointPanel);
...
void sg_ModSettingsMenu() {
    ...
    uMenuItemFunction fc(&mod_menu, "Combat Settings >>",
        "Rubber gauge settings", sg_ModCombatMenu);
