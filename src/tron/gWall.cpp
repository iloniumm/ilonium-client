...
static rFileTexture gWallRim_vanilla(rTextureGroups::TEX_WALL, "textures/vanilla_rim_wall.png", 1, 0, 1);
...
// How much wall one turn of the texture covers.
//
// The coordinate below counts in SEGLEN units, so the picture repeats every
// two and a half of them - often enough that anything drawn on it reads as
// hatching rather than as a picture. Dividing the coordinate down spreads one
// turn over more wall.
REAL sg_wallTextureStretch = 1;
static tConfItem<REAL> sg_wts( "MOD_WALL_TEXTURE_STRETCH", sg_wallTextureStretch );

static inline REAL WallTexU( REAL t )
{
    // a reciprocal kept beside the setting, because this is called for every
    // vertex of every wall on screen and a divide there is not free
    static REAL known = -1;
    static REAL inverse = 1;
    if ( known != sg_wallTextureStretch )
    {
        known = sg_wallTextureStretch;
        inverse = known > 0 ? 1 / known : 1;
    }
    return t * inverse;
}
...
static void gWallRim_helper(eCoord p1,eCoord p2,REAL tBeg,REAL tEnd,REAL h,
    ...
    if (rc_NeonWalls()) {
        ...
        // The colour is decided here rather than before the call, because this
        // is where it actually lands: whatever is set outside is overwritten by
        // the next line either way.
        //
        // Taken per end rather than per wall, from a smooth field over the
        // arena's own coordinates. Neighbouring walls share an endpoint, so
        // they agree on the colour there and the blend runs round the rim in
        // one piece instead of nineteen.
        rcPaint bodyOne = { 0.06f, 0.20f, 0.60f };
        rcPaint bodyTwo = bodyOne;
        rcPaint capOne  = { 0.20f, 0.85f, 1.00f };
        rcPaint capTwo  = capOne;

        if ( rc_RimPainted() )
        {
            REAL along1 = 0.5f - 0.5f * cosf( ( p1.x + p1.y ) * 0.010f );
            REAL along2 = 0.5f - 0.5f * cosf( ( p2.x + p2.y ) * 0.010f );

            capOne = rc_RimPaint( along1 );
            capTwo = rc_RimPaint( along2 );

            // The body is the same colour held well back - it is glass with a
            // light behind it, not a painted panel, and at full strength it
            // swallows everything in front of it.
            bodyOne.r = capOne.r * 0.35f; bodyOne.g = capOne.g * 0.35f; bodyOne.b = capOne.b * 0.35f;
            bodyTwo.r = capTwo.r * 0.35f; bodyTwo.g = capTwo.g * 0.35f; bodyTwo.b = capTwo.b * 0.35f;
        }

        // 1. Sleek glass perimeter wall body
        BeginQuads();
        glColor4f(bodyOne.r, bodyOne.g, bodyOne.b, 0.75f);
        Vertex(p1.x, p1.y, 0);
        Vertex(p1.x, p1.y, h);
        glColor4f(bodyTwo.r, bodyTwo.g, bodyTwo.b, 0.75f);
        Vertex(p2.x, p2.y, h);
        Vertex(p2.x, p2.y, 0);
        RenderEnd();

        // 2. Glowing bright cyan top rim cap
        glLineWidth(2.5f);
        BeginLines();
        glColor4f(capOne.r, capOne.g, capOne.b, 1.0f);
        Vertex(p1.x, p1.y, h);
        glColor4f(capTwo.r, capTwo.g, capTwo.b, 1.0f);
        Vertex(p2.x, p2.y, h);
        RenderEnd();
    } else {
        if (sg_MoviePack()){
      ...
      if (sg_MoviePack()){
...

        {
            ...
            if (rc_NeonWalls()) {
                RenderEnd( true );
                glDisable(GL_TEXTURE_2D);
                glColor4f(0.10f * intensity, 0.35f * intensity, 0.85f * intensity, 0.85f);
            } else {
                RenderEnd( true );
...
void gNetPlayerWall::RenderList(bool list, gWallRenderMode renderMode ){
...
#ifndef DEDICATED
    // Asked here rather than in Render, which is not the path walls take: they
    // are drawn from their lists, and the first attempt guarded a door nobody
    // walks through.
    //
    // The wall is still there and still kills. This only stops it being drawn,
    // and never for the person riding it.
    if ( rc_HideOtherTrails() && !rc_IsMine( cycle_ ) )
        return;
#endif
...

    {
        REAL r,g,b;
        if (cycle_){
            REAL glow = rc_TrailBloom();
            r=cycle_->trailColor_.r * glow;
            g=cycle_->trailColor_.g * glow;
            b=cycle_->trailColor_.b * glow;

#ifndef DEDICATED
            // Only ever this machine's own wall. Repainting everybody's would
            // take away the one thing wall colour is actually for, which is
            // telling at a glance whose wall is in front of you.
            if ( rc_TrailPainted() && rc_IsMine( cycle_ ) )
            {
                // Where along the wall this piece sits, taken from the moment
                // it was laid. Eased with a cosine rather than a straight ramp
                // so the colours meet smoothly and there is no seam where the
                // pattern comes round again.
                REAL along = 0.5f - 0.5f * cosf( tBeg * 0.35f );

                rcPaint paint = rc_TrailPaint( along );
                r = paint.r * glow;
                g = paint.g * glow;
                b = paint.b * glow;
            }
...
//! The warning a wall gives before it comes down.
//!
//! It is a warning, so it starts when there is something to warn about and
//! grows more insistent as the moment arrives. It used to begin the instant
//! its owner died, which dimmed every wall on the grid by a third for the
//! whole time it was standing and told nobody anything.
//!
//! \param left seconds until this wall goes
static void rc_CorpseWarning( REAL left, REAL & r, REAL & g, REAL & b, REAL & a )
{
    REAL const warnFor = 3.0f;
    if ( left >= warnFor )
        return;

    float urgency = 1.0f - (float)left / (float)warnFor;
    if ( urgency < 0.0f ) urgency = 0.0f;
    if ( urgency > 1.0f ) urgency = 1.0f;

    // The phase comes from the clock, so neighbouring pieces of one wall
    // cannot drift apart and the whole length blinks together.
    float const blink = sinf( (float)se_GameTime() * ( 8.0f + 22.0f * urgency ) );
    float const depth = 0.15f + 0.25f * urgency;

    a *= 1.0f - depth + depth * blink;

    REAL const flash = 0.25f * urgency * ( 0.5f + 0.5f * blink );
    r += flash;
    g += flash;
    b += flash;
}
...
void gNetPlayerWall::RenderNormal(const eCoord &p1,const eCoord &p2,REAL ta,REAL te,REAL r,REAL g,REAL b,REAL a, gWallRenderMode mode ){
    ...
    if (bool(cycle_) && !cycle_->Alive() && totalDelay >= 0.0f ){
        REAL timeSinceDeath = se_GameTime() - cycle_->deathTime;
        REAL dt = (timeSinceDeath - totalDelay) * 2.0f;
        ...
        if (sg_corpseTrailStyle == 1) { // Blinking Warning (Competitive)
            if (dt < 0.0f)
                rc_CorpseWarning( -dt * 0.5f, r, g, b, a );
            if (dt >= 0.0f) {
                REAL ca = REAL(0.5f / (dt + 0.5f));
                REAL alpha = 1.0f - dt;
                hfrac = 1.0f - dt;
                r += ca;
                g += ca;
        ...
        } else if (sg_corpseTrailStyle == 2) { // Gradual Fade & Shrink
            if (timeSinceDeath >= 0.0f && timeSinceDeath <= totalDelay) {
                REAL ratio = timeSinceDeath / totalDelay;
                a *= (1.0f - ratio * 0.8f);
                if (ratio > 0.8f) {
                    REAL shrinkRatio = (1.0f - ratio) / 0.2f;
                    hfrac *= shrinkRatio;
                }
            }
            if (dt >= 0.0f) {
    ...
    // The height of a trail is the player's choice in either look. Tying it to
    // the enhanced mode meant the slider did nothing for anybody playing in the
    // plain one, with no hint as to why.
    ...
    if (hfrac>0){
        ...
        if ( mode & gWallRenderMode_Quads && sg_renderBulkQuads )
        {
            ...
            if (cfg_EnableInstancing && g_EnhancedGraphicsMode)
            {
                ...
                bool const shape = sg_modTrailGradientEnabled && !rc_WallFaithful();
                REAL a_top = shape ? (a * sg_modTrailAlphaTop) : a;
                REAL a_bottom = shape ? (a * sg_modTrailAlphaBottom) : a;

                TrailSegment seg;
                // Clamp all color channels to [0,1] to prevent
                // overexposed/burning colors from death-flash additions (+ca)
                // and pure black from underflow.
                float ceiling = (float)rc_TrailBloom();
                float rc = (r < 0.0f ? 0.0f : r > ceiling ? ceiling : r);
                float gc = (g < 0.0f ? 0.0f : g > ceiling ? ceiling : g);
                float bc = (b < 0.0f ? 0.0f : b > ceiling ? ceiling : b);
                float atc = (a_top    < 0.0f ? 0.0f : a_top    > 1.0f ? 1.0f : a_top);
                float abc = (a_bottom < 0.0f ? 0.0f : a_bottom > 1.0f ? 1.0f : a_bottom);
                seg.transform = mat4::transformation(p1.x, p1.y, p2.x, p2.y, h * hfrac, extrarise);
                seg.color = {rc, gc, bc, abc};
                seg.texCoords = {WallTexU(ta), WallTexU(te), hfrac, atc};
                g_collectedTrails.push_back(seg);
            }
            else
            {
                BeginQuads();
                ...
                extern REAL sg_modTrailAlphaTop;
                extern REAL sg_modTrailAlphaBottom;
                bool const shape2 = g_EnhancedGraphicsMode && sg_modTrailGradientEnabled && !rc_WallFaithful();
                REAL a_top = shape2 ? (a * sg_modTrailAlphaTop) : a;
                REAL a_bottom = shape2 ? (a * sg_modTrailAlphaBottom) : a;

                glColor4f(r,g,b,a_bottom);
                glTexCoord2f(WallTexU(ta),hfrac);
                glVertex3f(p1.x,p1.y,extrarise);
                
                glColor4f(r,g,b,a_top);
                glTexCoord2f(WallTexU(ta),0);
                glVertex3f(p1.x,p1.y,extrarise + h*hfrac);
                
                glColor4f(r,g,b,a_top);
                glTexCoord2f(WallTexU(te),0);
                glVertex3f(p2.x,p2.y,extrarise + h*hfrac);
                
                glColor4f(r,g,b,a_bottom);
                glTexCoord2f(WallTexU(te),hfrac);
...
#define seginv (1/float(segs))
    ...
    if( mode & gWallRenderMode_Quads && sg_renderBeginQuads )
    {
        ...
        if (cfg_EnableInstancing && g_EnhancedGraphicsMode)
        {
            ...
            for (int i = 0; i < segs; i++) {
                REAL frag1 = i * seginv;
                REAL frag2 = (i + 1) * seginv;
                REAL rat1 = ra + frag1 * (re - ra);
                REAL rat2 = ra + frag2 * (re - ra);

                REAL x1 = (p1.x + frag1 * (p2.x - p1.x)) * (1 - xfunc(rat1)) + ppos.x * xfunc(rat1);
                REAL y1 = (p1.y + frag1 * (p2.y - p1.y)) * (1 - xfunc(rat1)) + ppos.y * xfunc(rat1);

                REAL x2 = (p1.x + frag2 * (p2.x - p1.x)) * (1 - xfunc(rat2)) + ppos.x * xfunc(rat2);
                REAL y2 = (p1.y + frag2 * (p2.y - p1.y)) * (1 - xfunc(rat2)) + ppos.y * xfunc(rat2);

                REAL a_top1 = sg_modTrailGradientEnabled ? (a * afunc(rat1) * sg_modTrailAlphaTop) : (a * afunc(rat1));
                REAL a_top2 = sg_modTrailGradientEnabled ? (a * afunc(rat2) * sg_modTrailAlphaTop) : (a * afunc(rat2));
                REAL avg_a_top = (a_top1 + a_top2) * 0.5f;

                REAL a_bottom1 = sg_modTrailGradientEnabled ? (a * afunc(rat1) * sg_modTrailAlphaBottom) : (a * afunc(rat1));
                REAL a_bottom2 = sg_modTrailGradientEnabled ? (a * afunc(rat2) * sg_modTrailAlphaBottom) : (a * afunc(rat2));
                REAL avg_a_bottom = (a_bottom1 + a_bottom2) * 0.5f;

                REAL H1 = h * hfrac * hfunc(rat1);
                REAL H2 = h * hfrac * hfunc(rat2);
                REAL avgHeight = (H1 + H2) * 0.5f;
                ...
                float gc = (g < 0.0f ? 0.0f : g > ceiling ? ceiling : g);
                float bc = (b < 0.0f ? 0.0f : b > ceiling ? ceiling : b);
                float atc = (avg_a_top    < 0.0f ? 0.0f : avg_a_top    > 1.0f ? 1.0f : avg_a_top);
                float abc = (avg_a_bottom < 0.0f ? 0.0f : avg_a_bottom > 1.0f ? 1.0f : avg_a_bottom);
                seg.transform = mat4::transformation(x1, y1, x2, y2, avgHeight);
                seg.color = {rc, gc, bc, abc};
                seg.texCoords = {WallTexU(ta + (te - ta) * frag1), WallTexU(ta + (te - ta) * frag2), hfrac, atc};
                g_collectedTrails.push_back(seg);
            }
        }
        else
        {
            ...
            for (int i=0;i<=segs;i++){
                ...
                REAL a_top = sg_modTrailGradientEnabled ? (a * afunc(rat) * sg_modTrailAlphaTop) : (a * afunc(rat));
                REAL a_bottom = sg_modTrailGradientEnabled ? (a * afunc(rat) * sg_modTrailAlphaBottom) : (a * afunc(rat));

                // bottom
                glColor4f(r+cfunc(rat),g+cfunc(rat),b+cfunc(rat),a_bottom);
                glTexCoord2f(WallTexU(ta+(te-ta)*frag),hfrac);
                glVertex3f(x,y,0);

                // top
                glColor4f(r+cfunc(rat),g+cfunc(rat),b+cfunc(rat),a_top);
                glTexCoord2f(WallTexU(ta+(te-ta)*frag),0);
                REAL H=h*hfrac*hfunc(rat);
                glVertex3f(x+H*cycle_->skew*sfunc(rat)*cycle_->dir.y,
                           y-H*cycle_->skew*sfunc(rat)*cycle_->dir.x,
                           H);
...
bool gNetPlayerWall::IsDangerousAnywhere( REAL time ) const
{
    ...
    if ( totalDelay >= 0.0f )
    {
        if ( !cycle_->Alive() && time - cycle_->deathTime > .2f + totalDelay )
            return false;
    }
...
bool gNetPlayerWall::IsDangerousApartFromHoles( REAL a, REAL time ) const
{
    ...
    if ( totalDelay >= 0.0f )
    {
        ...
        if (!cycle_ || ( !cycle_->Alive() && cycle_->deathTime + totalDelay + 0.2f <= time ) )
            return false;
    }
