...
//! The machine's own colour, apart from the wall it lays. Local only.
extern bool sg_overrideCycleColor;
extern REAL sg_cycleColorR, sg_cycleColorG, sg_cycleColorB;

//! Draws a cycle at the origin, facing along positive x, in the given colour.
//!
//! The models and textures are the ones a real cycle uses. Anything else in a
//! preview is a drawing of a cycle rather than the cycle, and the whole point
//! of previewing a camera is seeing what the camera will actually frame.
...
public:
    ...
    REAL lastAutopilotTurnTime_;  // last time autopilot turned the cycle
...
public:
    ...
    // [MOD] Autopilot member
    void Autopilot_Tick_Member();
...
// [MOD] Global autopilot hook
void Autopilot_Tick(ePlayerNetID* player);

//! How far past white the bike and its trail are drawn, for the bright pass.
REAL rc_CycleBloom();

//! The same for the trail, which needs a smaller one.
REAL rc_TrailBloom();
//! Whether walls are drawn the way a plain build draws them.
bool rc_WallFaithful();
extern bool sg_wallFaithful;
