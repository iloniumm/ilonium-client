...
int ANET_GetSocketAddr (tSocket sock, struct sockaddr *addr);
...
int ANET_CloseSocket (tSocket sock)
{
...
nSocket::nSocket( void )
        :socket_( INVALID_SOCKET ), family_( PF_INET ), socktype_( SOCK_DGRAM ), protocol_( IPPROTO_UDP ), broadcast_( false )
{
}
...
int nSocket::Create( void )
{
    ...
    // If this is a TCP socket, disable Nagle's algorithm to ensure lowest possible latency
    if ( socktype_ == SOCK_STREAM || protocol_ == IPPROTO_TCP ) {
        int yes = 1;
    ...
    // OPTIMIZATION: use DSCP EF (Expedited Forwarding, 0xB8) for maximum
    // real-time priority on routers. This is the highest standard priority
    // for latency-sensitive traffic (VoIP/gaming).
    // Previous value was IPTOS_DSCP_AF32 (0x70).
    char dscp = 0xB8; // DSCP EF = Expedited Forwarding

    setsockopt( socket_, IPPROTO_IP, IP_TOS, (char*)&dscp, sizeof( dscp ) );
#endif

    // OPTIMIZATION: reduce socket receive buffer to minimize bufferbloat.
    // Default kernel buffer is often 128-256KB, which can hold many packets
    // and add latency. 32KB is enough for game traffic and reduces jitter.
    int rcvbuf = 32768;
    setsockopt( socket_, SOL_SOCKET, SO_RCVBUF, (char*)&rcvbuf, sizeof( rcvbuf ) );
...
void nSocket::MoveFrom( const nSocket & other )
{
    ...
    const_cast< nSocket & >( other ).socket_ = INVALID_SOCKET;
}
...
bool nBasicNetworkSystem::Select( REAL dt )
{
    ...
    {
        if ( controlSocket_.GetSocket() == INVALID_SOCKET )
        {
