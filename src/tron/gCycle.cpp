...
public:
    ...
    // Rebuilt from scratch every time its owner changes colour, which a death
    // does several times a second. Enlarging it there costs a tenth of a
    // second a time and the game stops dead.
    virtual bool WantsUpscale() const { return false; }
...
void gTextureCycle::OnSelect(bool enforce){
    ...
    if(rTextureGroups::TextureMode[rTextureGroups::TEX_OBJ]<0){
        // Whatever colour the machine ended up wearing - its own, the one
        // picked for it, or the one every enemy shares - it burns by the same
        // figure as everything else. The glow used to reach the headlight and
        // stop there, so a chosen colour looked flat next to its own trail.
        REAL glow = rc_CycleBloom();
        REAL R=color_.r*glow,G=color_.g*glow,B=color_.b*glow;
        if(wheel){
            R*=.7;
            G*=.7;
            B*=.7;
...
// create wall renderer
    ...
    // [MOD] Initialize intent variables
    previousFrameRubber = 0.0f;
    lastAutopilotTurnTime_ = -100.0f;
...
// The machine's own colour, apart from the wall it lays.
//
// The client could already recolour a player, but body and trail moved
// together - the trail colour is derived from the cycle's. Wanting a dark
// machine behind a bright wall, or the other way round, was not expressible.
// This overrides only the body, and only for the seats at this machine: it is
// a local view of a remote player, not a claim about them.
bool sg_overrideCycleColor = false;
static tConfItem< bool > sg_occConf( "MOD_CYCLE_COLOR_OVERRIDE", sg_overrideCycleColor );

REAL sg_cycleColorR = 0.20f, sg_cycleColorG = 0.85f, sg_cycleColorB = 1.00f;
static tConfItem< REAL > sg_occR( "MOD_CYCLE_COLOR_R", sg_cycleColorR );
static tConfItem< REAL > sg_occG( "MOD_CYCLE_COLOR_G", sg_cycleColorG );
static tConfItem< REAL > sg_occB( "MOD_CYCLE_COLOR_B", sg_cycleColorB );
...
bool gCycle::Timestep(REAL currentTime){
    if (Player()) {
        ...
        // The body only. Taken after the trail has had its own colour, so the
        // two are free to disagree.
        if ( sg_overrideCycleColor && rc_IsMine( this ) )
        {
            color_.r = sg_cycleColorR;
            color_.g = sg_cycleColorG;
            color_.b = sg_cycleColorB;
        }
    }
    ...
    else if ( !currentDestination && pendingTurns.empty() )
    {
        ...
        // chatting or imgui open? activate autopilot
        bool chatActive = (player && player->IsHuman() && player->IsChatting() && player->Owner() == sn_myNetID);
        bool imguiActive = false;
#ifndef DEDICATED
        imguiActive = ModMenu::IsOpen();
#endif
        if ((chatActive || imguiActive) && player && player->Owner() == sn_myNetID)
        {
            Autopilot_Tick(player);
        }
        else if ( bool(player) &&
                player->IsHuman() &&
    ...
    // [MOD] Store previous frame rubber for NEXT frame's calculation
    previousFrameRubber = GetRubber();
    return ret;
}
...
bool gCycle::TimestepCore(REAL currentTime, bool calculateAcceleration ){
    ...
    if (Alive()){
        ...
        */
        ...
        // Dynamic Rubber & Drift Electric Sparks Spawning (Throttled for 200+ FPS Performance!)
        static REAL lastRubberSparkTime = 0.0f;
        if (rubber > 0.05f && animts > 0 && grid && (currentTime - lastRubberSparkTime > 0.12f)) {
            lastRubberSparkTime = currentTime;
            tRandomizer & randomizer = tRandomizer::GetInstance();
            eCoord sparkpos = pos - dirDrive * 0.2f;
            eCoord sparkdir = dirDrive.Turn(randomizer.Get() - 0.5f, randomizer.Get() - 0.5f) * 1.5f;
            new gSpark(grid, sparkpos, sparkdir, currentTime, color_.r, color_.g, color_.b, 0.0f, 0.85f, 1.0f);
        }
...
void gCycle::Die( REAL time )
{
    ...
    if (Alive()) {
        ...
        // The one place both a local game and a server pass through.
        //
        // On a server the scoring happens in KillAt below, which a client
        // never reaches - that early return is why a kill on somebody else's
        // server was never counted here at all. But a client is told about
        // every death by the server, and it arrives through this function; who
        // did it is worked out from the same wall the server uses, recorded by
        // the same simulation on this side.
        ePlayerNetID const * constHunter = enemyInfluence.GetEnemy();
        if ( constHunter && lastTime - enemyInfluence.GetTime() > sg_suicideTimeout )
            constHunter = NULL;

        rc_StatsDeath( player, constHunter );
#endif
    }
...
// How hard the bike and its trail are pushed past white.
//
// The scene is drawn into a float buffer, so a colour is allowed to come out
// above one and stay there. Nothing on screen looks different for it - the
// tonemap folds it back down - but the bright pass sees the overshoot and
// spreads it, which is what separates a thing that glows from a thing that is
// merely light coloured. Every glowing part of the game gets its own figure
// this way instead of sharing one threshold.
bool sg_cycleBloom = true;
static tConfItem< bool > sg_cycleBloomConf( "MOD_CYCLE_BLOOM", sg_cycleBloom );

REAL sg_cycleBloomPower = 3.0f;
static tConfItem< REAL > sg_cycleBloomPowerConf( "MOD_CYCLE_BLOOM_POWER", sg_cycleBloomPower );

// The trail carries its own figure. Its shader multiplies what it is handed by
// another one and a half to two and a bit on the way out, so the same number
// on both would leave the wall burning while the bike merely glowed.
REAL sg_trailBloomPower = 1.0f;
static tConfItem< REAL > sg_trailBloomPowerConf( "MOD_TRAIL_BLOOM_POWER", sg_trailBloomPower );

bool rc_BloomActive();

REAL rc_CycleBloom()
{
    return ( sg_cycleBloom && sg_cycleBloomPower > 1 && rc_BloomActive() ) ? sg_cycleBloomPower : 1;
}

//! Whether anything is there to spread the overshoot.
//!
//! Drawing past white with nothing to catch it just makes a white shape, so
//! when the pass is off the arena goes back to drawing itself plainly.
bool rc_BloomActive()
{
    extern bool sg_postProcessingEnabled;
    return sg_postProcessingEnabled;
}

// Walls exactly as a plain build draws them.
//
// Ours does three things to a wall that no other client does: it scales the
// colour up for the glow, it shapes the alpha into a gradient, and the shader
// adds a brighter body, a travelling pulse and a white cap. All of that is
// ours and all of it is on top of whatever texture a pack supplies - so a
// texture made for a normal build, and especially one that is meant to be
// slightly see-through, comes out too bright and too solid here. This turns
// the three of them off and leaves colour times texture, which is all a wall
// ever was.
bool sg_wallFaithful = false;
static tConfItem<bool> sg_wallFaithfulConf( "MOD_WALL_FAITHFUL", sg_wallFaithful );

bool rc_WallFaithful()
{
    return sg_wallFaithful;
}
...
REAL rc_TrailBloom()
{
    ...
    return ( sg_cycleBloom && sg_trailBloomPower > 1 && rc_BloomActive() ) ? sg_trailBloomPower : 1;
}

static rFileTexture dir_eWall_vanilla(rTextureGroups::TEX_WALL,"textures/vanilla_dir_wall.png",1,0,1);
...
bool gCycleWallsDisplayListManager::CannotHaveList( REAL distance, gCycle const * cycle )
{
    ...
    // A warning that blinks has to be drawn again to blink. Baked into a
    // display list its colour is frozen where it was baked, which is why the
    // blinking was only ever seen on the pieces that cannot be baked - the one
    // still growing at the bike and the one already coming down. The rest of
    // the wall stood there lit exactly as it was at the moment somebody died.
    extern int sg_corpseTrailStyle;
    if ( !cycle->Alive() && totalDelay >= 0.0f && sg_corpseTrailStyle == 1 )
    {
        // Only while it is actually blinking. Before that the wall is drawn
        // like any other and there is no reason to make the machine redraw a
        // whole grid of dead men's walls every frame.
        REAL const left = totalDelay - ( se_GameTime() - cycle->DeathTime() );
        if ( left < 3.0f )
            return true;
    }
...
void gCycleWallsDisplayListManager::RenderAll( eCamera const * camera, gCycle * cycle )
{
    ...
    if (cfg_EnableInstancing && g_EnhancedGraphicsMode) {
        ...
        // Render all walls directly (so they get collected)
        RenderAll( camera, cycle, wallList_ );
        RenderAll( camera, cycle, wallsWithDisplayList_ );
        
        // Draw the collected instanced trails and clear!
...
void gCycle::UpdateTexturesColor() {
#ifndef DEDICATED
    // Only recreate if color has actually changed
    ...
    if (now - lastTexUpdateTime_ < 0.033) {
        return; // Limit texture updates to 30 FPS
    }
...
void rc_RenderCycleModel( REAL r, REAL g, REAL b )
{
#ifndef DEDICATED
    static gCycleVisuals * kit = 0;
    static REAL was[3] = { -1, -1, -1 };

    rModel * hull = rModel::GetModel( "models/cycle_body.mod" );
    if ( !hull )
        return;

    gRealColor want;
    want.r = r; want.g = g; want.b = b;
    se_MakeColorValid( want.r, want.g, want.b, 1.0f );

    if ( !kit || was[0] != want.r || was[1] != want.g || was[2] != want.b )
    {
        delete kit;
        kit = tNEW( gCycleVisuals )( want );

        if ( !kit->LoadModel( false ) )
        {
            delete kit;
            kit = 0;
            return;
        ...
        was[0] = want.r; was[1] = want.g; was[2] = want.b;
    }

    // The hull alone. Standing on its own outside a round, the wheels sit at
    // the ends of a shape barely twenty pixels long and read as two loose
    // blocks rather than as part of it; the hull is what makes the silhouette
    // recognisable from behind, which is all this view is for.

    rDisplayListUsage cached = sr_useDisplayLists;
    ...
    if ( kit->mpType && kit->customModel && kit->customTexture )
    {
        kit->customTexture->Select();
        kit->customModel->Render();
    }
    else if ( kit->bodyTexture )
    {
        glTranslatef( -1.5f, 0, 0 );

        kit->bodyTexture->Select();
        hull->Render();
    }

    glPopMatrix();

    sr_useDisplayLists = cached;
#endif
}

namespace
{
//! Keeps the stamp on for as long as the bike is being drawn, however it leaves.
struct rcBloomStamp
{
    rcBloomStamp()  { rc_BloomExclude( true ); }
    ~rcBloomStamp() { rc_BloomExclude( false ); }
};
}
...
void gCycle::Render(const eCamera *cam){
    rcBloomStamp stamp;
#ifndef DEDICATED
    // Hiding somebody else's machine takes their name with it: a name floating
    // over nothing is worse than either. Only ever other people's - a rider
    // who cannot see their own has no way to play at all.
    if ( rc_HideOtherCycles() && !rc_IsMine( this ) )
        return;
    ...
    if (Alive()){
        ...
        if (mp){
            ...
            if ( !blinking )
            {
                ...
                {
                    // The colour is baked into this texture, so the only way
                    // to push it past white is through the modulate.
                    REAL glow = rc_CycleBloom();
        ...
        */
...

        {
            ...
            if ( renderPyramid )
            {
                ...
                REAL pyramidGlow = rc_CycleBloom();
                glColor4f( colorPyramid.r * pyramidGlow,colorPyramid.g * pyramidGlow,colorPyramid.b * pyramidGlow, alpha );
                glVertex3f(0,0,3);
                glVertex3f(0,1,4.5);
                glVertex3f(0,-1,4.5);

                glColor4f( colorPyramid.r * .7f * pyramidGlow,colorPyramid.g * .7f * pyramidGlow,colorPyramid.b * .7f * pyramidGlow, alpha );
                glVertex3f(0,0,3);
                glVertex3f(1,0,4.5);
                glVertex3f(-1,0,4.5);
        ...
        if(headlights) {
            ...
            {
                REAL glow = rc_CycleBloom();
                glColor3f( color_.r * glow, color_.g * glow, color_.b * glow );
            }
        ...
        // Hearts drifting up out of your own wake.
        //
        // Worked out from the clock and the machine's own position rather than
        // kept in a list: nothing to allocate, nothing to update, nothing to
        // leak when a round ends, and it costs one batch however many are in
        // the air. That mattered here - the last thing that threw hearts about
        // took the frame rate with it.
        if ( rc_HeartTrail() && rc_IsMine( this ) && Alive() )
        {
            int howMany = rc_HeartTrailCount();
            REAL now = lastTime;

            eCoord back = Direction();
            eCoord side( -back.y, back.x );

            BeginTriangles();

            for ( int i = 0; i < howMany; ++i )
            {
                // Each one is a little further along its own rise, so they
                // form a stream rather than appearing together in a clump.
                REAL phase = fmodf( now * 0.9f + (REAL)i / (REAL)howMany, 1.0f );

                REAL trail = 0.6f + phase * 3.4f;          // how far behind
                REAL lift = phase * 2.6f;                  // how high it has got
                REAL drift = sinf( phase * 6.2f + i ) * 0.45f;

                // Thins out as it climbs, so they disappear rather than
                // stopping.
                REAL alpha = ( 1.0f - phase ) * 0.85f;

                rc_HeartSolidInto( pos.x - back.x * trail + side.x * drift,
                                   pos.y - back.y * trail + side.y * drift,
                                   0.35f + lift,
                                   0.13f,
                                   phase * 5.0f + i,
                                   alpha );
...
#endif
        {
            ...
            // [MOD] Cinematic mode suppression
            extern bool sg_noclipCinematic;
        ...
        // [MOD] Recursive Tree-Search Pathfinding Escape Vector
        // Branches at every wall: forward / left 90° / right 90°
        // Picks the route with the longest total survival distance (depth 4)
        // Only renders for the local player's camera-followed cycle

    } else {
        // Render Corpse Disappearance Timer
            ...
            if (rem > 0.0f && rem <= totalDelay) {
                ...
                if (wp > 0) {
                    ...
                    if (xp > -1 && xp < 1 && yp > -1 && yp < 1) {
                        ...
                        // Green to Red gradient as it decays
                        float r = 1.0f - ratio;
                        float g = ratio;
                        float b = 0.1f;
...
static tConfItem< bool > sg_corpseTimerConf( "CORPSE_TIMER_HUD", sg_corpseTimerEnabled );

bool sg_corpseTimerOverride = false;
static tConfItem< bool > sg_corpseTimerOverrideConf( "CORPSE_TIMER_OVERRIDE", sg_corpseTimerOverride );

float sg_corpseTimerDuration = 8.0f;
static tConfItem< float > sg_corpseTimerDurationConf( "CORPSE_TIMER_DURATION", sg_corpseTimerDuration );

int sg_corpseTrailStyle = 1;
static tConfItem< int > sg_corpseTrailStyleConf( "CORPSE_TRAIL_STYLE", sg_corpseTrailStyle );
...
REAL sg_modNameSizeScale = 1.0;
static tConfItem< REAL > sg_modNameSizeScaleConf( "MOD_PLAYER_NAME_SCALE", sg_modNameSizeScale );

bool sg_modPlainNames = false;
static tConfItem< bool > sg_modPlainNamesConf( "MOD_PLAIN_CYCLE_NAMES", sg_modPlainNames );
...
void gCycle::RenderName( const eCamera* cam ) {
    ...
    // [MOD] Suppress names and gauges in Cinematic Mode
    extern bool sg_noclipCinematic;
    ...
    // Mod: always show the name/gauge, do not fade out.
    /*
    ...
    */
    ...
    if ( sg_modPlainNames )
    {
        // strip the colour codes so the label reads in plain white; only the
        // name over the cycle is affected, the scoreboard and chat keep theirs
        nameToRender = tColoredString::RemoveColors( nameToRender );
    }
    if ( rc_SharpNames() )
    {
        // Handed to the interface layer instead of drawn here. The engine's
        // font is a bitmap sheet built for 640x480: over a cycle it lands on
        // fractional pixels and the letters come out uneven, which is the one
        // thing a name you read mid-corner cannot afford. Same place, same
        // size, same colours - only the glyphs are the crisp ones.
        rc_QueueCycleName( xp, yp, nameToRender,
                           rCHEIGHT_NORMAL * sg_modNameSizeScale, alpha );
    }
    else
    {
        rTextField::SetBlendColor(tColor(1, 1, 1, alpha));
        DisplayTextAutoWidth(xp, yp, nameToRender, rCHEIGHT_NORMAL * sg_modNameSizeScale, 0, 0);
...
{
#ifndef DEDICATED
    if (tRecorder::IsPlayingBack()) return false;
#endif
    return Alive() && lastTime > spawnTime_ + sg_cycleInvulnerableTime;
}

// [MOD] Autopilot implementation
void Autopilot_Tick(ePlayerNetID* player)
{
    if (player)
    {
        ...
        if (cycle)
        {
            cycle->Autopilot_Tick_Member();
        }
    }
}

void gCycle::Autopilot_Tick_Member()
{
    if (!Alive())
        return;

    // Spawn / Zero Speed Protection / Start Immunity Timer
    if (Speed() < 1.0f)
        return;
    if (lastTime < spawnTime_ + 2.0f)
        return;

    // Turn Cooldown (min 150 ms)
    if (lastTime < lastAutopilotTurnTime_ + 0.15f)
        return;

    eCoord currentPos = Position();
    eCoord fwdDir = Direction();

    // 1. Ignore self (Raycast Origin Offset)
    eCoord rayStart = currentPos + fwdDir * 0.5f;

    // 2. Distance Clamping
    REAL critDist = Speed() * 0.5f;
    if (critDist < 1.0f) critDist = 1.0f;
    if (critDist > 15.0f) critDist = 15.0f;

    // Forward sensor check
    gSensor fwdSensor(this, rayStart, fwdDir);
    fwdSensor.detect(critDist);
    REAL fwdDist = fwdSensor.hit;

    // Draw forward ray debug line
    eDebugLine::SetTimeout(0.05f);
    if (fwdDist < critDist)
    {
        eDebugLine::SetColor(1.0f, 0.0f, 0.0f); // Red if blocked
    }
    else
    {
        eDebugLine::SetColor(0.0f, 1.0f, 0.0f); // Green if clear
    }
    eDebugLine::Draw(rayStart, 0.5f, rayStart + fwdDir * fwdDist, 0.5f);

    // If wall is closer than critical distance, perform avoidance maneuver
    if (fwdDist < critDist)
    {
        // Calculate side directions
        int leftWN = windingNumberWrapped_;
        Grid()->Turn(leftWN, -1);
        eCoord leftDir = Grid()->GetDirection(leftWN);

        int rightWN = windingNumberWrapped_;
        Grid()->Turn(rightWN, 1);
        eCoord rightDir = Grid()->GetDirection(rightWN);

        // Side sensors check (Side Distance: 4 meters)
        gSensor leftSensor(this, rayStart, leftDir);
        leftSensor.detect(4.0f);
        REAL leftSpace = leftSensor.hit;

        gSensor rightSensor(this, rayStart, rightDir);
        rightSensor.detect(4.0f);
        REAL rightSpace = rightSensor.hit;

        // Draw side ray debug lines
        eDebugLine::SetColor(0.0f, 0.5f, 1.0f); // Cyan for side checks
        eDebugLine::Draw(rayStart, 0.5f, rayStart + leftDir * leftSpace, 0.5f);
        eDebugLine::Draw(rayStart, 0.5f, rayStart + rightDir * rightSpace, 0.5f);
        ...
        {
            turnDir = 1;  // Equal or both free, default to right
        }

        bool turned = false;
        // Try to turn in the decided direction if allowed, otherwise try opposite
        if (turnDir == -1)
        {
            if (CanMakeTurn(-1))
            {
        ...
        if (turned)
        {
            lastAutopilotTurnTime_ = lastTime;
        }
    }
}
