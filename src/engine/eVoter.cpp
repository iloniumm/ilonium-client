...
//! Ask for somebody to be kicked, exactly as the old menu item did.
//!
//! On a server it happens; anywhere else it becomes a suggestion for everyone
//! to vote on. Exported so the client can offer it without going through a
//! menu built out of uMenuItems.
void se_RequestKick( ePlayerNetID * p )
{
    if ( !p )
        return;
