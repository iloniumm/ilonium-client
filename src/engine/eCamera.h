...
typedef enum {CAMERA_IN=0, CAMERA_CUSTOM=1, CAMERA_FREE=2, CAMERA_FOLLOW=3,
              CAMERA_SMART=4,  CAMERA_SERVER_CUSTOM=5, CAMERA_SMART_IN=6,
              CAMERA_NOCLIP=7, CAMERA_COUNT=8 } eCamMode;
...
//! The custom camera's own numbers, read and written.
//!
//! They live inside the camera and stay there; this is the seam a tool needs
//! to show somebody what those numbers actually look like from the seat.
//! All eight of them, in the order the tool shows them: how far back, how high,
//! the tilt, and the four that only matter while the machine is moving or
//! turning, plus the zoom the custom camera applies on top.
struct rcCameraSet
{
    REAL back, rise, pitch, zoom;
    REAL backFromSpeed, riseFromSpeed;
    REAL turnSpeed, turnSpeed180;
};

//! Sets the field of view on every seat at this machine, now rather than at
//! the next round.
//!
//! The stored one is only read when a camera is built, so writing it alone
//! leaves the picture exactly as it was and reads as a slider that does
//! nothing. This writes both.
...
public:
    ...
    //! Widens or narrows the shot now. See rc_CameraFOV.
    void SetFOV( REAL f ){ fov = f; }
...
//! Switches the free camera off, wherever we are.
void rc_NoclipStop();

extern bool sg_waypointsVisible;
extern int  sg_noclipKeyMarkHide;

//! The keys the free camera answers to. Plain settings rather than actions,
//! so whatever puts them on screen needs them by name.
extern int sg_noclipKeyForward, sg_noclipKeyBack, sg_noclipKeyLeft, sg_noclipKeyRight;
extern int sg_noclipKeyUp, sg_noclipKeyDown, sg_noclipKeyZoomIn, sg_noclipKeyZoomOut;
extern int sg_noclipKeyCenter, sg_noclipKeyLookAt, sg_noclipKeyLock, sg_noclipKeyFollow;
extern int sg_noclipKeyNextPl, sg_noclipKeyPrevPl, sg_noclipKeySlow, sg_noclipKeyFast;
