...
static REAL se_maxLagSpeedup=.5;        // maximal speed increase of timer while lag is compensated for (was .2 — faster recovery)
static REAL se_lagSlowDecayTime=10.0;   // timescale the slow lag measurement decays on (was 30 — faster adaptation)
static REAL se_lagFastDecayTime=1.0;    // timescale the fast lag measurement decays on (was 2 — quicker reaction)
