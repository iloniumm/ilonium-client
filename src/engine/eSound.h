...
//! Checks the playback device is still alive and opens a new one if it is not.
void se_SoundKeepAlive();

//! short click played when a feature is switched on or off
void se_PlayUiClick();

//! How loud interface sounds are, nought to one.
extern REAL se_uiVolume;

//! Plays a short interface sound by name, from sound/ui/<name>.wav.
//! Known: click, hover, select, back, open, confirm, deny.
void se_PlayUi( char const * name );

// mixer setup; quality and buffer size only take hold after a sound restart
enum { se_SOUND_OFF = 0, se_SOUND_LOW, se_SOUND_MED, se_SOUND_HIGH };
...
struct LegacyAudioSpec {
    int freq;
    unsigned int format;
    unsigned char channels;
    unsigned short samples;
    void (SDLCALL *callback)(void *userdata, unsigned char *stream, int len);
    void *userdata;
};
...
class eWavData: public tListItem<eWavData>{
    LegacyAudioSpec spec; // the audio format
