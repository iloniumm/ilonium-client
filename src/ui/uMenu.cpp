...
{
    return su_inMenu || ModMenu::g_MainMenuActive || ModMenu::g_InGameMenuOpen;
}

bool uMenu::IsLegacyMenuActive()
{
    return su_inMenu;
}
...
#ifndef DEDICATED
        // Ensure text input is started if a text field is selected
        bool isTextField = (dynamic_cast<uMenuItemString*>(items[selected]) != nullptr);
        if (isTextField) {
...
bool uMenuItemString::Event(SDL_Event &event){
#ifndef DEDICATED
    switch (event.type) {
    case SDL_EVENT_TEXT_INPUT: {
        for (int i = 0; event.text.text[i] != '\0';) {
            unsigned char b1 = event.text.text[i];
            if ((b1 == 208 || b1 == 209) && event.text.text[i + 1] != '\0') {
                unsigned char b2 = event.text.text[i + 1];
    ...
    case SDL_EVENT_KEY_UP: {
        ...
        bool isControlKey = (key == SDLK_ESCAPE || key == SDLK_RETURN || key == SDLK_KP_ENTER ||
                             key == SDLK_UP || key == SDLK_DOWN || key == SDLK_TAB ||
    ...
    case SDL_EVENT_KEY_DOWN: {
        ...
        bool moveWordLeft = false, moveWordRight = false, deleteWordLeft = false, deleteWordRight = false;
        bool moveBeginning = false, moveEnd = false, killForwards = false;
...
bool uMenuItemStringWithHistory::Event(SDL_Event &e)
{
    ...
    if (e.type == SDL_EVENT_KEY_DOWN
            && ((e.key.key == SDLK_UP)
                || (e.key.key == SDLK_P && mod & KMOD_CTRL)))
    {
        ...
        ret = true;
             ...
             && ((e.key.key == SDLK_DOWN)
                 || (e.key.key == SDLK_N && mod & KMOD_CTRL)))
    {
        if (m_HistoryPos > 0)
        {
            m_HistoryPos--;
