...
extern ImFont* g_FontDisplay;   //!< headings meant to be read across a room
extern ImFont* g_FontMono;      //!< figures that should line up in a column

// Multi-size font array for crisp rendering at any scale
// Sizes: 10, 12, 14, 16, 18, 20, 24, 28, 32 px
#define MOD_FONT_SIZE_COUNT 9
extern ImFont* g_FontSizes[MOD_FONT_SIZE_COUNT];  // index matches sizes below
extern ImFont* g_FontNames[MOD_FONT_SIZE_COUNT];  // the face for names over cycles
static const float g_FontSizeValues[MOD_FONT_SIZE_COUNT] = {10.f, 12.f, 14.f, 16.f, 18.f, 20.f, 24.f, 28.f, 32.f};

// Helper: find closest loaded font size
//! The closest baked size of the face used for names over cycles.
inline ImFont* GetBestNameFont(float targetSize) {
    int best = 2;
    float bestDiff = 9999.f;
    for (int i = 0; i < MOD_FONT_SIZE_COUNT; i++) {
        if (!g_FontNames[i]) continue;
        float diff = fabsf(g_FontSizeValues[i] - targetSize);
        if (diff < bestDiff) { bestDiff = diff; best = i; }
    }
    return g_FontNames[best] ? g_FontNames[best] : ImGui::GetFont();
}

inline ImFont* GetBestFont(float targetSize) {
    int best = 2; // default = 14px
    float bestDiff = 9999.f;
    for (int i = 0; i < MOD_FONT_SIZE_COUNT; i++) {
        if (!g_FontSizes[i]) continue;
        float diff = fabsf(g_FontSizeValues[i] - targetSize);
        if (diff < bestDiff) { bestDiff = diff; best = i; }
    }
    return g_FontSizes[best] ? g_FontSizes[best] : ImGui::GetFont();
}
...
//! The game's own chat, drawn in the client's style.
//!
//! Owns nothing but the look: which lines appear and for how long is still
//! the engine console's business. Lives in the HUD editor like any gauge, so
//! it is moved, sized and styled where everything else is.
extern bool sg_chatSkin;

//! The line you type into. Handed over by the engine's own chat item, which
//! keeps doing the typing: history, completion, word delete, all of it.
void rc_QueueChatInput( char const * text, int cursor, bool team );
//! Whether our own line is drawn, so the old one can stay out of the way.
bool rc_ChatInputSkinned();
//! Draws the queued line inside our own frame.
void rc_DrawChatInput();

//! The word in the middle of the screen.
void rc_DrawCenterMessage();

//! A short line in the middle of the screen that fades on its own. Drawn with
//! the rest of the HUD, so whatever hides the HUD hides this too.
void rc_HudFlash( char const * text, double seconds );

//! The count in, as a light gantry rather than three numbers.
void rc_DrawStartLights();
//! Whether the lights are counting, so nothing else counts alongside them.
bool rc_StartLightsUp();

//! Alive, as the score table on Tab says it.
/**
 *  One answer in one place. The table, the live scoreboard and the teammate
 *  warning each used to work this out for themselves, and a warning that
 *  disagrees with the table somebody is looking at is a warning nobody
 *  believes. A player with no object at all is not dead - the game is between
 *  rounds or rebuilding them - and that is not the same question.
 */
class ePlayerNetID;
bool rc_PlayerIsAlive( ePlayerNetID const * p );

extern bool sg_chatBar;
extern int  sg_startLights;
extern REAL sg_startLightsSize;
extern REAL sg_startLightsY;

class ChatSkinWidget : public HudWidget {
public:
    ChatSkinWidget();
    void Draw() override;
...
extern REAL sg_modCenterMessagePosX;
extern REAL sg_modCenterMessagePosY;
...
extern bool sg_sharpNames;

//! Whether cycle names are drawn with the interface font.
bool rc_SharpNames();

//! Hands one cycle name to the interface layer to be drawn this frame.
//!
//! Position is where the engine had already projected the label, in the
//! -1..1 the projection leaves behind; height is in the same units, so the
//! label keeps whatever size the old path gave it.
void rc_QueueCycleName( REAL ndcX, REAL ndcY, tString const & text,
                        REAL ndcHeight, REAL alpha );

//! Draws and clears whatever was queued. Called from inside the frame.
