...
bool su_GetSDLInput(SDL_Event &tEvent,REAL &time){
    ...
    //
    // The menu answers "handled" for nearly everything while it is up, and the
    // event's type is then replaced below - fine for the game, fatal for a
    // blocking dialog of ours that reads these events itself: a typed letter
    // arrives as a blank service event and cannot be recognised. While such a
    // dialog owns the screen the events pass through whole, and it forwards
    // them to the interface itself.
    extern bool rc_ModalOverlayOpen();

    if (ret && !rc_ModalOverlayOpen()) {
