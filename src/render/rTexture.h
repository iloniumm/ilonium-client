...
{
public:
    //! Whether enlarging this one is worth the wait.
    //!
    //! Almost always yes: a texture is read from disk once and lives for the
    //! session. The exception is anything the game builds again while it is
    //! running - a cycle repainted as its owner's colour changes goes through
    //! here dozens of times a second, and a filter that costs a tenth of a
    //! second cannot be run on that.
    virtual bool WantsUpscale() const { return true; }
...
public:
    ...
    bool Loaded(){ return textureModeLast_ >= 0 && uploaded_; }  //!< returns whether the texture is currently loaded
...
private:
    ...
    bool uploaded_;         //!< flag indicating if image data was uploaded to GL
};
