...
static bool se_SmoothTime()
{
    ...
    {
        // OPTIMIZATION: force disable smoothing on Linux.
        // tTimerIsAccurate() is always true on Linux (gettimeofday/clock_gettime).
        // Smoothing adds ~1 frame of perceived input lag with no benefit.
        smooth = false;
    }
...
REAL se_GameTime(){
    ...
    if (DemoPlayerManager::Instance().IsViewerActive() && DemoPlayerManager::Instance().IsLoaded()) {
        return DemoPlayerManager::Instance().CurrentTime();
    }
#endif
    if (se_mainGameTimer)
...
{
    // OPTIMIZATION: reduced from .5 to .25 for tighter visual accuracy
    // at high FPS, less overshooting of game object positions
    return se_AverageFrameTime()*.25;
}
