...
public:
    ...
    tString     teamName;        // custom team name preferred by player
...
private:
    ...
    bool nameTeamAfterMe; 					// player prefers to call his team after his name
    tString teamName;                       // custom team name
...
public:
    ...
    //! the colour as configured, before it is lifted clear of the floor
    void RawColor( REAL&r, REAL&g, REAL&b ) const;
...
public:
    ...
    tColoredString const & GetColoredName( void ) const;	//!< Gets this player's name, cleared by the server. Use this for onscreen screen display.
    ePlayerNetID const & GetColoredName( tColoredString & coloredName ) const;	//!< Gets this player's name, cleared by the server. Use this for onscreen screen display.
...
//! Whether the player is asking to see the table right now.
bool se_ScoresWanted();

//! Whether the client draws its own table rather than the built-in one.
extern bool sg_customScoreboard;

//! How solid that table is drawn. Glanced at mid-round, so it leans see-through.
extern REAL sg_scoreboardOpacity; // show scores based on user input
...
// GetColoredName is defined in ePlayer.cpp
