...
void gLogo::Display()
{
    ...
    {
        std::ifstream f;
        if (tDirectories::Data().Open(f, "moviepack/title.jpg")) {
            f.close();
            sg_LogoMPTitle = tNEW(rFileTexture)(rTextureGroups::TEX_FONT, "moviepack/title.jpg",0,0,1);
        } else {
            sg_LogoMPTitle = tNEW(rFileTexture)(rTextureGroups::TEX_FONT, "moviepack/title.png",0,0,1);
        }
        sg_DisplayStatus = 1;
    }
...
#ifndef KRAWALL
        std::ifstream f;
        if (tDirectories::Data().Open(f, "textures/title.jpg")) {
            f.close();
