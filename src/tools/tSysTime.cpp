...
void GetTime( tTime & time )
{
    // OPTIMIZATION: use clock_gettime(CLOCK_MONOTONIC) instead of gettimeofday.
    // - Nanosecond precision vs microsecond (10x more precise)
    // - Monotonic: immune to NTP adjustments and wall-clock jumps
    // - Slightly faster syscall on modern Linux kernels (vDSO)
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);

    time.microseconds = ts.tv_nsec / 1000; // convert ns to us
...
bool tTimerIsAccurate()
{
    return true; // always on unix, especially with CLOCK_MONOTONIC
}
...
void tAdvanceFrameSys( tTime & start, tTime & relative )
{
    ...
    {
        // The clock jumped. On a desktop this is almost always the window being
        // minimised: the process stops being scheduled, comes back seconds
        // later, and the difference looks like a fault. The correction below is
        // the part that matters; telling the player about it only ever put a
        // line in their chat that they could do nothing about.
...
float tGetTimeFactor() {
    return st_timeFactor;
}

void tSetTimeFactor(float f) {
    st_timeFactor = f;
}
