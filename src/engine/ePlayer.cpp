...
#ifndef DEDICATED
extern bool ModMenu_WantsTextInput();
#endif
#include "eTeam.h"
#include "tMemManager.h"
#include "ePlayer.h"
#include "tron/HudManager.h"
...
static tConfItem< bool > sg_overrideLocalColorConf( "OVERRIDE_LOCAL_COLOR", sg_overrideLocalColor );

REAL sg_localColorR = 1.0f;
static tConfItem< REAL > sg_localColorRConf( "LOCAL_COLOR_R", sg_localColorR );
REAL sg_localColorG = 0.4f;
static tConfItem< REAL > sg_localColorGConf( "LOCAL_COLOR_G", sg_localColorG );
REAL sg_localColorB = 0.1f;
static tConfItem< REAL > sg_localColorBConf( "LOCAL_COLOR_B", sg_localColorB );

bool sg_distributeEnemyColors = false;
static tConfItem< bool > sg_distributeEnemyColorsConf( "DISTRIBUTE_ENEMY_COLORS", sg_distributeEnemyColors );

bool sg_overrideEnemyUnifiedColor = false;
static tConfItem< bool > sg_overrideEnemyUnifiedColorConf( "OVERRIDE_ENEMY_UNIFIED_COLOR", sg_overrideEnemyUnifiedColor );

REAL sg_enemyUnifiedColorR = 0.2f;
static tConfItem< REAL > sg_enemyUnifiedColorRConf( "ENEMY_UNIFIED_COLOR_R", sg_enemyUnifiedColorR );
REAL sg_enemyUnifiedColorG = 0.6f;
static tConfItem< REAL > sg_enemyUnifiedColorGConf( "ENEMY_UNIFIED_COLOR_G", sg_enemyUnifiedColorG );
REAL sg_enemyUnifiedColorB = 1.0f;
static tConfItem< REAL > sg_enemyUnifiedColorBConf( "ENEMY_UNIFIED_COLOR_B", sg_enemyUnifiedColorB );
                              ...
                              nKrawall::nPasswordAnswer & answer )
{
    ...
    */
    ...
    // The client draws this itself when it can. The old menu stays for the
    // cases where there is no window yet, and for the dedicated build.
    bool entered = false;

    // On a build with no window at all there is only the old menu, so the
    // choice below is not one the server has to make.
    bool ourOwn = false;
#ifndef DEDICATED
    ourOwn = sr_glOut;
#endif
    ...
    if ( ourOwn )
    {
        std::string user( (char const *)username ), pass;
        int store = se_PasswordStorageMode;

        entered = ModMenu::AskLogin( (char const *)tOutput( message ), user, pass, store );

        se_PasswordStorageMode = store;
    ...
    if ( !ourOwn )
    {
        ...
        uMenuItemSelection<int> storepw(&login,
                                        ...
                                        "$login_storepw_help",
                                        se_PasswordStorageMode);
        storepw.NewChoice("$login_storepw_dont_text", "$login_storepw_dont_help", -1);
        storepw.NewChoice("$login_storepw_mem_text", "$login_storepw_mem_help", 0);
        storepw.NewChoice("$login_storepw_disk_text", "$login_storepw_disk_help", 1);
        ...
        entered = eMenuItemPassword::entered;
    }

    eMenuItemPassword::entered = entered;
...
// Colour picker chat handler, defined further down.
...
#if !PUBLIC_BUILD
        ChatCommands::checkChat(message);
#endif
#ifndef DEDICATED
        tString cleanSay = tColoredString::RemoveColors((const char*)say);
        ModMenu::HandleIncomingChat(p, (const char*)cleanSay);
#endif
    }
}
...
#endif
#ifndef DEDICATED
        tString cleanMsg = tColoredString::RemoveColors((const char*)message);
        ModMenu::HandleIncomingChat(p, (const char*)cleanMsg);
#endif
    }
}
...
static REAL sg_chatLastReply = -10.0f;
...
static unsigned sg_chatSeed = 0;
static int sg_chatRand(int max) {
    if (max <= 0) return 0;
    if (sg_chatSeed == 0) sg_chatSeed = (unsigned)(tSysTimeFloat() * 1000000.0) ^ 0xDEADBEEFu;
    sg_chatSeed ^= (unsigned)(tSysTimeFloat() * 37.0);
    sg_chatSeed ^= sg_chatSeed << 13;
    sg_chatSeed ^= sg_chatSeed >> 17;
    sg_chatSeed ^= sg_chatSeed << 5;
    return (int)(sg_chatSeed % (unsigned)max);
}
...
static void getCleanName(ePlayerNetID* p, char* out, int maxLen) {
    ...
    ChatText::stripColors(static_cast<const char*>(name), out, maxLen);
}
...
static bool checkLocalCommand(const tString& rawMessage, ePlayerNetID* sender) {
    ...
    char msg[512];
    ChatText::stripColors(static_cast<const char*>(rawMessage), msg, sizeof(msg));
...
void checkChat(const tString& rawMessage) {
    ...
    char clean[512];
    ChatText::stripColors(static_cast<const char*>(rawMessage), clean, sizeof(clean));
    ...
    if (lp && lp->netPlayer) {
        ...
        char myClean[256];
        ChatText::stripColors(static_cast<const char*>(myName), myClean, sizeof(myClean));
...
#if !PUBLIC_BUILD
        ChatCommands::checkLocalCommand(s_orig, this);
#endif
#ifndef DEDICATED
        if (s_orig.StartsWith("!")) {
            std::string cmdStr(static_cast<const char*>(s_orig));
            if (ModMenu::HandleChatCommand(cmdStr)) {
                return;
            }
        }
#endif
...
public:
    ...
    //! Hands the line over to be drawn with the rest of the chat.
    //!
    //! Nothing about the typing changes - this item still holds the text, the
    //! cursor, the history and the completion. Only the drawing moves.
    virtual void Render(REAL x, REAL y, REAL alpha = 1, bool selected = 0)
    {
#ifndef DEDICATED
        if ( rc_ChatInputSkinned() )
        {
            // Team chat is a command typed into the same line, so the line
            // itself is what says which it is.
            bool team = false;
            {
                char const * said = &((*content)[0]);
                team = said && ( strncmp( said, "/team ", 6 ) == 0 ||
                                 strncmp( said, "/t ", 3 ) == 0 );
            }

            rc_QueueChatInput( &((*content)[0]), cursorPos, team );
            return;
        }
#endif
        uMenuItemStringWithHistory::Render( x, y, alpha, selected );
    }
    ...
    virtual bool Event(SDL_Event &e){
        ...
        case SDL_EVENT_KEY_DOWN: {
            ...
            // These four are also the keys the word-wise editing below is built
            // on. Handled here first and returned, that code could never run,
            // so control plus backspace, delete or an arrow did the plain thing
            // instead of the word thing.
#if defined (MACOSX)
            bool wordModifier = ( mod & KMOD_ALT ) != 0 || ( mod & KMOD_META ) != 0;
#else
            bool wordModifier = ( mod & KMOD_CTRL ) != 0;
#endif

            // 3. Backspace
            if (key == SDLK_BACKSPACE && !wordModifier) {
                if (cursorPos > 0) {
            ...
            // 4. Delete
            if (key == SDLK_DELETE && !wordModifier) {
            ...
            if (key == SDLK_LEFT && !wordModifier) {
                if (cursorPos > 0) cursorPos--;
                return true;
            }
            if (key == SDLK_RIGHT && !wordModifier) {
            ...
            // 8. Global breaking binds (e.g. screenshot)
            if (uActionGlobal::IsBreakingGlobalBind(e.key.scancode)) {
                return su_HandleEvent(e, true);
            }

            // 9. Tab key — player name autocomplete
            if (key == SDLK_TAB) {
                // Collect the word being typed before the cursor
                int wordStart = cursorPos;
                const char* buf = content->operator const char*();
                while (wordStart > 0 && buf[wordStart - 1] != ' ') {
                    wordStart--;
                }
                int wordLen = cursorPos - wordStart;
                if (wordLen > 0) {
                    tString prefix(content->SubStr(wordStart, wordLen));
                    std::string prefixStr((const char*)prefix);
                    // Convert prefix to lowercase for case-insensitive matching
                    std::string prefixLower = prefixStr;
                    for (char& c : prefixLower) c = tolower((unsigned char)c);

                    // Find the first player whose plain name starts with the prefix
                    tString bestMatch;
                    for (int pi = 0; pi < se_PlayerNetIDs.Len(); pi++) {
                        ePlayerNetID* pn = se_PlayerNetIDs[pi];
                        if (!pn) continue;
                        tString pname = pn->GetName();
                        std::string pnameStr((const char*)pname);
                        std::string pnameLower = pnameStr;
                        for (char& c : pnameLower) c = tolower((unsigned char)c);
                        if (pnameLower.size() >= prefixLower.size() &&
                            pnameLower.substr(0, prefixLower.size()) == prefixLower) {
                            bestMatch = pname;
                            break;
                        }
                    }
                    ...
                    if (bestMatch.Len() > 1) {
                        // Replace the typed prefix with the full player name
                        tString before = content->SubStr(0, wordStart);
                        tString after  = content->SubStr(cursorPos);
                        tString replacement = before + bestMatch + after;
                        if (replacement.Len() <= maxLength_) {
                            *content  = replacement;
                            cursorPos = wordStart + bestMatch.Len() - 1;
                        }
                    }
                }
                return true;
            ...
            // 10. Exclude modifier keys from game action dispatch
            if (!( (key >= SDLK_CAPSLOCK && key <= SDLK_NUMLOCKCLEAR) ||
                   (key >= SDLK_LCTRL   && key <= SDLK_RGUI) ||
                   key == SDLK_MODE ||
                   key == SDLK_MULTI_KEY_COMPOSE ))
            {
                // On Windows (and all platforms) we must NOT dispatch to
                // su_HandleEvent(e, false) here because that would fire instant
                // chat actions as real sends (instead of insertions) if the
                // se_chatter pointer is not yet set at the time of the call.
                // All legitimate text insertion is handled by SDL_EVENT_TEXT_INPUT
                // above, and Tab completion is handled just above.
                // We simply consume the event.
            }
...
static void do_chat(){
    if (se_chatterPlanned){
        ...
        chat_menu.Enter();
#ifndef DEDICATED
        if (!ModMenu_WantsTextInput()) SDL_StopTextInput(sr_screen);
#endif
...
bool ePlayer::Act(uAction *act,REAL x){
    ...
    // Must NOT fire while the player is actively typing in the chat UI.
    if ( act == &se_resetKD && x > 0 )
    {
        // se_chatter is the ePlayer* that is currently in the chat dialog.
        // If se_chatter is non-null, a chat is open — ignore the keybind.
        if ( se_chatter )
            return false;
        sg_modKDResetFlag = true;
        con << "K/D stats reset!\n";
...
// Whether the client draws its own table instead of the one below.
//
// The old one is a bitmap font on a transparent background with columns lined
// up by counting characters. It works, and it has looked like a different
// program from the rest of this client since the rest of it was rewritten.
bool sg_customScoreboard = true;
static tConfItem< bool > sg_customScoreboardConf( "MOD_CUSTOM_SCOREBOARD", sg_customScoreboard );

bool se_ScoresWanted(){ return show_scores; }
...
// A cycle has to stay readable against the floor, and with the grid switched
// off that floor is pure black. Colours dark enough to disappear are lifted
// while their hue is kept; a colour with no hue left to keep becomes a light
// grey rather than a silhouette.
static void se_LiftColorOffFloor( REAL& r, REAL& g, REAL& b )
{
    const REAL minLum = 0.34f;
    ...
    if ( peak < 0.04f )
    {
        // Nothing to preserve: this is the black entry.
        r = g = b = 0.70f;
        return;
    }

    // Brighten as far as the strongest channel allows before clipping.
    REAL headroom = 1.0f / peak;
    REAL scale = minLum / ( lum > 0.0001f ? lum : 0.0001f );
    if ( scale > headroom ) scale = headroom;
    r *= scale; g *= scale; b *= scale;

    // Deeply saturated hues cannot reach the target on brightness alone, so
    // blend the remainder toward white instead of leaving them murky.
    lum = 0.2126f * r + 0.7152f * g + 0.0722f * b;
    if ( lum < minLum )
    {
        REAL t = ( minLum - lum ) / ( 1.0f - lum );
        r += ( 1.0f - r ) * t;
        g += ( 1.0f - g ) * t;
        b += ( 1.0f - b ) * t;
    }
...
{
    RawColor( a_r, a_g, a_b );
    se_LiftColorOffFloor( a_r, a_g, a_b );
}

void ePlayerNetID::RawColor( REAL&a_r, REAL&a_g, REAL&a_b ) const
{
...
#ifndef DEDICATED
void ModMenu::SendLocalChat(const char* text) {
    tColoredString message;
    message << tColoredString::ColorString(0.4f, 0.8f, 1.0f);
    message << text << "\n";
    con << message;
}
#endif
