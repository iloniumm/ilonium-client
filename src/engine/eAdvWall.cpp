...
{
#ifndef DEDICATED
    // These are recorded once and replayed every frame after, which is what
    // makes them cheap. It is also why changing their colour appeared to do
    // nothing: the recording still had the old one in it. Throw the recording
    // away when the paint changes and it is made again with the new colour.
    {
        static unsigned painted = 0;
        unsigned now = rc_RimPaintGeneration();
        if ( now != painted )
        {
            painted = now;

            // Nought, not the default. Clearing a list normally also stops it
            // being made again, which is exactly right when the screen mode is
            // changing underneath it and exactly wrong here: it left the walls
            // cleared and never redrawn, so they simply vanished.
