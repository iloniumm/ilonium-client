...
// Making the zone look lit rather than merely present.
//
// There is no bloom pass in this renderer and adding one for a single object
// would be absurd, so the glow is done the way it was done before shaders: the
// same ring drawn again a few times, each a little wider and a lot fainter,
// added on top of itself. From a few metres away that is indistinguishable
// from the real thing, and it costs one display list call per layer because
// the geometry is already in one.
bool sg_zoneGlow = true;
static tConfItem< bool > sg_zoneGlowConf( "MOD_ZONE_GLOW", sg_zoneGlow );

REAL sg_zoneGlowStrength = 4.0f;
static tConfItem< REAL > sg_zoneGlowStrengthConf( "MOD_ZONE_GLOW_STRENGTH", sg_zoneGlowStrength );


// How high the light climbs above the top edge of the wall.

// A zone of your own, burning white in the middle. Telling whose a zone is
// should not require reading anything.
bool sg_zoneGlowOwnWhite = true;
static tConfItem< bool > sg_zoneGlowOwnWhiteConf( "MOD_ZONE_GLOW_OWN_WHITE", sg_zoneGlowOwnWhite );

//! Whether a zone in this colour belongs to the team we are on.
static bool rc_ZoneIsOurs( gRealColor const & col )
{
    ePlayer * seat = ePlayer::PlayerConfig( 0 );
    if ( !seat || !seat->netPlayer )
        return false;

    eTeam * mine = seat->netPlayer->CurrentTeam();
    if ( !mine )
        return false;

    // Zones take their team's colour, so matching it is the same question and
    // does not need the zone to say which team it is for - which the base
    // class does not know.
    REAL d = fabsf( col.r - mine->R() ) + fabsf( col.g - mine->G() ) + fabsf( col.b - mine->B() );
    return d < 0.25f;
}
...
bool sg_drawZoneCenter = true;
static tConfItem< bool > sg_drawZoneCenterConf( "ZONE_CENTER_INDICATOR", sg_drawZoneCenter );
...
void gZone::Render( const eCamera * cam )
{
    ...
    // ---- the glow
    //
    // Nothing is drawn for it here, and that is the point. Three attempts at
    // faking light with extra geometry - scaled rings, a fatter pen, then
    // gradient bands - all came out as the same cheap lamp, because a shape
    // that fades out is still a shape. Light is what the frame does to its
    // neighbours, so it belongs in the frame, not in the arena: the bloom pass
    // spreads anything bright across the pixels around it. All the zone owes
    // that pass is enough brightness to be picked up, so it draws itself over
    // again, added on top of what is already there.
    extern bool rc_BloomActive();
    if ( sg_zoneGlow && sg_zoneGlowStrength > 0.001f && rc_BloomActive() )
    {
        REAL cr = color_.r, cg = color_.g, cb = color_.b;

        // A zone that is yours burns towards white, which says whose it is
        // without a word - and white is what bleeds furthest.
        if ( sg_zoneGlowOwnWhite && rc_ZoneIsOurs( color_ ) )
        {
            cr = 0.45f + 0.55f * cr;
            cg = 0.45f + 0.55f * cg;
            cb = 0.45f + 0.55f * cb;
        ...
        REAL power = sg_zoneGlowStrength > 0 ? sg_zoneGlowStrength : 0;
        glColor4f( cr * power, cg * power, cb * power, 1 );
        zoneList.Call();
...
#undef glEnd
    if ( sg_drawZoneCenter )
    {
        ...
        REAL centerAlpha = alpha * 0.6f;
        glColor4f(color_.r, color_.g, color_.b, centerAlpha);
        ...
        REAL crossSize = 0.15f;
        glBegin(GL_LINES);
        glVertex3f(-crossSize, 0.0f, 0.01f);
        glVertex3f(crossSize, 0.0f, 0.01f);
        glVertex3f(0.0f, -crossSize, 0.01f);
        glVertex3f(0.0f, crossSize, 0.01f);
        glEnd();

        int circleSegments = 16;
        glBegin(GL_LINE_LOOP);
        REAL circleRadius = 0.05f;
        for (int i = 0; i < circleSegments; ++i) {
            REAL theta = i * 2.0f * M_PI / circleSegments;
            glVertex3f(cos(theta) * circleRadius, sin(theta) * circleRadius, 0.01f);
        }
        glEnd();
