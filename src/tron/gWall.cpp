    ...
    if (bool(cycle_) && !cycle_->Alive() && totalDelay >= 0.0f ){
        ...
        if (sg_corpseTrailStyle == 1) { // Blinking Warning (Competitive)
            if (timeSinceDeath >= 0.0f && timeSinceDeath <= totalDelay) {
                ...
                if (timeLeft <= 1.5f) {
                    float blink = sinf((float)se_GameTime() * 30.0f);
                    float opacityFactor = 0.65f + 0.35f * blink;
                    a *= opacityFactor;
                    
                    REAL colorFlash = 0.2f * (0.5f + 0.5f * blink);
                    r += colorFlash;
                    g += colorFlash;
                    b += colorFlash;
                }
    ...
    if (hfrac>0){
        ...
        if ( mode & gWallRenderMode_Quads && sg_renderBulkQuads )
        {
            if (cfg_EnableInstancing)
            {
                ...
                REAL a_top = sg_modTrailGradientEnabled ? (a * sg_modTrailAlphaTop) : a;
                REAL a_bottom = sg_modTrailGradientEnabled ? (a * sg_modTrailAlphaBottom) : a;

                TrailSegment seg;
                seg.transform = mat4::transformation(p1.x, p1.y, p2.x, p2.y, h * hfrac, extrarise);
                seg.color = {r, g, b, a_bottom};
                seg.texCoords = {ta, te, hfrac, a_top};
    ...
    if( mode & gWallRenderMode_Quads && sg_renderBeginQuads )
    {
        if (cfg_EnableInstancing)
        {
            ...
            for (int i = 0; i < segs; i++) {
                ...
                seg.color = {r, g, b, avg_a_bottom};
                seg.texCoords = {ta + (te - ta) * frag1, ta + (te - ta) * frag2, hfrac, avg_a_top};
                g_collectedTrails.push_back(seg);
            }
        }
        else
