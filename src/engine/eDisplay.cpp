...
void eGrid::display_simple( int viewer,bool floor,
    ...
    */
    ...
    if (floor){
        ...
        if (!rc_NeonFloor()) {
            // Our own ground, in the look that has ground in it.
            //
            // Two things kept it off the screen. The floor's texture group is
            // switched off in most saved configs, which blanks the floor
            // whatever else is set; and the two-texture setting draws the pair
            // of crossing sheets rather than the single tile, so the shipped
            // texture never came up. Asking for the custom floor answers both.
            if ( rc_FloorTexture() )
            {
                if ( rTextureGroups::TextureMode[ rTextureGroups::TEX_FLOOR ] < 0 )
                    rTextureGroups::TextureMode[ rTextureGroups::TEX_FLOOR ] = GL_LINEAR_MIPMAP_LINEAR;

                if ( floorDetail != rFLOOR_OFF )
                    floorDetail = rFLOOR_TEXTURE;
            }
        ...
        #define INTENSITY(x,xx) (1-(((x)-(xx))*((x)-(xx))/(EXTENSION*SIDELEN*EXTENSION*SIDELEN)))
            ...
            case rFLOOR_TEXTURE:
            {
                REAL gs = se_GridSize() > 0 ? se_GridSize() : 1;
                if ( rc_FloorTexture() )
                    gs *= rc_FloorTextureScale();
                ...
                {
                    REAL tr, tg, tb;
                    rc_FloorTextureColour( tr, tg, tb );
                    glColor4f( tr, tg, tb, flooralpha );
                }
                else
                {
            ...
            // CycleArena style Glossy Dark Mirror Floor + Glowing Neon Grid Lines
            glPushAttrib(GL_ALL_ATTRIB_BITS);
            glDisable(GL_TEXTURE_2D);
            glEnable(GL_BLEND);
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

            // 1. Glossy dark obsidian glass floor base
            {
                rcPaint const & floorPaint = rc_FloorPaint();
                glColor4f(floorPaint.r, floorPaint.g, floorPaint.b, 0.95f);
            }
            infinity_xy_plane( camPos, CameraDir(viewer) );
    ...
    // The pools belong to the floor: laid before anything that stands on it,
    // so a cycle crossing one is lit rather than painted over.
    rc_FloodlightPools();
    ...
    if(eWalls){
        ...
        // The lamps themselves, after the walls, so nearer things hide them.
        rc_FloodlightFixtures();
...
void eGrid::Render( eCamera* cam, int viewer, REAL& zNear ){
    ...
    if (sr_floorMirror){
        // Planar Reflections using Stencil Buffer
        glDepthMask(GL_TRUE);
        glEnable(GL_DEPTH_TEST);
        glClearStencil(0);
        ...
        // Turn off writing to color & depth buffers for mask pass
        glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);
        glDepthMask(GL_FALSE);

        // Render floor geometry to stencil mask
        BeginQuads();
        eRectangle rect = eWallRim::GetBounds();
        glVertex3f(rect.GetLow().x, rect.GetLow().y, 0.0f);
        glVertex3f(rect.GetLow().x, rect.GetHigh().y, 0.0f);
        glVertex3f(rect.GetHigh().x, rect.GetHigh().y, 0.0f);
        glVertex3f(rect.GetHigh().x, rect.GetLow().y, 0.0f);
        RenderEnd();
        ...
        // Only render where stencil mask is 1 (floor region)
