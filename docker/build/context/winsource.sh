...
if test -n "${CODEBLOCKS}" && test -d ${CODEBLOCKS}; then
    ...
    # Update makedist.bat to copy SDL3 and mimalloc DLLs instead of SDL2
    python3 -c '
import os
path = "winsource/makedist.bat"
if os.path.exists(path):
    with open(path, "r") as f:
        content = f.read()
    content = content.replace("SDL2", "SDL3")
    content = content.replace("SDL_image", "SDL3_image")
    content = content.replace("SDL_mixer", "SDL3_mixer")
    content = content.replace(
        "xcopy %LIBS_DIR%\\\\SDL3_mixer\\\\i686-w64-mingw32\\\\bin\\\\*.dll %1 /C /Y /I",
        "xcopy %LIBS_DIR%\\\\SDL3_mixer\\\\i686-w64-mingw32\\\\bin\\\\*.dll %1 /C /Y /I\\n\\t\\txcopy %LIBS_DIR%\\\\mimalloc\\\\i686-w64-mingw32\\\\bin\\\\*.dll %1 /C /Y /I"
    )
