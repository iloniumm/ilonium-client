...
static tString GeneratePrefix()
{
    ...
    // We want to find prefixNow such that bindirNow is prefixNow + suffixCompiled,
    // or if we are relocated to a different binary dir layout, we should determine prefixNow accordingly.
    tString suffixCompiled = bindirCompiled.SubStr( prefixCompiled.Len() );

    // If bindirNow ends with suffixCompiled, then prefixNow is bindirNow with suffixCompiled removed.
    if ( st_StringEndsWith( bindirNow, suffixCompiled ) )
    {
        return bindirNow.SubStr( 0, bindirNow.Len() - suffixCompiled.Len() );
    }

    // Fallback: what if the binary is in /bin but suffixCompiled is /games?
    if ( st_StringEndsWith( bindirNow, "/bin" ) )
    {
        return bindirNow.SubStr( 0, bindirNow.Len() - 4 );
    }
    // If bindirNow ends with "/games", then prefixNow is bindirNow with "/games" removed.
    if ( st_StringEndsWith( bindirNow, "/games" ) )
    {
        return bindirNow.SubStr( 0, bindirNow.Len() - 6 );
    }

    // General fallback check using suffix compiled length
    int bindirSuffixLength = bindirCompiled.Len() - prefixCompiled.Len();
    if ( bindirNow.Len() >= bindirSuffixLength )
    {
        tString suffixNow = bindirNow.SubStr( bindirNow.Len() - bindirSuffixLength );
        if ( suffixNow == suffixCompiled )
        {
            return bindirNow.SubStr( 0, bindirNow.Len() - bindirSuffixLength );
        }
    }
    ...
    // If we couldn't match, let's at least print a warning instead of aborting, and make a best guess
    return GetParent( bindirNow, 1 );
}
...
static tString st_RelocatePath( tString const & original )
{
    ...
    if ( original.StartsWith( prefixCompiled ) )
    {
        ...
        return prefixNow + original.SubStr( prefixCompiled.Len() );
    }
    else
    {
...
//! A file the player has no reason to open, kept where it is not in the way.
/**
 *  The settings folder is the one place somebody looks when they want to change
 *  something by hand, and it was filling up with things that are not settings:
 *  a session token, a private key, a bridge to the media player, a log, the
 *  numbers behind the dashboard. All of it lives in one subfolder now.
 *
 *  The path is worked out here rather than asked for, and that is the whole
 *  point of this function. Both the path checker and the directory maker refuse
 *  any segment beginning with a dot - they are guarding against names arriving
 *  from configuration files and from the network, and a hidden folder is
 *  exactly the sort of thing they exist to stop. The names below are constants
 *  in this file, so there is nothing to guard against.
 *
 *  Worse than the refusal was how it was delivered: through the console. The
 *  console asks this function where its log file is, so the first line the game
 *  ever printed went round that circle and never came back.
 *
 *  A file left over from an older build is moved in on the way past, so nothing
 *  is lost - least of all the key, which is the whole of this installation's
 *  identity to the backend.
 */
tString rc_InternalPath( char const * name )
{
    // Nothing in here may reach the console, but a caller could be the console
    // itself part way through a line. If we are somehow asked again while
    // answering, the plain path is the safe answer.
    static bool answering = false;
    if ( answering )
        return tDirectories::Var().GetWritePath( name );
    ...
    if ( !looked )
    {
        // The first lines the game prints arrive before it knows where its own
        // directory is, and the console asks for its log file on the very
        // first of them. A failure then is not an answer for the whole
        // session, so nothing is remembered until there is something to
        // remember.
        std::string probe( static_cast< char const * >( tDirectories::Var().GetWritePath( "x" ) ) );
        size_t const cut = probe.find_last_of( "/\\" );

        if ( cut != std::string::npos )
        {
            folder = probe.substr( 0, cut ) + "/.ilonium";
    ...
    std::string const inside = folder + "/" + name;

    // Once, and only when there is nothing there already.
    FILE * there = fopen( inside.c_str(), "r" );
    if ( there )
    {
        fclose( there );
        return tString( inside.c_str() );
    }

    tString const old = tDirectories::Var().GetWritePath( name );
    FILE * before = fopen( static_cast< char const * >( old ), "r" );
    if ( before )
    {
        fclose( before );
        rename( static_cast< char const * >( old ), inside.c_str() );
    }
