...
static bool sg_noclipActive = false;
static REAL sg_noclipYaw = 0;
static REAL sg_noclipPitch = 0;
REAL sg_noclipSpeed = 60.0;
REAL sg_noclipSlowFactor = 0.25;

// Held down, everything moves this much faster. Its own factor rather than a
// second speed, so raising the base speed raises the boost with it.
REAL sg_noclipFastFactor = 3.0;

// Level flight. With this on, forward means forward across the ground however
// far the view is tilted - looking down no longer sinks you. It is a toggle
// rather than a hold: keeping a key down to fly straight is the same problem
// by another name.
bool sg_noclipLevel = false;
REAL sg_noclipMouseSens = 0.010f;
static eCoord sg_noclipPos(0, 0);
static REAL sg_noclipZ = 30;
static REAL sg_noclipFOV = 90;
static bool sg_noclipLocked = false;
static bool sg_noclipCtrlLast = false;
static bool sg_noclipFLast = false;
static bool sg_noclipLockToPlayer = false;
static int sg_noclipPlayerIndex = 0;
static bool sg_noclipArrowUpLast = false;
static bool sg_noclipArrowDownLast = false;
int sg_noclipMouseWheel = 0;

#ifndef DEDICATED
int sg_noclipKeyForward  = SDLK_W;
int sg_noclipKeyBack     = SDLK_S;
int sg_noclipKeyLeft     = SDLK_A;
int sg_noclipKeyRight    = SDLK_D;
int sg_noclipKeyUp       = SDLK_SPACE;
int sg_noclipKeyDown     = SDLK_LSHIFT;
int sg_noclipKeyZoomIn   = SDLK_E;
int sg_noclipKeyZoomOut  = SDLK_Q;
int sg_noclipKeyCenter   = SDLK_Z;
int sg_noclipKeyLookAt   = SDLK_C;
int sg_noclipKeyLock     = SDLK_LCTRL;
int sg_noclipKeyFollow   = SDLK_F;
int sg_noclipKeyNextPl   = SDLK_UP;
int sg_noclipKeyPrevPl   = SDLK_DOWN;
int sg_noclipKeySlow     = 0; // unbound by default
int sg_noclipKeyFast     = 0; // unbound by default
int sg_noclipKeyLevel    = 0; // unbound by default
int sg_noclipKeySphere   = 0; // unbound by default
int sg_noclipKeyMark     = 0; // unbound by default
int sg_noclipKeyMarkNext = 0; // unbound by default
int sg_noclipKeyMarkPrev = 0; // unbound by default
int sg_noclipKeyMarkDrop = 0; // unbound by default

static tConfItem<REAL> sg_noclipSpeedConf("NOCLIP_SPEED", sg_noclipSpeed);
static tConfItem<REAL> sg_noclipSlowConf("NOCLIP_SLOW_FACTOR", sg_noclipSlowFactor);
static tConfItem<REAL> sg_noclipFastConf("NOCLIP_FAST_FACTOR", sg_noclipFastFactor);
static tConfItem<bool> sg_noclipLevelConf("NOCLIP_LEVEL", sg_noclipLevel);
static tConfItem<REAL> sg_noclipSensConf("NOCLIP_MOUSE_SENS", sg_noclipMouseSens);

static tConfItem<int> sg_nkForward ("NOCLIP_KEY_FORWARD",  sg_noclipKeyForward);
static tConfItem<int> sg_nkBack    ("NOCLIP_KEY_BACK",     sg_noclipKeyBack);
static tConfItem<int> sg_nkLeft    ("NOCLIP_KEY_LEFT",     sg_noclipKeyLeft);
static tConfItem<int> sg_nkRight   ("NOCLIP_KEY_RIGHT",    sg_noclipKeyRight);
static tConfItem<int> sg_nkUp      ("NOCLIP_KEY_UP",       sg_noclipKeyUp);
static tConfItem<int> sg_nkDown    ("NOCLIP_KEY_DOWN",     sg_noclipKeyDown);
static tConfItem<int> sg_nkZoomIn  ("NOCLIP_KEY_ZOOM_IN",  sg_noclipKeyZoomIn);
static tConfItem<int> sg_nkZoomOut ("NOCLIP_KEY_ZOOM_OUT", sg_noclipKeyZoomOut);
static tConfItem<int> sg_nkCenter  ("NOCLIP_KEY_CENTER",   sg_noclipKeyCenter);
static tConfItem<int> sg_nkLookAt  ("NOCLIP_KEY_LOOKAT",   sg_noclipKeyLookAt);
static tConfItem<int> sg_nkLock    ("NOCLIP_KEY_LOCK",     sg_noclipKeyLock);
static tConfItem<int> sg_nkFollow  ("NOCLIP_KEY_FOLLOW",   sg_noclipKeyFollow);
static tConfItem<int> sg_nkNextPl  ("NOCLIP_KEY_NEXT_PL",  sg_noclipKeyNextPl);
static tConfItem<int> sg_nkPrevPl  ("NOCLIP_KEY_PREV_PL",  sg_noclipKeyPrevPl);
static tConfItem<int> sg_nkSlow    ("NOCLIP_KEY_SLOW",     sg_noclipKeySlow);
static tConfItem<int> sg_nkFast    ("NOCLIP_KEY_FAST",    sg_noclipKeyFast);
void rc_NoclipPose( REAL & x, REAL & y, REAL & z, REAL & yaw, REAL & pitch, REAL & fov )
{
    ...
    z = sg_noclipZ;
    yaw = sg_noclipYaw;
    pitch = sg_noclipPitch;
    fov = sg_noclipFOV;
}

void rc_NoclipSetPose( REAL x, REAL y, REAL z, REAL yaw, REAL pitch, REAL fov )
{
    sg_noclipPos = eCoord( x, y );
    sg_noclipZ = z;
    sg_noclipYaw = yaw;
    sg_noclipPitch = pitch;
    sg_noclipFOV = fov;
}

// Locked onto somebody, the camera stops being a free flying thing and becomes
// a point on a sphere around them. The mouse walks it over that sphere, the
// wheel changes how far out it sits, and it looks at them the whole time - so
// the shot stays framed while the subject moves and turns underneath it.
//
// This is a different job from the orbit below, which drives itself at a fixed
// speed. That one is for leaving running; this one is for aiming.
bool sg_noclipSphere = false;
REAL sg_noclipSphereRadius = 45;
static REAL sg_noclipSphereAround = 0;
static REAL sg_noclipSphereAbove = 0.5;

static tConfItem<int> sg_nkSphere  ("NOCLIP_KEY_SPHERE",     sg_noclipKeySphere);
static tConfItem<REAL> sg_nSphereR ("NOCLIP_SPHERE_RADIUS",  sg_noclipSphereRadius);
static tConfItem<bool> sg_nSphere  ("NOCLIP_SPHERE",         sg_noclipSphere);
static tConfItem<int> sg_nkMark    ("NOCLIP_KEY_MARK",       sg_noclipKeyMark);
static tConfItem<int> sg_nkMarkNext("NOCLIP_KEY_MARK_NEXT",  sg_noclipKeyMarkNext);
static tConfItem<int> sg_nkMarkPrev("NOCLIP_KEY_MARK_PREV",  sg_noclipKeyMarkPrev);
static tConfItem<int> sg_nkMarkDrop("NOCLIP_KEY_MARK_DELETE",sg_noclipKeyMarkDrop);
static tConfItem<int> sg_nkLevel   ("NOCLIP_KEY_LEVEL",   sg_noclipKeyLevel);

static bool sg_noclipSavedMouseGrab = false;

extern bool su_mouseGrab;
#else
// Server stubs — values never used but needed for link
int sg_noclipKeyForward = 0, sg_noclipKeyBack = 0, sg_noclipKeyLeft = 0, sg_noclipKeyRight = 0;
int sg_noclipKeyUp = 0, sg_noclipKeyDown = 0, sg_noclipKeyZoomIn = 0, sg_noclipKeyZoomOut = 0;
int sg_noclipKeyCenter = 0, sg_noclipKeyLookAt = 0, sg_noclipKeyLock = 0, sg_noclipKeyFollow = 0;
int sg_noclipKeyNextPl = 0, sg_noclipKeyPrevPl = 0, sg_noclipKeySlow = 0;
static bool sg_noclipSavedMouseGrab = false;
extern bool su_mouseGrab;
#endif

void sg_ToggleNoclip(eCamera* cam) {
#ifndef DEDICATED
    sg_noclipActive = !sg_noclipActive;
    extern SDL_Window* sr_screen;
    if (sg_noclipActive) {
        if (cam) {
            sg_noclipPos = cam->CameraPos();
            sg_noclipZ = cam->CameraZ();
            eCoord d = cam->CameraDir();
            sg_noclipYaw = atan2(d.y, d.x);
            sg_noclipPitch = 0;
            sg_noclipLocked = false;
        }
        sg_noclipSavedMouseGrab = su_mouseGrab;
        ...
        tColoredString m;
        m << tColoredString::ColorString(0.3, 1.0, 0.8)
          << "[NOCLIP] " << tColoredString::ColorString(1,1,1)
          << "ON  WASD/Arrows:move  Space/Shift:up/down  Q/E/Wheel:zoom  Ctrl:lock\n";
        con << m;
    } else {
        if (sr_screen) {
        ...
        tColoredString m;
        m << tColoredString::ColorString(0.3, 1.0, 0.8)
          << "[NOCLIP] " << tColoredString::ColorString(1,1,1) << "OFF\n";
        con << m;
    }
#endif
}

bool sg_IsNoclipActive() { return sg_noclipActive; }

//! Puts the free camera away.
//!
//! Leaving to the main menu with it still on meant coming back to a game you
//! could not steer, with no obvious sign why. Nothing here is a toggle - it
//! only ever switches off - so it is safe to call from anywhere that is
//! tearing a game down.
void rc_NoclipStop()
{
#ifndef DEDICATED
...
// Whether the markers are on screen. They are a working aid, and when you are
// lining a shot up they are in the way.
bool sg_waypointsVisible = true;
static tConfItem<bool> sg_wpVis( "NOCLIP_WAYPOINTS_VISIBLE", sg_waypointsVisible );

int sg_noclipKeyMarkHide = 0; // unbound by default
static tConfItem<int> sg_nkMarkHide( "NOCLIP_KEY_MARK_HIDE", sg_noclipKeyMarkHide );

bool sg_noclipCinematic = false;
static tConfItem<bool> sg_noclipCinematicConf("NOCLIP_CINEMATIC", sg_noclipCinematic);

static bool sg_noclipOrbit = false;
REAL sg_noclipOrbitSpeed = 0.3;
REAL sg_noclipOrbitRadius = 80;
REAL sg_noclipOrbitHeight = 40;
static REAL sg_noclipOrbitAngle = 0;
static tConfItem<REAL> sg_orbitSpeedConf("NOCLIP_ORBIT_SPEED", sg_noclipOrbitSpeed);
static tConfItem<REAL> sg_orbitRadiusConf("NOCLIP_ORBIT_RADIUS", sg_noclipOrbitRadius);
static tConfItem<REAL> sg_orbitHeightConf("NOCLIP_ORBIT_HEIGHT", sg_noclipOrbitHeight);

REAL sg_noclipSmoothFactor = 5.0;
REAL sg_noclipFollowDist = 35;
REAL sg_noclipFollowHeight = 20;
static tConfItem<REAL> sg_smoothFactConf("NOCLIP_SMOOTH_FACTOR", sg_noclipSmoothFactor);
static tConfItem<REAL> sg_followDistConf("NOCLIP_FOLLOW_DIST", sg_noclipFollowDist);
static tConfItem<REAL> sg_followHeightConf("NOCLIP_FOLLOW_HEIGHT", sg_noclipFollowHeight);

static bool sg_noclipZoneFocus = false;
static bool sg_noclipMapCenterFocus = false;

#ifndef DEDICATED
int sg_noclipKeyToggle    = SDLK_N;    // N to toggle noclip
int sg_noclipKeyCinematic = SDLK_H;    // H for HUD toggle
bool sg_noclipHideConsole = true;      // Hide console with HUD
bool sg_noclipHideNames   = true;      // Hide player names with HUD
int sg_noclipKeyOrbit     = SDLK_O;    // O for orbit
int sg_noclipKeySmooth    = SDLK_P;    // P for smooth follow
int sg_noclipKeyZoneFocus = SDLK_G;    // G for zone focus
int sg_noclipKeyMapCenter = SDLK_J;    // J for map center focus

static tConfItem<int> sg_nkToggle   ("NOCLIP_KEY_TOGGLE",     sg_noclipKeyToggle);
static tConfItem<int> sg_nkCinematic("NOCLIP_KEY_CINEMATIC",  sg_noclipKeyCinematic);
static tConfItem<bool> sg_ncHideCon ("NOCLIP_HIDE_CONSOLE",   sg_noclipHideConsole);
static tConfItem<bool> sg_ncHideNam ("NOCLIP_HIDE_NAMES",     sg_noclipHideNames);
static tConfItem<int> sg_nkOrbit    ("NOCLIP_KEY_ORBIT",      sg_noclipKeyOrbit);
static tConfItem<int> sg_nkSmooth   ("NOCLIP_KEY_SMOOTH",     sg_noclipKeySmooth);
static tConfItem<int> sg_nkZoneFocus("NOCLIP_KEY_ZONE_FOCUS", sg_noclipKeyZoneFocus);
static tConfItem<int> sg_nkMapCenter("NOCLIP_KEY_MAP_CENTER", sg_noclipKeyMapCenter);
#else
int sg_noclipKeyToggle = 0, sg_noclipKeyCinematic = 0;
bool sg_noclipHideConsole = false, sg_noclipHideNames = false;
int sg_noclipKeyOrbit = 0, sg_noclipKeySmooth = 0, sg_noclipKeyZoneFocus = 0, sg_noclipKeyMapCenter = 0;
#endif

uActionCamera sg_noclipToggleAct    ("NOCLIP_TOGGLE",      tOutput("Noclip: Toggle"),                    tOutput("Toggles the free-look noclip camera mode."),          -200);
uActionCamera sg_noclipCinematicAct ("NOCLIP_CINEMATIC",   tOutput("Noclip: Cinematic HUD Toggle"),      tOutput("Toggles HUD visibility in noclip mode."),             -210);
uActionCamera sg_noclipOrbitAct     ("NOCLIP_ORBIT",       tOutput("Noclip: Orbit Camera Toggle"),       tOutput("Toggles auto-orbit around target."),                  -220);
uActionCamera sg_noclipSmoothAct    ("NOCLIP_SMOOTH",      tOutput("Noclip: Smooth Follow Toggle"),      tOutput("Toggles smooth tracking follow."),                    -230);
uActionCamera sg_noclipZoneFocusAct ("NOCLIP_ZONE_FOCUS",  tOutput("Noclip: Zone Focus Toggle"),         tOutput("Cycles camera focus target to active zones."),        -240);
uActionCamera sg_noclipMapCenterAct ("NOCLIP_MAP_CENTER",  tOutput("Noclip: Map Center Focus Toggle"),  tOutput("Locks camera focus to the map center."),              -250);
uActionCamera sg_noclipLockAct      ("NOCLIP_LOCK",        tOutput("Noclip: Look/Rotation Lock"),        tOutput("Locks look/rotation changes in noclip mode."),        -260);
uActionCamera sg_noclipCenterAct    ("NOCLIP_CENTER",      tOutput("Noclip: Center Camera"),             tOutput("Centers noclip camera at (0, 0) and high Z."),        -270);
uActionCamera sg_noclipLookAtAct    ("NOCLIP_LOOK_AT",     tOutput("Noclip: Look At Center"),            tOutput("Orient camera to look towards center."),              -280);
uActionCamera sg_noclipFollowAct    ("NOCLIP_FOLLOW",      tOutput("Noclip: Follow Target Toggle"),      tOutput("Toggles noclip following the targeted player."),      -290);
uActionCamera sg_noclipNextPlAct    ("NOCLIP_NEXT_PLAYER",  tOutput("Noclip: Next Player Target"),        tOutput("Selects the next player target for following."),      -300);
uActionCamera sg_noclipPrevPlAct    ("NOCLIP_PREV_PLAYER",  tOutput("Noclip: Previous Player Target"),    tOutput("Selects the previous player target for following."),  -310);
uActionCamera sg_noclipSlowAct      ("NOCLIP_SLOW",        tOutput("Noclip: Slow Speed"),                tOutput("Hold to temporarily slow down movement speed."),      -320);

void sg_SyncNoclipBinds() {
    // Deprecated to prevent gameplay keybind override.
}

static bool sg_noclipToggleLast = false;
static bool sg_noclipCinematicLast = false;
static bool sg_noclipOrbitLast = false;
static bool sg_noclipSmoothLast = false;
static bool sg_noclipZoneFocusLast = false;

//! Asked of the client, which owns its own menu; the engine only needs the yes
//! or no and gets it without including anything from the layer above it.
bool rc_ModMenuOpen();

//! A short line in the middle of the screen, drawn by the client's HUD.
void rc_HudFlash( char const * text, double seconds );

static bool sg_NoclipTimestep(eCamera* cam, REAL ts) {
#ifndef DEDICATED
    if (!sg_noclipActive) return false;

    const ePlayer* lp = cam ? cam->LocalPlayer() : NULL;

    // The camera holds its position but takes no input while somebody is typing
    // or has a menu open. The mod menu counts: it has a cursor of its own, and
    // a mouse that turns the world underneath the panel you are trying to click
    // is the sort of thing that makes a client feel broken.
    if ( ( lp && lp->netPlayer && lp->netPlayer->IsChatting() ) ||
         uMenu::MenuActive() || rc_ModMenuOpen() ) {
        return true;
    }
    ...
    auto isKeyPressed = [&](int key) -> bool {
        if (key <= 0 || !keystate) return false;
        SDL_Scancode sc = SDL_GetScancodeFromKey((SDL_Keycode)key);
        if (sc > 0 && sc < numkeys && keystate[sc]) return true;
        if (key >= 'A' && key <= 'Z') {
            sc = SDL_GetScancodeFromKey((SDL_Keycode)(key + 32));
            if (sc > 0 && sc < numkeys && keystate[sc]) return true;
        }
        return false;
    };
    ...
    if (!sg_noclipLocked) {
        if (sg_noclipSphere) {
            // The same movement, spent on where the camera stands rather than
            // on where it points. Pushing the mouse right walks it round to
            // the right, which is what the hand expects when it can see the
            // thing it is circling.
            sg_noclipSphereAround -= fmx * sg_noclipMouseSens;
            sg_noclipSphereAbove  += fmy * sg_noclipMouseSens;

            // Stopped short of directly overhead and directly underneath: at
            // either pole the view spins about itself and there is no way to
            // tell which way is forward.
            if (sg_noclipSphereAbove >  1.45f) sg_noclipSphereAbove =  1.45f;
            if (sg_noclipSphereAbove < -1.45f) sg_noclipSphereAbove = -1.45f;
        } else {
            sg_noclipYaw   -= fmx * sg_noclipMouseSens;
            sg_noclipPitch -= fmy * sg_noclipMouseSens;
            if (sg_noclipPitch >  1.57f) sg_noclipPitch =  1.57f;
            if (sg_noclipPitch < -1.57f) sg_noclipPitch = -1.57f;
        }
    }

    bool lockKey = isKeyPressed(sg_noclipKeyLock) || (keystate && keystate[SDL_SCANCODE_LCTRL]);
    if (lockKey && !sg_noclipCtrlLast) {
        sg_noclipLocked = !sg_noclipLocked;
    }
    sg_noclipCtrlLast = lockKey;

    REAL effectiveTs = ts;
    if (effectiveTs <= 0.0001f) effectiveTs = 0.016f; // prevent stuck camera on pause
    REAL speed = sg_noclipSpeed * effectiveTs;

    if ((sg_noclipKeySlow > 0 && isKeyPressed(sg_noclipKeySlow)) || (keystate && keystate[SDL_SCANCODE_LALT])) {
        speed *= sg_noclipSlowFactor;
    }

    // Held, not toggled: this is for crossing the arena and then stopping, and
    // a speed somebody has to remember to switch off is a speed they will fly
    // into a wall with.
    if (sg_noclipKeyFast > 0 && isKeyPressed(sg_noclipKeyFast)) {
        speed *= sg_noclipFastFactor;
    }

    // Toggled the moment the key goes down rather than every frame it is held.
    {
        static bool wasLevel = false;
        bool nowLevel = ( sg_noclipKeyLevel > 0 && isKeyPressed(sg_noclipKeyLevel) );

        if (nowLevel && !wasLevel) {
            sg_noclipLevel = !sg_noclipLevel;

            tString said;
            said << "Level flight: " << (sg_noclipLevel ? "on" : "off");
            sr_con.DoCenterDisplay(said, 2.0, 0.3, 1.0, 0.8);
        }
        wasLevel = nowLevel;
    }

    // Locking on, the moment the key goes down.
    {
        static bool wasSphere = false;
        bool nowSphere = ( sg_noclipKeySphere > 0 && isKeyPressed( sg_noclipKeySphere ) );

        if (nowSphere && !wasSphere) {
            sg_noclipSphere = !sg_noclipSphere;

            tString said;
            said << "Lock on: " << (sg_noclipSphere ? "on" : "off");
            sr_con.DoCenterDisplay(said, 2.0, 0.3, 1.0, 0.8);
        }
        wasSphere = nowSphere;
    }

    // Waypoints, all four acting the moment their key goes down rather than
    // while it is held: each of these does something once, and repeating them
    // forty times a second would be four different kinds of mess.
    {
        static bool wasMark = false, wasNext = false, wasPrev = false, wasDrop = false;

        bool mark = ( sg_noclipKeyMark > 0 && isKeyPressed( sg_noclipKeyMark ) );
        bool next = ( sg_noclipKeyMarkNext > 0 && isKeyPressed( sg_noclipKeyMarkNext ) );
        bool prev = ( sg_noclipKeyMarkPrev > 0 && isKeyPressed( sg_noclipKeyMarkPrev ) );
        bool drop = ( sg_noclipKeyMarkDrop > 0 && isKeyPressed( sg_noclipKeyMarkDrop ) );

        if ( mark && !wasMark ) rc_WaypointAdd();
        if ( next && !wasNext ) rc_WaypointStep( 1 );
        if ( prev && !wasPrev ) rc_WaypointStep( -1 );
        if ( drop && !wasDrop ) rc_WaypointDelete( rc_WaypointCurrent() );

        static bool wasHide = false;
        bool hide = ( sg_noclipKeyMarkHide > 0 && isKeyPressed( sg_noclipKeyMarkHide ) );
        if ( hide && !wasHide )
        {
            extern bool sg_waypointsVisible;
            sg_waypointsVisible = !sg_waypointsVisible;

            tString said;
            said << "Waypoints: " << ( sg_waypointsVisible ? "shown" : "hidden" );
            sr_con.DoCenterDisplay( said, 2.0, 0.3, 1.0, 0.8 );
        }
        wasHide = hide;

        wasMark = mark; wasNext = next; wasPrev = prev; wasDrop = drop;
    }

    // How much of the view's tilt carries into forward movement. Level flight
    // simply takes it out, which leaves the mouse free to look anywhere while
    // the height stays where it was put.
    REAL climb = sg_noclipLevel ? 0.0f : sin(sg_noclipPitch);

    REAL cy = cos(sg_noclipYaw), sy = sin(sg_noclipYaw);
    eCoord forward(cy, sy);
    eCoord right(sy, -cy);

    if (!sg_noclipLocked) {
        if (isKeyPressed(sg_noclipKeyForward) || keystate[SDL_SCANCODE_W] || keystate[SDL_SCANCODE_UP]) {
            sg_noclipPos = sg_noclipPos + forward * speed;
            sg_noclipZ  += climb * speed;
        }
        if (isKeyPressed(sg_noclipKeyBack) || keystate[SDL_SCANCODE_S] || keystate[SDL_SCANCODE_DOWN]) {
            sg_noclipPos = sg_noclipPos - forward * speed;
            sg_noclipZ  -= climb * speed;
        }
        if (isKeyPressed(sg_noclipKeyLeft) || keystate[SDL_SCANCODE_A] || keystate[SDL_SCANCODE_LEFT]) {
            sg_noclipPos = sg_noclipPos - right * speed;
        }
        if (isKeyPressed(sg_noclipKeyRight) || keystate[SDL_SCANCODE_D] || keystate[SDL_SCANCODE_RIGHT]) {
            sg_noclipPos = sg_noclipPos + right * speed;
        }
        if (isKeyPressed(sg_noclipKeyUp) || keystate[SDL_SCANCODE_SPACE] || keystate[SDL_SCANCODE_E]) {
            sg_noclipZ += speed;
        }
        if (isKeyPressed(sg_noclipKeyDown) || keystate[SDL_SCANCODE_LSHIFT] || keystate[SDL_SCANCODE_RSHIFT] || keystate[SDL_SCANCODE_Q]) {
            sg_noclipZ -= speed;
        }

        bool fovChanged = false;

        if (isKeyPressed(sg_noclipKeyZoomOut)) { sg_noclipFOV += speed * 0.5; fovChanged = true; }
        if (isKeyPressed(sg_noclipKeyZoomIn))  { sg_noclipFOV -= speed * 0.5; fovChanged = true; }

        if (sg_noclipSphere) {
            // On the sphere the wheel is how close you are, which is the thing
            // being adjusted constantly; the field of view is not.
            if (sg_noclipMouseWheel > 0) sg_noclipSphereRadius *= 0.85f;
            else if (sg_noclipMouseWheel < 0) sg_noclipSphereRadius /= 0.85f;

            if (sg_noclipSphereRadius < 4.0f)   sg_noclipSphereRadius = 4.0f;
            if (sg_noclipSphereRadius > 400.0f) sg_noclipSphereRadius = 400.0f;
        } else {
            if (sg_noclipMouseWheel > 0) { sg_noclipFOV -= 10; fovChanged = true; }
            else if (sg_noclipMouseWheel < 0) { sg_noclipFOV += 10; fovChanged = true; }
        }
        sg_noclipMouseWheel = 0;

        if (sg_noclipFOV > 160) sg_noclipFOV = 160;
        if (sg_noclipFOV < 10) sg_noclipFOV = 10;

        if (fovChanged) {
            char zoom[ 32 ];
            snprintf( zoom, sizeof( zoom ), "Zoom  %d", (int)sg_noclipFOV );
            rc_HudFlash( zoom, 1.4 );
        }

        if (isKeyPressed(sg_noclipKeyCenter)) {
            sg_noclipPos = eCoord(0, 0);
            sg_noclipZ = 350;
            sg_noclipPitch = -1.57;
        }
        if (isKeyPressed(sg_noclipKeyLookAt)) {
            eCoord diff = eCoord(0,0) - sg_noclipPos;
            sg_noclipYaw = atan2(diff.y, diff.x);
            REAL dist = sqrt(diff.x*diff.x + diff.y*diff.y);
            if (dist > 0.001) {
                sg_noclipPitch = atan2(-sg_noclipZ, dist);
            }
        }
    }

    if (sg_noclipZ < 0.5) sg_noclipZ = 0.5;

    bool arrowUp   = isKeyPressed(sg_noclipKeyNextPl);
    bool arrowDown = isKeyPressed(sg_noclipKeyPrevPl);

    int numPlayers = se_PlayerNetIDs.Len();

    if (arrowUp && !sg_noclipArrowUpLast && numPlayers > 0) {
        for (int tries = 0; tries < numPlayers; tries++) {
            sg_noclipPlayerIndex = (sg_noclipPlayerIndex + 1) % numPlayers;
            ePlayerNetID* p = se_PlayerNetIDs(sg_noclipPlayerIndex);
            eGameObject* obj = p ? p->Object() : NULL;
            if (obj) {
                sg_noclipPos = obj->Position();
                sg_noclipZ   = 20;
                eCoord d = obj->Direction();
                sg_noclipYaw = atan2(d.y, d.x);
                REAL fcy = cos(sg_noclipYaw), fsy = sin(sg_noclipYaw);
                sg_noclipPos = sg_noclipPos - eCoord(fcy, fsy) * 25;
    ...
    if (arrowDown && !sg_noclipArrowDownLast && numPlayers > 0) {
        for (int tries = 0; tries < numPlayers; tries++) {
            sg_noclipPlayerIndex = (sg_noclipPlayerIndex - 1 + numPlayers) % numPlayers;
            ePlayerNetID* p = se_PlayerNetIDs(sg_noclipPlayerIndex);
            eGameObject* obj = p ? p->Object() : NULL;
            if (obj) {
                sg_noclipPos = obj->Position();
    ...
    sg_noclipArrowUpLast   = arrowUp;
    sg_noclipArrowDownLast = arrowDown;

    bool fKey = isKeyPressed(sg_noclipKeyFollow);
    if (fKey && !sg_noclipFLast) {
        sg_noclipLockToPlayer = !sg_noclipLockToPlayer;
        if (!sg_noclipLockToPlayer) sg_noclipOrbit = false; // disable orbit when unfollowing
        tColoredString m; m << tColoredString::ColorString(0.3, 1.0, 0.8) << "[NOCLIP] " << tColoredString::ColorString(1,1,1) << "Player Follow: " << (sg_noclipLockToPlayer ? "ON" : "OFF") << "\n"; con << m;
    }
    sg_noclipFLast = fKey;

    eCoord playerTarget(0, 0);
    eCoord playerDir(1, 0);
    REAL playerZ = 0;
    bool hasPlayerTarget = false;

    // Locking on without anybody chosen did nothing at all, which from the
    // outside is a switch wired to nothing. Turning it on takes whoever the
    // follow list is pointing at.
    if (sg_noclipSphere && !sg_noclipLockToPlayer && numPlayers > 0)
        sg_noclipLockToPlayer = true;

    if (sg_noclipLockToPlayer && numPlayers > 0) {
        ePlayerNetID* p = se_PlayerNetIDs(sg_noclipPlayerIndex);
        eGameObject* obj = p ? p->Object() : NULL;
        if (obj) {
            playerTarget = obj->Position();
            playerDir = obj->Direction();
            hasPlayerTarget = true;
        }
    }

    eCoord zoneTarget(0, 0);
    bool hasZoneTarget = false;
    eGrid* grid = eGrid::CurrentGrid();
    static int sg_zoneCycleIndex = 0;
    
    std::vector<eCoord> activeZones;
    if (grid) {
        const tList<eGameObject>& gameObjects = grid->GameObjects();
        for (int j = gameObjects.Len() - 1; j >= 0; j--) {
            eGameObject* go = gameObjects(j);
            if (go) {
                const char* typeName = typeid(*go).name();
                if (strstr(typeName, "Zone") != nullptr && strstr(typeName, "Death") == nullptr) {
                    activeZones.push_back(go->Position());
                }
            }
        }
    }

    if (activeZones.size() > 0 && sg_noclipZoneFocus) {
        if (sg_zoneCycleIndex >= activeZones.size() || sg_zoneCycleIndex < 0) {
            sg_zoneCycleIndex = 0;
        }
        zoneTarget = activeZones[sg_zoneCycleIndex];
        hasZoneTarget = true;
    }

    static eCoord lastPlayerTarget(0,0);
    static bool wasFollowing = false;
    static bool sg_smoothFollow = false; // "P" key toggle
    ...
    if (hasPlayerTarget) {
        ...
        if (!sg_smoothFollow && !sg_noclipOrbit) {
            eCoord deltaPos = playerTarget - lastPlayerTarget;
            sg_noclipPos = sg_noclipPos + deltaPos;
        }
        lastPlayerTarget = playerTarget;
    } else {
        wasFollowing = false;
    ...
    bool zfKey = isKeyPressed(sg_noclipKeyZoneFocus);
    if (zfKey && !sg_noclipZoneFocusLast) {
        if (activeZones.size() > 0) {
            if (!sg_noclipZoneFocus) {
                sg_noclipZoneFocus = true;
                sg_zoneCycleIndex = 0;
            } else {
                sg_zoneCycleIndex++;
                if (sg_zoneCycleIndex >= activeZones.size()) {
                    sg_noclipZoneFocus = false;
                    sg_zoneCycleIndex = 0;
                }
            }
            
            sg_noclipMapCenterFocus = false;

            if (sg_noclipZoneFocus) {
                eCoord zPos = activeZones[sg_zoneCycleIndex];
                eCoord dirToCenter = eCoord(0,0) - zPos;
                REAL dist = sqrt(dirToCenter.x*dirToCenter.x + dirToCenter.y*dirToCenter.y);
                if (dist > 0.001f) {
                    dirToCenter.x /= dist;
                    dirToCenter.y /= dist;
                } else {
                    dirToCenter = eCoord(1, 0);
                }
                
                sg_noclipPos = zPos - dirToCenter * 150.0f;
                sg_noclipZ = 40.0f;
                
                tString s; s << "Zone Focus: ZONE " << (sg_zoneCycleIndex + 1);
                sr_con.DoCenterDisplay(s, 2.0, 0.3, 1.0, 0.5);
            } else {
                tString s; s << "Zone Focus: OFF";
                sr_con.DoCenterDisplay(s, 2.0, 0.3, 1.0, 0.5);
            }
        }
    }
    sg_noclipZoneFocusLast = zfKey;

    static bool sg_noclipMapCenterLast = false;
    bool mcKey = isKeyPressed(sg_noclipKeyMapCenter);
    if (mcKey && !sg_noclipMapCenterLast) {
        sg_noclipMapCenterFocus = !sg_noclipMapCenterFocus;
        if (sg_noclipMapCenterFocus) {
            sg_noclipZoneFocus = false;
        }
        tString s; s << "Map Center Focus: " << (sg_noclipMapCenterFocus ? "ON" : "OFF");
        sr_con.DoCenterDisplay(s, 2.0, 1.0, 0.5, 0.0);
    }
    sg_noclipMapCenterLast = mcKey;

    bool orbitKey = isKeyPressed(sg_noclipKeyOrbit);
    if (orbitKey && !sg_noclipOrbitLast) {
        sg_noclipOrbit = !sg_noclipOrbit;
        if (sg_noclipOrbit) {
            eCoord tgt = hasPlayerTarget ? playerTarget : (hasZoneTarget ? zoneTarget : eCoord(0,0));
            eCoord diff = sg_noclipPos - tgt;
            sg_noclipOrbitAngle = atan2(diff.y, diff.x);
            sg_noclipOrbitRadius = sqrt(diff.x*diff.x + diff.y*diff.y);
            if (sg_noclipOrbitRadius < 1.0) sg_noclipOrbitRadius = sg_noclipFollowDist;
            sg_noclipOrbitHeight = sg_noclipZ;
        }
        tString s; s << "Orbit Cam: " << (sg_noclipOrbit ? "ON" : "OFF");
        sr_con.DoCenterDisplay(s, 2.0, 0.3, 0.8, 1.0);
    }
    sg_noclipOrbitLast = orbitKey;

    bool smoothKey = isKeyPressed(sg_noclipKeySmooth);
    if (smoothKey && !sg_noclipSmoothLast) {
        if (!sg_noclipLockToPlayer) {
            sg_noclipLockToPlayer = true;
        }
        sg_smoothFollow = !sg_smoothFollow;
        tString s; s << "Smooth Follow: " << (sg_smoothFollow ? "ON" : "OFF");
        sr_con.DoCenterDisplay(s, 2.0, 1.0, 0.65, 0.3);
    }
    sg_noclipSmoothLast = smoothKey;

    eCoord orbitTarget(0, 0);
    bool doOrbit = false;

    if (sg_noclipOrbit) {
        if (hasPlayerTarget) {
            orbitTarget = playerTarget;
            doOrbit = true;
        } else if (sg_noclipZoneFocus && hasZoneTarget) {
            orbitTarget = zoneTarget;
            doOrbit = true;
        } else if (sg_noclipMapCenterFocus) {
            orbitTarget = eCoord(0,0);
            doOrbit = true;
        } else {
            orbitTarget = eCoord(0,0);
            doOrbit = true;
    ...
    // Locked to a player, everything below about free movement stops applying:
    // the camera has one degree of freedom in each direction and they are all
    // measured from the subject.
    if (sg_noclipSphere && hasPlayerTarget) {
        REAL flat = cos(sg_noclipSphereAbove) * sg_noclipSphereRadius;

        sg_noclipPos.x = playerTarget.x + cos(sg_noclipSphereAround) * flat;
        sg_noclipPos.y = playerTarget.y + sin(sg_noclipSphereAround) * flat;
        sg_noclipZ     = playerZ + sin(sg_noclipSphereAbove) * sg_noclipSphereRadius;

        eCoord toward = playerTarget - sg_noclipPos;
        REAL across = sqrt(toward.x*toward.x + toward.y*toward.y);

        sg_noclipYaw = atan2(toward.y, toward.x);
        if (across > 0.001f)
            sg_noclipPitch = atan2(playerZ - sg_noclipZ, across);

        doOrbit = false;
    }

    if (doOrbit) {
        sg_noclipOrbitAngle += sg_noclipOrbitSpeed * ts;
        sg_noclipPos.x = orbitTarget.x + cos(sg_noclipOrbitAngle) * sg_noclipOrbitRadius;
        sg_noclipPos.y = orbitTarget.y + sin(sg_noclipOrbitAngle) * sg_noclipOrbitRadius;
        sg_noclipZ = sg_noclipOrbitHeight;
        
        eCoord diff = orbitTarget - sg_noclipPos;
        sg_noclipYaw = atan2(diff.y, diff.x);
        REAL dist = sqrt(diff.x*diff.x + diff.y*diff.y);
        if (dist > 0.001) {
            sg_noclipPitch = atan2(playerZ - sg_noclipZ, dist);
        }
    } else if (hasPlayerTarget) {
        if (sg_smoothFollow) {
            REAL pcy = playerDir.x, psy = playerDir.y;
            eCoord idealPos = playerTarget - eCoord(pcy, psy) * sg_noclipFollowDist;
            REAL idealZ = sg_noclipFollowHeight;

            REAL tParam = 1.0 - exp(-sg_noclipSmoothFactor * ts);
            sg_noclipPos = sg_noclipPos + (idealPos - sg_noclipPos) * tParam;
            sg_noclipZ = sg_noclipZ + (idealZ - sg_noclipZ) * tParam;

            eCoord diff = playerTarget - sg_noclipPos;
            sg_noclipYaw = atan2(diff.y, diff.x);
            REAL dist = sqrt(diff.x*diff.x + diff.y*diff.y);
            if (dist > 0.001) {
                sg_noclipPitch = atan2(playerZ - sg_noclipZ, dist);
            ...
            if (!sg_noclipZoneFocus && !sg_noclipMapCenterFocus) {
                eCoord diff = playerTarget - sg_noclipPos;
                sg_noclipYaw = atan2(diff.y, diff.x);
                REAL dist = sqrt(diff.x*diff.x + diff.y*diff.y);
                if (dist > 0.001) {
    ...
    if (!doOrbit && (sg_noclipZoneFocus || sg_noclipMapCenterFocus)) {
        eCoord tgt = (sg_noclipZoneFocus && hasZoneTarget) ? zoneTarget : eCoord(0,0);
        eCoord diff = tgt - sg_noclipPos;
        sg_noclipYaw = atan2(diff.y, diff.x);
        REAL dist = sqrt(diff.x*diff.x + diff.y*diff.y);
        if (dist > 0.001) {
            sg_noclipPitch = atan2(playerZ - sg_noclipZ + 5, dist);
        }
    }
...
void rc_CustomCameraGet( rcCameraSet & out )
{
    out.back = s_customBack;
    out.rise = s_customRise;
    out.pitch = s_customPitch;
    out.zoom = s_customZoom;
    out.backFromSpeed = s_customBackSpeed;
    out.riseFromSpeed = s_customRiseSpeed;
    out.turnSpeed = s_customTurnSpeed;
    out.turnSpeed180 = s_customTurnSpeed180;
}

void rc_CustomCameraSet( rcCameraSet const & in )
{
    s_customBack = in.back;
    s_customRise = in.rise;
    s_customPitch = in.pitch;
    s_customZoom = in.zoom;
    s_customBackSpeed = in.backFromSpeed;
    s_customRiseSpeed = in.riseFromSpeed;
    s_customTurnSpeed = in.turnSpeed;
    s_customTurnSpeed180 = in.turnSpeed180;
}
...
bool eCamera::Act(uActionCamera *Act,REAL x){
    ...
    if (se_lookLeft==*Act && x>0)
        ...
        if (sg_IsNoclipActive()) zi*=1+zi*.1;
    }
    ...
    */
    ...
    if(CenterCockpitFixedBefore()){
        ...
        */
        ...
        VisualsManager::ApplyCameraShake();
        glMatrixMode(GL_MODELVIEW);
        ...
        // The particle engine has a switch of its own and answers to it alone.
        // Drawing them only in enhanced mode meant the switch appeared to do
        // nothing whenever enhanced was off - the engine ran, spawned and
        // updated everything, and then nobody put it on the screen.
        VisualsManager::RenderParticles();
...
void eCamera::Timestep(REAL ts){
...

    {
        ...
        static int lastNoclipKeybind = -1;
        static int lastNoclipKeyToggle = -1;
        static int lastCleanScreenKeybind = -1;
        static int lastCleanScreenKeyToggle = -1;
        static bool firstRun = true;

        if (firstRun) {
            if (sg_noclipKeyToggle != 0) {
                g_NoclipKeybind = sg_noclipKeyToggle;
            } else if (g_NoclipKeybind != 0) {
                sg_noclipKeyToggle = g_NoclipKeybind;
            } else {
                g_NoclipKeybind = SDLK_N;
                sg_noclipKeyToggle = SDLK_N;
            }

            if (sg_noclipKeyCinematic != 0) {
                g_CleanScreenKeybind = sg_noclipKeyCinematic;
            } else if (g_CleanScreenKeybind != 0) {
                sg_noclipKeyCinematic = g_CleanScreenKeybind;
            } else {
                g_CleanScreenKeybind = SDLK_H;
                sg_noclipKeyCinematic = SDLK_H;
            }

            lastNoclipKeybind = g_NoclipKeybind;
            lastNoclipKeyToggle = sg_noclipKeyToggle;
            lastCleanScreenKeybind = g_CleanScreenKeybind;
            lastCleanScreenKeyToggle = sg_noclipKeyCinematic;
            firstRun = false;
        }

        if (g_NoclipKeybind != lastNoclipKeybind) {
            sg_noclipKeyToggle = g_NoclipKeybind;
            lastNoclipKeybind = g_NoclipKeybind;
            lastNoclipKeyToggle = sg_noclipKeyToggle;
        } else if (sg_noclipKeyToggle != lastNoclipKeyToggle) {
            g_NoclipKeybind = sg_noclipKeyToggle;
            lastNoclipKeyToggle = sg_noclipKeyToggle;
            lastNoclipKeybind = g_NoclipKeybind;
        }

        if (g_CleanScreenKeybind != lastCleanScreenKeybind) {
            sg_noclipKeyCinematic = g_CleanScreenKeybind;
            lastCleanScreenKeybind = g_CleanScreenKeybind;
            lastCleanScreenKeyToggle = sg_noclipKeyCinematic;
        } else if (sg_noclipKeyCinematic != lastCleanScreenKeyToggle) {
            g_CleanScreenKeybind = sg_noclipKeyCinematic;
            lastCleanScreenKeyToggle = sg_noclipKeyCinematic;
            lastCleanScreenKeybind = g_CleanScreenKeybind;
        }

        const ePlayer* lp = localPlayer;
        int numkeys;
        const bool* keystate = SDL_GetKeyboardState(&numkeys);

        auto isKeyActive = [&](int keycode) -> bool {
            if (keycode <= 0 || !keystate) return false;
            SDL_Scancode sc = SDL_GetScancodeFromKey((SDL_Keycode)keycode);
            if (sc > 0 && sc < numkeys && keystate[sc]) return true;
            if (keycode >= 'A' && keycode <= 'Z') {
                sc = SDL_GetScancodeFromKey((SDL_Keycode)(keycode + 32));
                if (sc > 0 && sc < numkeys && keystate[sc]) return true;
            }
            if (keycode > 0 && keycode < numkeys && keystate[keycode]) return true;
            return false;
        };

        // Single Authoritative Sync for NoClip Mode
        extern bool g_NoclipMode;
        if (g_NoclipMode != sg_noclipActive) {
            sg_ToggleNoclip(this);
        }

        // Clean Screen / Hide HUD Sync
        extern bool g_CleanScreen;
        if (g_CleanScreen != sg_noclipCinematic) {
            sg_noclipCinematic = g_CleanScreen;
        }
    }
#endif

    if (sg_IsNoclipActive()) {
        if (mode != CAMERA_NOCLIP) {
            mode = CAMERA_NOCLIP;
        }
    } else if (mode == CAMERA_NOCLIP) {
        mode = localPlayer ? localPlayer->startCamera : CAMERA_SMART;
    }

    if (sg_NoclipTimestep(this, ts)) {
        // Apply noclip state directly to camera
        REAL cy = cos(sg_noclipYaw), sy = sin(sg_noclipYaw);
        REAL cp = cos(sg_noclipPitch), sp = sin(sg_noclipPitch);
        pos = sg_noclipPos;
        z   = sg_noclipZ;
        fov = sg_noclipFOV;
        dir = eCoord(cy * cp, sy * cp);
        rise = sp;
        lastPos = pos;
        lastrendertime = se_GameTime();
        return;
...
#define GLANCE_SPEED 20
    ...
    // --- Cardinal Snap + Absolute Lock Glance Mod (always active) ---
    bool isGlancing = glancingLeft || glancingRight || glancingBack;

    if (isGlancing && !wasGlancing) {
        // Just pressed! Snap cycle direction to nearest cardinal (0/90/180/270)
        eCoord cycleDir = CenterDir();
        eCoord snappedDir;
        if (fabs(cycleDir.x) >= fabs(cycleDir.y)) {
            snappedDir = eCoord(cycleDir.x > 0 ? 1 : -1, 0);
        } else {
            snappedDir = eCoord(0, cycleDir.y > 0 ? 1 : -1);
        }

        // Compute absolute world glance direction from snapped cycle forward
        if (glancingBack)
            absTargetDir = snappedDir.Turn(eCoord(-1, 0));  // 180°
        else if (glancingLeft)
            absTargetDir = snappedDir.Turn(eCoord(0, 1));   // +90°
        else // glancingRight
            absTargetDir = snappedDir.Turn(eCoord(0, -1));  // -90°
    }
    wasGlancing = isGlancing;

    eCoord targetRelDir;
    if (isGlancing) {
        // Lock to absolute world direction (convert to relative for rendering)
        targetRelDir = absTargetDir.Turn(dir.Conj());
    } else {
        targetRelDir = eCoord(1, 0); // forward
    }

    // Smoothly rotate glanceDir_ towards targetRelDir
    eCoord diff = targetRelDir.Turn(glanceDir_.Conj());
    float theta = atan2(diff.y, diff.x);
    float max_turn = GLANCE_SPEED * ts * 1.5f;

    if (fabs(theta) <= max_turn) {
        glanceDir_ = targetRelDir;
    } else {
        float turn_amt = (theta > 0) ? max_turn : -max_turn;
        glanceDir_ = glanceDir_.Turn(eCoord(cos(turn_amt), sin(turn_amt)));
    }
    glanceDir_.Normalize();
    // --- End Cardinal Snap + Absolute Lock Glance Mod ---
...
// Invisible watermark
volatile const char* __ilona_copyright_watermark = "Noclip Director Mod exclusively developed by Ilona. All rights reserved.";
