...
// Raise this whenever the shipped retrocycles.cfg is revised and the new
// defaults should reach players who already have a config. The value the
// player last received is kept in MOD_CONFIG_VERSION.
#define RC_SHIPPED_CONFIG_VERSION 3
static int sg_shippedConfigVersion = 0;
static tConfItem<int> sg_shippedConfigVersionConf("MOD_CONFIG_VERSION", sg_shippedConfigVersion);
...
//! @return success flag
bool tConfItemBase::OpenFile( std::ifstream & s, tString const & filename, tSearchPath path )
{
...
void (*st_PostLoadConfigCallback)() = nullptr;
...
void st_LoadConfig( bool printChange )
{
    ...
    // The client ships a full configuration in retrocycles.cfg. Handing it out
    // only on a first-ever run left everyone who had already started the game
    // without it, and never delivered later revisions. It is stamped with a
    // version instead: whenever the shipped version is newer than the one the
    // player's config records, it is applied once and the stamp is advanced.
    // A fresh install and an updated one both receive it; changes the player
    // makes afterwards persist until the shipped defaults are deliberately
    // revised and the version raised.
    if ( sg_shippedConfigVersion < RC_SHIPPED_CONFIG_VERSION )
    {
        Load( config, "retrocycles.cfg" );
        sg_shippedConfigVersion = RC_SHIPPED_CONFIG_VERSION;
    }
    ...
    if (st_PostLoadConfigCallback)
    {
        st_PostLoadConfigCallback();
    }
...
static tConfItemFunc st_DummyRetired1("MOD_COLOR_PICKER",&st_Dummy);
