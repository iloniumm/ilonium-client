...
// [MOD] Custom fog variables
REAL CUSTOM_FOG_R = 0.5f;
REAL CUSTOM_FOG_G = 0.1f;
REAL CUSTOM_FOG_B = 0.8f;
REAL CUSTOM_FOG_DENSITY = 0.03f;
static tConfItem<REAL> conf_fog_r("CUSTOM_FOG_R", CUSTOM_FOG_R);
static tConfItem<REAL> conf_fog_g("CUSTOM_FOG_G", CUSTOM_FOG_G);
static tConfItem<REAL> conf_fog_b("CUSTOM_FOG_B", CUSTOM_FOG_B);
static tConfItem<REAL> conf_fog_density("CUSTOM_FOG_DENSITY", CUSTOM_FOG_DENSITY);
...
tCONTROLLED_PTR(gGame) sg_currentGame;
...
void RenderAllViewports(eGrid *grid){
    ...
    if(sr_glOut){
        ...
        // The arena goes to an offscreen buffer so the light in it can be
        // spread before anybody sees it. Only the arena - the console, the
        // scores and the hud are drawn afterwards, and text that bleeds is
        // text you cannot read.
        cPostProcessor::Instance().BindMSAAFBO();

        // Distance fog.
        //
        // The colour, the density and the switch have all been in the menu for
        // a long time; the only code that could act on them was the block
        // below, commented out, so all three did nothing at all. The arena
        // scales, so the density is read against its size rather than used
        // raw - the same figure has to mean the same haze on a small map and
        // a large one.
        {
            extern bool g_CustomFog;

            if ( g_CustomFog && CUSTOM_FOG_DENSITY > 0.0001f )
            {
                REAL size = gArena::SizeMultiplier();
                if ( size < 0.01f )
                    size = 1;

                GLfloat colour[4] = { (GLfloat)CUSTOM_FOG_R, (GLfloat)CUSTOM_FOG_G,
                                      (GLfloat)CUSTOM_FOG_B, 1.0f };

                glFogi( GL_FOG_MODE, GL_EXP );
                glFogf( GL_FOG_DENSITY, (GLfloat)( CUSTOM_FOG_DENSITY / size ) );
        ...
        cPostProcessor::Instance().UnbindFBO();
        cPostProcessor::Instance().RenderFullscreenQuad();
    }

    // [MOD] Hide console/scores/chat in cinematic clean screen mode
    extern bool sg_noclipCinematic;
    extern bool sg_noclipHideConsole;
    bool hideConsole = sg_noclipCinematic || sg_noclipHideConsole;
    ...
    if( sr_con.autoDisplayAtSwap && !hideConsole )
    {
        sr_con.Render();
    }
...
void Render(eGrid *grid, REAL time, bool swap=true){
    ...
    if (sr_glOut){
        ...
        // [MOD] Update particles & screen shake
        static double lastFrameTime = se_GameTime();
        double currentFrameTime = se_GameTime();
        float dt = currentFrameTime - lastFrameTime;
        if (dt < 0.0f || dt > 0.1f) dt = 0.0f;
        lastFrameTime = currentFrameTime;
        
        VisualsManager::Init();
        VisualsManager::Update(dt);

        RenderAllViewports(grid);

        sr_ResetRenderState(true);

        // Register overlay to render AFTER HUD (inside SwapGL, after DoPerFrameTasks)
    ...
    else
    {
        ...
        static const int sg_idleFrameDelay = 10000;
        tDelay( sn_defaultDelay > sg_idleFrameDelay ? sn_defaultDelay : sg_idleFrameDelay );
    }
#endif
}
...
// What sort of ending this was, so the screen can say it in one word rather
// than leave it to be worked out from a paragraph.
enum sg_EndingKind
{
    sg_EndingTerminated = 0,   //!< the server closed the connection on us
    sg_EndingDenied     = 1,   //!< it would not let us in
    sg_EndingNoAnswer   = 2,   //!< it never answered
    sg_EndingLost       = 3,   //!< the line went down
    sg_EndingNoSync     = 4    //!< we got in, the gamestate never arrived
};

static bool sg_NetworkError( int kind, const tOutput& title, const tOutput& message, REAL timeout )
{
    tString reason;
    if ( sn_DenyReason.Len() > 2 )
        reason = sn_DenyReason;

    tString redirectLine;
    nServerInfoBase * redirect = sn_PeekRedirectTo();
    if ( redirect )
        redirectLine = tString( tOutput( "$network_redirect", redirect->GetConnectionName(), (int)redirect->GetPort() ) );

#ifndef DEDICATED
    if ( sr_glOut )
    {
        ModMenu::ShowDisconnect( kind, tString( title ), tString( message ),
                                 reason, redirectLine, timeout );
        return true;
    }
#endif

    // no window to draw on - the old path still has to work
    ...
    if ( reason.Len() > 0 )
    {
        ...
        message2.AddLiteral("\n");
        message2.AddLiteral(reason);
    }

    if ( redirectLine.Len() > 0 )
        message2.AddLiteral( redirectLine );
...
bool sg_hasLastServer = false;
tString sg_lastServerIP("");
unsigned int sg_lastServerPort = 0;
tString sg_lastServerName("");
...
bool ConnectToServerCore(nServerInfoBase *server)
{
    ...
    if (server) {
        sg_lastServerIP = server->GetConnectionName();
        sg_lastServerPort = server->GetPort();
        sg_lastServerName = server->GetName();
        sg_hasLastServer = true;
    }
    ...
    case nOK:
        break;
    case nTIMEOUT:
        sg_NetworkError(sg_EndingNoAnswer, "$network_message_timeout_title", "$network_message_timeout_inter", 20);
        return false;
        break;

    case nDENIED:
        sg_NetworkError(sg_EndingDenied, "$network_message_denied_title", sn_DenyReason.Len() > 2 ? "$network_message_denied_inter2" : "$network_message_denied_inter", 20);
        return false;
        break;
    }
    }
    ...
    if (sn_GetNetState()==nCLIENT){
        ...
        if (sg_currentGame){
            ...
            sg_NetworkError(sg_EndingNoSync, "$network_message_lateto_title", "$network_message_lateto_inter", 20);
        }
    }
    ...
    if (!sg_RequestedDisconnection && uMenu::quickexit != uMenu::QuickExit_Total )
    {
        ...
        {
        case nOK:
            ret = sg_NetworkError(sg_EndingTerminated, "$network_message_abortconn_title",
                                  "$network_message_abortconn_inter", 20);
            break;
        default:
            ret = sg_NetworkError(sg_EndingLost, "$network_message_lostconn_title",
...
uMenu* g_SettingsMenuPtr = nullptr;
...
#ifndef DEDICATED
    uMenuItemFunction socials(&MainMenu,
                              "Social Networks",
                              "Community links & social networks",
                              &sg_SocialsMenu);

    uMenuItemFunction tickets(&MainMenu,
                              "Support & Tickets",
...
#ifndef DEDICATED
    uMenuItemFunction mod_settings_item
    (&Settings, "Ilona's Mod Menu",
     "Configure mod features (waypoints, minimap, rubber gauge)",
     &sg_ModSettingsMenu);
#endif
...
void gGame::StateUpdate(){
    ...
    while ( sg_AbeforeB( state, stateNext ) && goon )
    {
        ...
        case GS_CREATE_OBJECTS:
            ...
            // Tell the lights what the round is about. Zones belong to this
            // side of the link, so they are handed over rather than looked up.
            {
                std::vector< eCoord > where;
                std::vector< REAL > howBig;

                const tList<eGameObject>& objects = Grid()->GameObjects();
                for ( int i = objects.Len() - 1; i >= 0; --i )
                {
                    gZone * zone = dynamic_cast< gZone * >( objects(i) );
                    if ( !zone ) continue;

                    where.push_back( zone->GetPosition() );
                    howBig.push_back( zone->GetRadius() );
                }

                rc_FloodlightTargets( where.empty() ? 0 : &where[0],
                                      howBig.empty() ? 0 : &howBig[0],
                                      (int)where.size() );
            }
...
// Solo practice: dying costs you the wall you were riding and nothing else.
// Instead of ending the round the cycle is rebuilt at once near the middle of
// the arena, so the arena never empties and the round never resolves.
bool sg_trainingMode = false;
static tConfItem<bool> sg_trainingModeConf( "TRAINING_MODE", sg_trainingMode );

// Rubber is deliberately more effective the worse your connection is:
//   effectiveness *= ( rubber + ping * CYCLE_PING_RUBBER ) / rubber
// Offline the ping is zero, so the same trick that survives on a server dies
// here. This is the ping to pretend to have, in seconds, so practice matches
// the conditions the trick was learned under.
REAL sg_trainingPing = 0.0f;
static tConfItem<REAL> sg_trainingPingConf( "TRAINING_PING", sg_trainingPing );

static void sg_TrainingRespawn( eGrid * grid )
{
    if ( !sg_trainingMode || !grid )
        return;

    // Respawning is an authority decision; a client must not invent cycles.
    if ( sn_GetNetState() == nCLIENT )
        return;

    eRectangle const & bounds = eWallRim::GetBounds();
    eCoord centre( ( bounds.GetLow().x + bounds.GetHigh().x ) * 0.5f,
                   ( bounds.GetLow().y + bounds.GetHigh().y ) * 0.5f );
    REAL spanX = ( bounds.GetHigh().x - bounds.GetLow().x );
    REAL spanY = ( bounds.GetHigh().y - bounds.GetLow().y );
    REAL reach = ( spanX < spanY ? spanX : spanY ) * 0.18f;
    ...
    for ( int i = se_PlayerNetIDs.Len() - 1; i >= 0; --i )
    {
        ...
        // Feed the rubber formula the connection this practice is standing in
        // for. Harmless offline, where nothing else reads the value.
        if ( sg_trainingPing > 0.0f )
            player->ping = sg_trainingPing;

        gCycle * cycle = dynamic_cast< gCycle * >( player->Object() );
        if ( cycle && cycle->Alive() )
            continue;

        // Away from the rim, and scattered a little so consecutive lives do
        // not all begin on the same square.
        eCoord pos = centre;
        if ( reach > 1.0f )
        {
            pos.x += ( random.Get() * 2.0f - 1.0f ) * reach;
            pos.y += ( random.Get() * 2.0f - 1.0f ) * reach;
        }

        int winding = ( random.Get( grid->WindingNumber() ) );
        eCoord dir = grid->GetDirection( winding );

        gCycle * fresh = new gCycle( grid, pos, dir, player );
        player->ControlObject( fresh );
    }
...
bool gGame::GameLoop(bool input){
    ...
    if (input){
        ...
        // OPTIMIZATION: flush input packets immediately after input sync.
        // This ensures turn commands reach the server ASAP, before the
        // render pass (which can take several ms) introduces delay.
        if (sn_GetNetState() == nCLIENT)
        {
            sn_SendPlanned();
        }
    ...
    if (state==GS_PLAY){
        ...
        // [MOD] Post-render network flush: receive any packets that arrived
        // during the render pass (which includes vsync wait time). Without
        // this, packets can queue up and cause all players to appear frozen
        // until the user opens the ESC menu. This second pump per frame
        // keeps the connection responsive even under heavy render load.
        sg_Receive();
        sn_SendPlanned();
        ...
        // [MOD] Post-swap network flush (same as GS_PLAY path)
        sg_Receive();
        sn_SendPlanned();
#endif
        tDelay( 10000 );
...
void sg_EnterGameCleanup()
{
    ...
    // The free camera does not survive the game it was flying around in.
    // Coming back to a match you cannot steer, with nothing on screen saying
    // why, is a puzzle nobody should have to solve twice.
    rc_NoclipStop();

    // A different server has different people on it.
    rc_ForgetLogins();

    // Including whoever was being mourned when the game ended.
    void rc_ForgetTeammateWarning();
    rc_ForgetTeammateWarning();
}
...
void sg_ClientFullscreenMessage( tOutput const & title, tOutput const & message, REAL timeout ){
    ...
    // show message
    // A server message can arrive before there is anything to draw on: a
    // FULLSCREEN_MESSAGE in a config file is executed while settings are still
    // being read, long before the window exists. The old screen tolerated
    // that; ours needs a renderer, so it stands aside when there is not one.
...
// Sound data objects for the voice announcers
static eWavData s_soundMaleFirstBlood("sound/announcer/male/firstblood.wav");
static eWavData s_soundMaleDoubleKill("sound/announcer/male/doublekill.wav");
static eWavData s_soundMaleTripleKill("sound/announcer/male/triplekill.wav");
static eWavData s_soundMaleUltraKill("sound/announcer/male/ultrakill.wav");
static eWavData s_soundMaleRampage("sound/announcer/male/rampage.wav");
static eWavData s_soundMaleKillingSpree("sound/announcer/male/killingspree.wav");
static eWavData s_soundMaleDominating("sound/announcer/male/dominating.wav");
static eWavData s_soundMaleMegaKill("sound/announcer/male/megakill.wav");
static eWavData s_soundMaleUnstoppable("sound/announcer/male/unstoppable.wav");
static eWavData s_soundMaleWickedSick("sound/announcer/male/wickedsick.wav");
static eWavData s_soundMaleMonsterKill("sound/announcer/male/monsterkill.wav");
static eWavData s_soundMaleGodlike("sound/announcer/male/godlike.wav");
static eWavData s_soundMaleHolyShit("sound/announcer/male/holyshit.wav");
static eWavData s_soundMaleOwnage("sound/announcer/male/ownage.wav");

static eWavData s_soundFemaleFirstBlood("sound/announcer/female/firstblood.wav");
static eWavData s_soundFemaleDoubleKill("sound/announcer/female/doublekill.wav");
static eWavData s_soundFemaleTripleKill("sound/announcer/female/triplekill.wav");
static eWavData s_soundFemaleUltraKill("sound/announcer/female/ultrakill.wav");
static eWavData s_soundFemaleRampage("sound/announcer/female/rampage.wav");
static eWavData s_soundFemaleKillingSpree("sound/announcer/female/killingspree.wav");
static eWavData s_soundFemaleDominating("sound/announcer/female/dominating.wav");
static eWavData s_soundFemaleMegaKill("sound/announcer/female/megakill.wav");
static eWavData s_soundFemaleUnstoppable("sound/announcer/female/unstoppable.wav");
static eWavData s_soundFemaleWickedSick("sound/announcer/female/wickedsick.wav");
static eWavData s_soundFemaleMonsterKill("sound/announcer/female/monsterkill.wav");
static eWavData s_soundFemaleGodlike("sound/announcer/female/godlike.wav");
static eWavData s_soundFemaleHolyShit("sound/announcer/female/holyshit.wav");
static eWavData s_soundFemaleOwnage("sound/announcer/female/ownage.wav");

static eWavData s_soundNeutralHit("sound/announcer/neutral/hit.wav");

// Players to wrap the WAV data
static eSoundPlayer s_playerMaleFirstBlood(s_soundMaleFirstBlood);
static eSoundPlayer s_playerMaleDoubleKill(s_soundMaleDoubleKill);
static eSoundPlayer s_playerMaleTripleKill(s_soundMaleTripleKill);
static eSoundPlayer s_playerMaleUltraKill(s_soundMaleUltraKill);
static eSoundPlayer s_playerMaleRampage(s_soundMaleRampage);
static eSoundPlayer s_playerMaleKillingSpree(s_soundMaleKillingSpree);
static eSoundPlayer s_playerMaleDominating(s_soundMaleDominating);
static eSoundPlayer s_playerMaleMegaKill(s_soundMaleMegaKill);
static eSoundPlayer s_playerMaleUnstoppable(s_soundMaleUnstoppable);
static eSoundPlayer s_playerMaleWickedSick(s_soundMaleWickedSick);
static eSoundPlayer s_playerMaleMonsterKill(s_soundMaleMonsterKill);
static eSoundPlayer s_playerMaleGodlike(s_soundMaleGodlike);
static eSoundPlayer s_playerMaleHolyShit(s_soundMaleHolyShit);
static eSoundPlayer s_playerMaleOwnage(s_soundMaleOwnage);

static eSoundPlayer s_playerFemaleFirstBlood(s_soundFemaleFirstBlood);
static eSoundPlayer s_playerFemaleDoubleKill(s_soundFemaleDoubleKill);
static eSoundPlayer s_playerFemaleTripleKill(s_soundFemaleTripleKill);
static eSoundPlayer s_playerFemaleUltraKill(s_soundFemaleUltraKill);
static eSoundPlayer s_playerFemaleRampage(s_soundFemaleRampage);
static eSoundPlayer s_playerFemaleKillingSpree(s_soundFemaleKillingSpree);
static eSoundPlayer s_playerFemaleDominating(s_soundFemaleDominating);
static eSoundPlayer s_playerFemaleMegaKill(s_soundFemaleMegaKill);
static eSoundPlayer s_playerFemaleUnstoppable(s_soundFemaleUnstoppable);
static eSoundPlayer s_playerFemaleWickedSick(s_soundFemaleWickedSick);
static eSoundPlayer s_playerFemaleMonsterKill(s_soundFemaleMonsterKill);
static eSoundPlayer s_playerFemaleGodlike(s_soundFemaleGodlike);
static eSoundPlayer s_playerFemaleHolyShit(s_soundFemaleHolyShit);
static eSoundPlayer s_playerFemaleOwnage(s_soundFemaleOwnage);

static eSoundPlayer s_playerNeutralHit(s_soundNeutralHit);

static bool s_announcerSoundsInitialized = false;
static void InitAnnouncerSounds() {
    if (s_announcerSoundsInitialized) return;
    s_announcerSoundsInitialized = true;
    
    s_playerMaleFirstBlood.MakeGlobal();
    s_playerMaleDoubleKill.MakeGlobal();
    s_playerMaleTripleKill.MakeGlobal();
    s_playerMaleUltraKill.MakeGlobal();
    s_playerMaleRampage.MakeGlobal();
    s_playerMaleKillingSpree.MakeGlobal();
    s_playerMaleDominating.MakeGlobal();
    s_playerMaleMegaKill.MakeGlobal();
    s_playerMaleUnstoppable.MakeGlobal();
    s_playerMaleWickedSick.MakeGlobal();
    s_playerMaleMonsterKill.MakeGlobal();
    s_playerMaleGodlike.MakeGlobal();
    s_playerMaleHolyShit.MakeGlobal();
    s_playerMaleOwnage.MakeGlobal();

    s_playerFemaleFirstBlood.MakeGlobal();
    s_playerFemaleDoubleKill.MakeGlobal();
    s_playerFemaleTripleKill.MakeGlobal();
    s_playerFemaleUltraKill.MakeGlobal();
    s_playerFemaleRampage.MakeGlobal();
    s_playerFemaleKillingSpree.MakeGlobal();
    s_playerFemaleDominating.MakeGlobal();
    s_playerFemaleMegaKill.MakeGlobal();
    s_playerFemaleUnstoppable.MakeGlobal();
    s_playerFemaleWickedSick.MakeGlobal();
    s_playerFemaleMonsterKill.MakeGlobal();
    s_playerFemaleGodlike.MakeGlobal();
    s_playerFemaleHolyShit.MakeGlobal();
    s_playerFemaleOwnage.MakeGlobal();

    s_playerNeutralHit.MakeGlobal();
}

static void TriggerAnnouncerSound(int announcerPack, int soundId) {
    InitAnnouncerSounds();

    s_playerMaleFirstBlood.End();
    s_playerMaleDoubleKill.End();
    s_playerMaleTripleKill.End();
    s_playerMaleUltraKill.End();
    s_playerMaleRampage.End();
    s_playerMaleKillingSpree.End();
    s_playerMaleDominating.End();
    s_playerMaleMegaKill.End();
    s_playerMaleUnstoppable.End();
    s_playerMaleWickedSick.End();
    s_playerMaleMonsterKill.End();
    s_playerMaleGodlike.End();
    s_playerMaleHolyShit.End();
    s_playerMaleOwnage.End();

    s_playerFemaleFirstBlood.End();
    s_playerFemaleDoubleKill.End();
    s_playerFemaleTripleKill.End();
    s_playerFemaleUltraKill.End();
    s_playerFemaleRampage.End();
    s_playerFemaleKillingSpree.End();
    s_playerFemaleDominating.End();
    s_playerFemaleMegaKill.End();
    s_playerFemaleUnstoppable.End();
    s_playerFemaleWickedSick.End();
    s_playerFemaleMonsterKill.End();
    s_playerFemaleGodlike.End();
    s_playerFemaleHolyShit.End();
    s_playerFemaleOwnage.End();

    s_playerNeutralHit.End();

    if (announcerPack == 0) { // Male
        switch (soundId) {
            case 0: s_playerMaleFirstBlood.Reset(); break;
            case 1: s_playerMaleDoubleKill.Reset(); break;
            case 2: s_playerMaleTripleKill.Reset(); break;
            case 3: s_playerMaleUltraKill.Reset(); break;
            case 4: s_playerMaleRampage.Reset(); break;
            case 5: s_playerMaleKillingSpree.Reset(); break;
            case 6: s_playerMaleDominating.Reset(); break;
            case 7: s_playerMaleMegaKill.Reset(); break;
            case 8: s_playerMaleUnstoppable.Reset(); break;
            case 9: s_playerMaleWickedSick.Reset(); break;
            case 10: s_playerMaleMonsterKill.Reset(); break;
            case 11: s_playerMaleGodlike.Reset(); break;
            case 12: s_playerMaleHolyShit.Reset(); break;
            case 13: s_playerMaleOwnage.Reset(); break;
        }
    } else if (announcerPack == 1) { // Female
        switch (soundId) {
            case 0: s_playerFemaleFirstBlood.Reset(); break;
            case 1: s_playerFemaleDoubleKill.Reset(); break;
            case 2: s_playerFemaleTripleKill.Reset(); break;
            case 3: s_playerFemaleUltraKill.Reset(); break;
            case 4: s_playerFemaleRampage.Reset(); break;
            case 5: s_playerFemaleKillingSpree.Reset(); break;
            case 6: s_playerFemaleDominating.Reset(); break;
            case 7: s_playerFemaleMegaKill.Reset(); break;
            case 8: s_playerFemaleUnstoppable.Reset(); break;
            case 9: s_playerFemaleWickedSick.Reset(); break;
            case 10: s_playerFemaleMonsterKill.Reset(); break;
            case 11: s_playerFemaleGodlike.Reset(); break;
            case 12: s_playerFemaleHolyShit.Reset(); break;
            case 13: s_playerFemaleOwnage.Reset(); break;
        }
    } else if (announcerPack == 2) { // Neutral (Hitmarker)
        s_playerNeutralHit.Reset();
    }
}

static int s_localKillStreak = 0;
static int s_localComboCount = 0;
static double s_lastKillTime = 0.0;
static bool s_firstBloodPlayed = false;

static bool IsLocalPlayerName(const std::string& name) {
    for (int i = 0; i < MAX_PLAYERS; ++i) {
        if (ePlayer::PlayerIsInGame(i)) {
            ePlayer* lp = ePlayer::PlayerConfig(i);
            if (lp) {
                tString lpClean = tColoredString::RemoveColors(lp->Name());
                if (name == static_cast<const char*>(lpClean)) {
                    return true;
                }
            }
        }
...
static void OnPlayerKillOrDeath(const std::string& killer, const std::string& prey, bool isSuicide, bool isTeamkill) {
    if (!sg_modKillSoundsEnabled) return;

    double currentTime = se_GameTime();
    
    // Reset round-specific multikills/blood if the round restarted
    if (currentTime <= 0.0) {
        s_localComboCount = 0;
        s_lastKillTime = 0.0;
        s_firstBloodPlayed = false;
    }

    bool localIsKiller = IsLocalPlayerName(killer);
    bool localIsPrey = IsLocalPlayerName(prey);

    if (localIsKiller) {
        if (isSuicide || killer == prey) {
            s_localKillStreak = 0;
            s_localComboCount = 0;
            TriggerAnnouncerSound(sg_modKillAnnouncerPack, 13); // Ownage
        } else if (isTeamkill) {
            s_localKillStreak = 0;
            s_localComboCount = 0;
            TriggerAnnouncerSound(sg_modKillAnnouncerPack, 13); // Ownage
            ...
            if (currentTime - s_lastKillTime <= 15.0 && s_lastKillTime > 0.0) {
                s_localComboCount++;
            } else {
                s_localComboCount = 1;
            }
            s_lastKillTime = currentTime;

            bool playFirstBlood = false;
            if (!s_firstBloodPlayed) {
                s_firstBloodPlayed = true;
                playFirstBlood = true;
            }

            if (sg_modKillAnnouncerPack == 2) {
                TriggerAnnouncerSound(2, 0);
            } else {
                int soundToPlay = -1;
                
                int streakSound = -1;
                if (s_localKillStreak == 3) streakSound = 5;       // Killing Spree
                else if (s_localKillStreak == 4) streakSound = 6;  // Dominating
                else if (s_localKillStreak == 5) streakSound = 7;  // Mega Kill
                else if (s_localKillStreak == 6) streakSound = 8;  // Unstoppable
                else if (s_localKillStreak == 7) streakSound = 9;  // Wicked Sick
                else if (s_localKillStreak == 8) streakSound = 10; // Monster Kill
                else if (s_localKillStreak == 9) streakSound = 11; // Godlike
                else if (s_localKillStreak >= 10) streakSound = 12; // Holy Shit

                int multikillSound = -1;
                if (s_localComboCount == 2) multikillSound = 1;      // Double Kill
                else if (s_localComboCount == 3) multikillSound = 2; // Triple Kill
                else if (s_localComboCount == 4) multikillSound = 3; // Ultra Kill
                else if (s_localComboCount >= 5) multikillSound = 4; // Rampage

                if (playFirstBlood) {
                    soundToPlay = 0; // First Blood
                } else if (multikillSound != -1) {
                    soundToPlay = multikillSound;
                } else if (streakSound != -1) {
                    soundToPlay = streakSound;
                }

                if (soundToPlay != -1) {
                    TriggerAnnouncerSound(sg_modKillAnnouncerPack, soundToPlay);
                }
            }
        }
    } else if (localIsPrey) {
...
static void HandleConsoleKillMessage(const char* rawMessage) {
    if (!rawMessage || !sg_modKillSoundsEnabled) return;
    
    tString clean = tColoredString::RemoveColors(rawMessage);
    std::string s(static_cast<const char*>(clean));
    ...
    size_t teamkillPos = s.find(" core dumped teammate ");
    if (teamkillPos != std::string::npos) {
        killer = s.substr(0, teamkillPos);
        std::string suffix = s.substr(teamkillPos + strlen(" core dumped teammate "));
        size_t exclam = suffix.find("! Boo!");
        if (exclam != std::string::npos) {
            prey = suffix.substr(0, exclam);
        } else {
            prey = suffix;
        }
        isKill = true;
        ...
        size_t fragPos = s.find(" core dumped ");
        if (fragPos != std::string::npos) {
            killer = s.substr(0, fragPos);
            std::string suffix = s.substr(fragPos + strlen(" core dumped "));
            size_t forPos = suffix.find(" for ");
            if (forPos != std::string::npos) {
                prey = suffix.substr(0, forPos);
            } else {
                prey = suffix;
            }
            isKill = true;
        } else {
            size_t suicidePos = s.find(" committed suicide");
            if (suicidePos != std::string::npos) {
                killer = s.substr(0, suicidePos);
                prey = killer;
                isSuicide = true;
            }
        }
    ...
    auto trim = [](std::string& str) {
        while (!str.empty() && (str.back() == ' ' || str.back() == '.' || str.back() == '!')) str.pop_back();
    ...
    if (isKill || isSuicide) {
        OnPlayerKillOrDeath(killer, prey, isSuicide, isTeamkill);
    }
}

class KillAnnouncerConsoleFilter : public tConsoleFilter {
protected:
    virtual void DoFilterLine(tString & line) override {
        HandleConsoleKillMessage(static_cast<const char*>(line));
    }
};
static KillAnnouncerConsoleFilter s_killAnnouncerConsoleFilter;
