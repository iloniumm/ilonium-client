...
#ifndef DEDICATED
static REAL sg_splashDuration = 1.5;
static tConfItem<REAL> conf_splashDuration("SPLASH_DURATION", sg_splashDuration);
...
static void welcome(){
    ...
    else
    {
        ...
        if ( showSplash )
        {
            timeout = tSysTimeFloat() + sg_splashDuration;
    ...
    if ( sr_glOut )
    {
        ...
        gLogo::Display();
        // no second clear here: it wiped the logo off the back buffer before
        // the swap below ever put it on screen
    }
    rSysDep::SwapGL();

    // The language picker and the setup sheet used to stand between a new
    // player and the game. Everything they asked for now arrives configured,
    // and the key template the setup sheet applied was landing on top of that
    // configuration and leaving a mixture of both.
    sr_textOut = textOutBack;
...
bool filter(void *userdata, SDL_Event *tEvent){
    ...
    if ( !recursion )
    {
        ...
        if (!imguiCaptureMouse && !ModMenu::g_MainMenuActive && !ModMenu::g_InGameMenuOpen && !ModMenu::g_ModalOverlayOpen && (su_mouseGrab || sg_IsNoclipActive())) {
        ...
        if (tEvent->type == SDL_EVENT_WINDOW_OCCLUDED)
        {
            return true;
        }
...
//! Every full-screen notice the game raises, in the client's own clothes.
//!
//! One registration covers them all - the server not answering, the welcome
//! screens, anything else that reaches for tConsole::Message - because they
//! all arrive through this callback. Falls back to the old screen when there
//! is nothing to draw on yet, which happens while settings are still loading.
static bool sg_Message( tOutput const & message, tOutput const & interpretation, REAL timeout )
{
#ifndef DEDICATED
    if ( sr_glOut )
    {
        char const * head = (char const *)message;
        char const * body = (char const *)interpretation;

        ModMenu::ShowServerMessage( head ? head : "", body ? body : "", (float)timeout );
        return true;
    }
#endif

    // no window to draw on, or a build with no window at all
    return uMenu::Message( message, interpretation, timeout );
}
...
#ifdef HAVE_MIMALLOC
    // ── mimalloc verification ──────────────────────────────────────────
    // mi_version() is only accessible when the header is included and the
    // library is linked. If this line appears in the log, mimalloc is
    // actively overriding malloc/new/free via the -override linkage.
    {
        int miVer = mi_version();
        std::cerr << "[ilonium] mimalloc v"
                  << (miVer / 100) << "." << (miVer % 100)
                  << " active — system allocator overridden.\n";
        // Sanity check: allocate and free via mimalloc
...
#define NOSOUND
...
#ifndef DEDICATED
            // Has to happen before the video subsystem comes up: X11 takes the
            // window class from the identifier, and without one SDL falls back
            // to the name of the binary. That class and the window title are
            // the whole of what the desktop - and Discord, when it notices a
            // game running - knows this build by.
            SDL_SetAppMetadata( sr_ClientName(), VERSION, sr_ClientId() );

            // SDL3 reports success as a bool, so the old "< 0" test never fired
            // and a failed audio subsystem took video down with it, silently
            if (
#ifndef NOSOUND
            ...
            // The identity this installation speaks with. Made here, once, so
            // it exists before anything needs it and never has to be created
            // in the middle of somebody sending a message.
            rc_IloniumReady();

            tConsole::RegisterMessageCallback(&sg_Message);
