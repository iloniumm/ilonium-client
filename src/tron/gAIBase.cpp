...
bool cfg_GodModeBot = true;
static tSettingItem< bool > cfg_GodModeBotConf( "GOD_MODE_BOT", cfg_GodModeBot );

#include <vector>
#include <algorithm>

// --- Apex Predator AI Subsystems ---
static REAL sg_GetZoneExitDistance(eCoord pos, eCoord dir, gZone* zone) {
    if (!zone || zone->GetRadius() <= 0.0f) return 10000.0f;
    eCoord center = zone->GetPosition();
    REAL r = zone->GetRadius();
    eCoord delta = pos - center;

    REAL a = eCoord::F(dir, dir);
    REAL b = 2.0f * eCoord::F(dir, delta);
    REAL c = eCoord::F(delta, delta) - r * r;
    ...
    // If we are outside and moving further away, immediate danger (distance = 0)
    if (eCoord::F(delta, delta) >= r * r && eCoord::F(dir, delta) >= 0.0f) return 0.0f;
...
static REAL sg_EvaluateSpace(const gCycle* me, eCoord startPos, eCoord startDir, int maxDepth) {
    REAL totalFreeSpace = 0.0f;
    std::vector<SpaceNode> queue;
    queue.push_back({startPos, startDir, 0});
    
    int iters = 0;
    while (!queue.empty() && iters < 20) {
        iters++;
        SpaceNode node = queue.front();
        queue.erase(queue.begin());

        gSensor sensor(const_cast<gCycle*>(me), node.pos, node.dir);
        sensor.detect(100.0f); 

        REAL dist = sensor.hit;
        totalFreeSpace += dist;

        if (node.depth < maxDepth && dist > 1.0f && dist < 99.0f) {
            eCoord endPos = node.pos + node.dir * (dist - 0.5f);
            eCoord dirLeft(-node.dir.y, node.dir.x);
            eCoord dirRight(node.dir.y, -node.dir.x);
            queue.push_back({endPos, dirLeft, node.depth + 1});
            queue.push_back({endPos, dirRight, node.depth + 1});
        }
    }
    return totalFreeSpace;
}

static eCoord sg_GetInterceptPoint(const gCycle* target, REAL extrapolationTime) {
    if (!target || !target->Alive()) return eCoord(0, 0);
    eCoord pos = target->Position();
    eCoord dir = target->Direction();
    REAL speed = target->Speed();
    REAL lookAheadDist = speed * extrapolationTime;
    eCoord intercept = pos + dir * lookAheadDist;
    
    gSensor enemySensor(const_cast<gCycle*>(target), pos, dir);
    enemySensor.detect(lookAheadDist);
    if (enemySensor.hit < lookAheadDist) {
        intercept = pos + dir * (enemySensor.hit - 0.5f);
    }
    return intercept;
}

static REAL sg_CalculateRubberCost(const gCycle* me, eCoord targetPos) {
    eCoord myPos = me->Position();
    eCoord myDir = me->Direction();
    eCoord toTarget = targetPos - myPos;
    REAL distToTarget = toTarget.Norm();
    if (distToTarget < 0.01f) return 0.0f;
    eCoord dir = toTarget * (1.0f / distToTarget);
    
    gSensor sensor(const_cast<gCycle*>(me), myPos, dir);
    sensor.detect(distToTarget);
    if (sensor.hit < distToTarget) {
        return (distToTarget - sensor.hit) * 0.5f; 
    }
    return 0.0f;
}
// ------------------------------------------
...
void gAIPlayer::ThinkSurvive(  ThinkData & data )
{
    ...
    */
    ...
    {
        data.thinkAgain = (cfg_GodModeBot) ? 0.005f : .5f;
        return;
    }
...
bool gAIPlayer::EmergencySurvive( ThinkData & data, int enemyevade, int preferedSide)
{
    ...
    if (cfg_GodModeBot) {
        if (Object() && Object()->Grid()) {
            ...
            for (int j = gameObjects.Len() - 1; j >= 0; j--) {
                ...
                if (z && z->GetRadius() > maxRadius) {
                    zone = z;
                    maxRadius = z->GetRadius();
                }
            }
        }
    }

    REAL distToCenter = 0.0f;
    REAL zoneRadius = 0.0f;
    eCoord toCenter(0.0f, 0.0f);
    if (zone) {
        zoneRadius = zone->GetRadius();
        toCenter = zone->GetPosition() - Object()->Position();
        distToCenter = toCenter.Norm();
        if (distToCenter > 0.001f) {
            toCenter = toCenter * (1.0f / distToCenter);
        }
    }

    eCoord dirForward = Object()->Direction();
    int currentDirectionWinding = Object()->Grid()->DirectionWinding(dirForward);
    int wnLeft = currentDirectionWinding;
    Object()->Grid()->Turn(wnLeft, -1);
    eCoord dirLeft = Object()->Grid()->GetDirection(wnLeft);
    int wnRight = currentDirectionWinding;
    Object()->Grid()->Turn(wnRight, 1);
    eCoord dirRight = Object()->Grid()->GetDirection(wnRight);

    if (zone) {
        REAL distFront = sg_GetZoneExitDistance(Object()->Position(), dirForward, zone);
        REAL distLeft = sg_GetZoneExitDistance(Object()->Position(), dirLeft, zone);
        REAL distRight = sg_GetZoneExitDistance(Object()->Position(), dirRight, zone);

        // [APEX PREDATOR] Project the zone directly onto the AI's primary sensor grid.
        // This ensures the AI treats the Zone like a solid physical wall natively.
        if (distFront < data.front.distance) {
            const_cast<gAISensor&>(data.front).distance = distFront;
            const_cast<gAISensor&>(data.front).hit = true;
            const_cast<gAISensor&>(data.front).front.wallType = gSENSOR_RIM;
        }
        if (distLeft < data.left.distance) {
            const_cast<gAISensor&>(data.left).distance = distLeft;
            const_cast<gAISensor&>(data.left).hit = true;
            const_cast<gAISensor&>(data.left).front.wallType = gSENSOR_RIM;
        }
        if (distRight < data.right.distance) {
            const_cast<gAISensor&>(data.right).distance = distRight;
            const_cast<gAISensor&>(data.right).hit = true;
            const_cast<gAISensor&>(data.right).front.wallType = gSENSOR_RIM;
        }
    }
    ...
    // [APEX PREDATOR] We MUST check CanMakeTurn so we don't spam the queue infinitely.
    // The bot still evaluates at 200Hz, so it will turn perfectly on the very tick it's allowed to.
    ...
    */
    ...
    bool hasZoneDanger = false;
    if (zone && distToCenter > zoneRadius - 10.0f) {
        hasZoneDanger = true;
        
        REAL dotF = eCoord::F(dirForward, toCenter);
        REAL dotL = eCoord::F(dirLeft, toCenter);
        REAL dotR = eCoord::F(dirRight, toCenter);

        // If moving away from center, add penalty
        if (dotF < 0.0f) {
            frontDanger[SPACELEVEL] += 100;
        }
        if (dotL < 0.0f) {
            sideDanger[SPACELEVEL][0] += 100;
        }
        if (dotR < 0.0f) {
            sideDanger[SPACELEVEL][1] += 100;
        }

        // Critical zone danger when very close to edge
        if (distToCenter > zoneRadius - 5.0f) {
            if (dotF < 0.0f) {
                frontDanger[SPACELEVEL] += 500;
            }
            if (dotL < 0.0f) {
                sideDanger[SPACELEVEL][0] += 500;
            }
            if (dotR < 0.0f) {
                sideDanger[SPACELEVEL][1] += 500;
            }
        }
    }

    // [APEX PREDATOR] Frame-Perfect Grinding
    if (cfg_GodModeBot && !hasZoneDanger) {
        REAL dt = se_AverageFrameTime();
        if (dt < 0.001f) dt = 0.005f;
        if (dt > 0.1f) dt = 0.05f;
        
        // Wait until we are perfectly close to the wall (e.g. 1-2 ticks away)
        REAL perfectDist = Object()->Speed() * dt + 0.5f;
        
        // Only nullify danger if we are further than the perfect turning distance.
        // DO NOT nullify if we are critically close, otherwise we crash straight into the wall!
        if (front.Hit() && front.distance >= perfectDist) {
            // Nullify front dangers so we can get closer before turning
    ...
    // [APEX PREDATOR] Flood-fill space maximization
    if (!turn && cfg_GodModeBot) {
        eCoord pos = Object()->Position();
        eCoord cycleDir = Object()->Direction();
        eCoord dirLeft(-cycleDir.y, cycleDir.x);
        eCoord dirRight(cycleDir.y, -cycleDir.x);
        REAL spaceLeft = sg_EvaluateSpace(Object(), pos, dirLeft, 3);
        REAL spaceRight = sg_EvaluateSpace(Object(), pos, dirRight, 3);
        if (spaceLeft > spaceRight + 0.1f) turn = -1;
        else if (spaceRight > spaceLeft + 0.1f) turn = 1;
    }
...
REAL gAIPlayer::Think(){
    ...
    if (cfg_GodModeBot) {
        ret = 0.005f; // Zero latency "neurons"
    }
...
void gAIPlayer::ActOnData( ThinkData & data )
{
    ...
    // Instant U-Turn logic:
    if (cfg_GodModeBot && data.turn != 0) {
        REAL dt = se_AverageFrameTime();
        if (dt < 0.001f) dt = 0.005f;
        if (dt > 0.1f) dt = 0.05f;
        REAL uTurnDist = Object()->Speed() * dt + 0.08f;
        
        // If we turned left (-1) but left side was blocked, turn left again
        if (data.turn == -1 && data.left.Hit() && data.left.distance < uTurnDist) {
            Object()->Turn(-1);
        }
        // If we turned right (1) but right side was blocked, turn right again
        else if (data.turn == 1 && data.right.Hit() && data.right.distance < uTurnDist) {
            Object()->Turn(1);
        }
    }
...
void gAIPlayer::Timestep(REAL time){
    ...
    if (bool(Object()) && Object()->Alive() && (cfg_GodModeBot || nextTime<time)){
        gRandomController random( randomizer_ );

        REAL nextthought=Think();
        if (cfg_GodModeBot) {
            nextTime = time - 0.001f;
        } else {
