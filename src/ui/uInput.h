...
bool su_IsActionActive(uAction* act, int player = 0);
bool su_HasAnyBind(uAction* act, int player = 0);

// the three families of bindable actions
enum uActionScope { uACTION_PLAYER = 0, uACTION_CAMERA, uACTION_GLOBAL };

// head of a scope's action list, ordered the way the configuration menus
// present it: by priority first, then alphabetically. Walk it with Next().
uAction * su_ActionList( uActionScope scope );

// index NewBind() and CheckPlayer() expect for a bind in the given scope;
// global actions live on 0, local player n on n+1
int su_BindSlot( uActionScope scope, int localPlayer );

// human readable name of a bound key
const char * su_KeyName( int sym );
#endif
