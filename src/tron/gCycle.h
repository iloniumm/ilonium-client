...
public:
    ...
    // [MOD] Perfect Turn Intent
    bool intentTurnLeft;
    bool intentTurnRight;
    
    // [MOD] AutoEscape State
    int intentAutoEscapeDir;
    REAL previousFrameRubber;

    // --- ANTI-360 SLIDING WINDOW ---
    REAL rightTurnTimes[3];
    REAL leftTurnTimes[3];
    int rightTurnCount;
    int leftTurnCount;

    // [MOD] Perfect Turn Telemetry
    int debug_GapTimerTicks;      // countdown after turn for gap measurement
    int debug_lastTurnDir;        // -1 = left, +1 = right (to know which side to measure)
