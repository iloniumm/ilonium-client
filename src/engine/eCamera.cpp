...
static tSettingItem<REAL> sg_noclipSpeedConf("NOCLIP_SPEED", sg_noclipSpeed);
static tSettingItem<REAL> sg_noclipSlowConf("NOCLIP_SLOW_FACTOR", sg_noclipSlowFactor);
static tSettingItem<REAL> sg_noclipSensConf("NOCLIP_MOUSE_SENS", sg_noclipMouseSens);

static tSettingItem<int> sg_nkForward ("NOCLIP_KEY_FORWARD",  sg_noclipKeyForward);
static tSettingItem<int> sg_nkBack    ("NOCLIP_KEY_BACK",     sg_noclipKeyBack);
static tSettingItem<int> sg_nkLeft    ("NOCLIP_KEY_LEFT",     sg_noclipKeyLeft);
static tSettingItem<int> sg_nkRight   ("NOCLIP_KEY_RIGHT",    sg_noclipKeyRight);
static tSettingItem<int> sg_nkUp      ("NOCLIP_KEY_UP",       sg_noclipKeyUp);
static tSettingItem<int> sg_nkDown    ("NOCLIP_KEY_DOWN",     sg_noclipKeyDown);
static tSettingItem<int> sg_nkZoomIn  ("NOCLIP_KEY_ZOOM_IN",  sg_noclipKeyZoomIn);
static tSettingItem<int> sg_nkZoomOut ("NOCLIP_KEY_ZOOM_OUT", sg_noclipKeyZoomOut);
static tSettingItem<int> sg_nkCenter  ("NOCLIP_KEY_CENTER",   sg_noclipKeyCenter);
static tSettingItem<int> sg_nkLookAt  ("NOCLIP_KEY_LOOKAT",   sg_noclipKeyLookAt);
static tSettingItem<int> sg_nkLock    ("NOCLIP_KEY_LOCK",     sg_noclipKeyLock);
static tSettingItem<int> sg_nkFollow  ("NOCLIP_KEY_FOLLOW",   sg_noclipKeyFollow);
static tSettingItem<int> sg_nkNextPl  ("NOCLIP_KEY_NEXT_PL",  sg_noclipKeyNextPl);
static tSettingItem<int> sg_nkPrevPl  ("NOCLIP_KEY_PREV_PL",  sg_noclipKeyPrevPl);
static tSettingItem<int> sg_nkSlow    ("NOCLIP_KEY_SLOW",     sg_noclipKeySlow);
...
void sg_ToggleNoclip(eCamera* cam) {
    ...
    if (sg_noclipActive) {
        ...
        m << tColoredString::ColorString(0.3, 1.0, 0.8)
          ...
          << "ON  WASD:move  Q/E/Wheel:zoom  Z/C:auto-cam  Ctrl:lock  F:follow  Arrows:TP\n";
        con << m;
    } else {
        if (sg_noclipSavedMouseGrab) {
            SDL_HideCursor();
            SDL_WM_GrabInput(SDL_GRAB_ON);
        } else {
            SDL_HideCursor();
...
static tSettingItem<bool> sg_noclipCinematicConf("NOCLIP_CINEMATIC", sg_noclipCinematic);
...
static tSettingItem<REAL> sg_orbitSpeedConf("NOCLIP_ORBIT_SPEED", sg_noclipOrbitSpeed);
static tSettingItem<REAL> sg_orbitRadiusConf("NOCLIP_ORBIT_RADIUS", sg_noclipOrbitRadius);
static tSettingItem<REAL> sg_orbitHeightConf("NOCLIP_ORBIT_HEIGHT", sg_noclipOrbitHeight);

REAL sg_noclipSmoothFactor = 5.0;
REAL sg_noclipFollowDist = 35;
REAL sg_noclipFollowHeight = 20;
static tSettingItem<REAL> sg_smoothFactConf("NOCLIP_SMOOTH_FACTOR", sg_noclipSmoothFactor);
static tSettingItem<REAL> sg_followDistConf("NOCLIP_FOLLOW_DIST", sg_noclipFollowDist);
static tSettingItem<REAL> sg_followHeightConf("NOCLIP_FOLLOW_HEIGHT", sg_noclipFollowHeight);
...
static tSettingItem<int> sg_nkToggle   ("NOCLIP_KEY_TOGGLE",     sg_noclipKeyToggle);
static tSettingItem<int> sg_nkCinematic("NOCLIP_KEY_CINEMATIC",  sg_noclipKeyCinematic);
static tSettingItem<bool> sg_ncHideCon ("NOCLIP_HIDE_CONSOLE",   sg_noclipHideConsole);
static tSettingItem<bool> sg_ncHideNam ("NOCLIP_HIDE_NAMES",     sg_noclipHideNames);
static tSettingItem<int> sg_nkOrbit    ("NOCLIP_KEY_ORBIT",      sg_noclipKeyOrbit);
static tSettingItem<int> sg_nkSmooth   ("NOCLIP_KEY_SMOOTH",     sg_noclipKeySmooth);
static tSettingItem<int> sg_nkZoneFocus("NOCLIP_KEY_ZONE_FOCUS", sg_noclipKeyZoneFocus);
static tSettingItem<int> sg_nkMapCenter("NOCLIP_KEY_MAP_CENTER", sg_noclipKeyMapCenter);
...
static bool sg_NoclipTimestep(eCamera* cam, REAL ts) {
    ...
    if ((lp && lp->netPlayer && lp->netPlayer->IsChatting()) || uMenu::MenuActive() ||
        (ImGui::GetCurrentContext() && ImGui::GetIO().WantCaptureKeyboard)) {
        return true; // Keep overriding engine, but do not process inputs
    }
    ...
    if (!sg_noclipLocked) {
        ...
        if (sg_noclipPitch >  1.57) sg_noclipPitch =  1.57;
        if (sg_noclipPitch < -1.57) sg_noclipPitch = -1.57;
    }

    bool lockKey = isKeyPressed(sg_noclipKeyLock);
    if (lockKey && !sg_noclipCtrlLast) {
        sg_noclipLocked = !sg_noclipLocked;
    }
    sg_noclipCtrlLast = lockKey;

    REAL speed = sg_noclipSpeed * ts;
    if (sg_noclipKeySlow > 0 && isKeyPressed(sg_noclipKeySlow)) {
    ...
    if (!sg_noclipLocked) {
        if (isKeyPressed(sg_noclipKeyForward)) {
            sg_noclipPos = sg_noclipPos + forward * speed;
            sg_noclipZ  += sin(sg_noclipPitch) * speed;
        }
        if (isKeyPressed(sg_noclipKeyBack)) {
            sg_noclipPos = sg_noclipPos - forward * speed;
            sg_noclipZ  -= sin(sg_noclipPitch) * speed;
        }
        if (isKeyPressed(sg_noclipKeyLeft)) {
            sg_noclipPos = sg_noclipPos - right * speed;
        }
        if (isKeyPressed(sg_noclipKeyRight)) {
            sg_noclipPos = sg_noclipPos + right * speed;
        }
        if (isKeyPressed(sg_noclipKeyUp)) sg_noclipZ += speed;
        if (isKeyPressed(sg_noclipKeyDown)) sg_noclipZ -= speed;
        ...
        if (fovChanged) {
            tString s; s << "Zoom: " << (int)sg_noclipFOV;
            sr_con.DoCenterDisplay(s, 2.0, 0.3, 1.0, 0.8);
        }
...
void eCamera::Timestep(REAL ts){
    ...
    if (DemoPlayerManager::Instance().IsViewerActive() && DemoPlayerManager::Instance().IsLoaded()) {
        ...
        // Escape / Tab or Exit button to exit playback
        int numkeys;
        const bool* keystate = SDL_GetKeyboardState(&numkeys);
        auto isKeyPressed = [&](int key) -> bool {
            if (key <= 0) return false;
        ...
        if (dp.ExitRequested() || (keystate && (isKeyPressed(SDLK_ESCAPE) || isKeyPressed(SDLK_TAB)))) {
            dp.Pause();
        ...
        // Handle spectator/playback controls from keyboard
        static bool keysPressed[SDL_SCANCODE_COUNT] = {false};
        auto wasPressed = [&](int key) {
            SDL_Scancode sc = SDL_GetScancodeFromKey((SDL_Keycode)key);
            if (sc < 0 || sc >= SDL_SCANCODE_COUNT) return false;
            bool state = keystate && keystate[sc];
            bool prev = keysPressed[sc];
            keysPressed[sc] = state;
            return state && !prev;
        };

        if (wasPressed(SDLK_SPACE))    { dp.TogglePlayPause(); }
        if (wasPressed(SDLK_LEFT))     { dp.SeekToTime(dp.CurrentTime() - 5.0); }
        if (wasPressed(SDLK_RIGHT))    { dp.SeekToTime(dp.CurrentTime() + 5.0); }
        if (wasPressed(SDLK_UP))       { dp.SpeedUp(); }
        if (wasPressed(SDLK_DOWN))     { dp.SlowDown(); }
        if (wasPressed(SDLK_PAGEUP))   { dp.PrevRound(); }
        if (wasPressed(SDLK_PAGEDOWN)) { dp.NextRound(); }
        if (wasPressed(SDLK_N))        { dp.NextSpectatorTarget(); }
        if (wasPressed(SDLK_P))        { dp.PrevSpectatorTarget(); }
        if (wasPressed(SDLK_F))        { dp.ToggleCamMode(); }
        ...
        if (dp.CamMode() == DemoCamMode::Freecam) {
            float dx_mouse = 0.f, dy_mouse = 0.f;
            bool fwd = false, back = false, left = false, right = false, up = false, down = false;
            ...
            if (rmbDown) {
                ...
                if (keystate) {
                    fwd   = isKeyPressed(SDLK_W);
                    back  = isKeyPressed(SDLK_S);
                    left  = isKeyPressed(SDLK_A);
                    right = isKeyPressed(SDLK_D);
                    up    = isKeyPressed(SDLK_Q) || isKeyPressed(SDLK_SPACE);
                    down  = isKeyPressed(SDLK_E);
                }
            } else {
                SDL_ShowCursor();
                SDL_WM_GrabInput(SDL_GRAB_OFF);
            ...
            dp.FreecamInput(ts, dx_mouse, dy_mouse, fwd, back, left, right, up, down);

            float ox, oy, oz, dx, dy;
            dp.GetFreecamTransform(ox, oy, oz, dx, dy);
            pos = eCoord(ox, oy);
            z = oz;
            dir = eCoord(dx, dy);
            rise = sinf(dp.Freecam().pitch);
            fov = 60.f;
            lastPos = pos;
            lastrendertime = se_GameTime();
            return;
        } else if (dp.CamMode() == DemoCamMode::Spectator) {
            SDL_ShowCursor();
            SDL_WM_GrabInput(SDL_GRAB_OFF);
            int rmx, rmy;
            SDL_GetRelativeMouseState(&rmx, &rmy);

            gCycle* targetCycle = nullptr;
            const auto& states = dp.CycleStates();
            int tgt = dp.SpectatorTarget();
            if (tgt >= 0 && tgt < (int)states.size()) {
                int slot = states[tgt].playerSlot;
                const auto& gameObjects = grid->GameObjects();
                for (int i = 0; i < gameObjects.Len(); ++i) {
                    gCycle* c = dynamic_cast<gCycle*>(gameObjects(i));
                    if (c && c->Player() && c->Player()->pID == slot) {
                        targetCycle = c;
                        break;
                    }
                }
            ...
            if (targetCycle) {
                center = targetCycle;
                if (mode == CAMERA_NOCLIP || mode == CAMERA_FREE) {
                    mode = CAMERA_SMART;
                }
            } else {
                if (tgt >= 0 && tgt < (int)states.size()) {
                    const auto& st = states[tgt];
                    pos = eCoord(st.posX - st.dirX * 15.f, st.posY - st.dirY * 15.f);
                    z = 6.f;
                    dir = eCoord(st.dirX, st.dirY);
                    rise = -0.15f;
                    fov = 60.f;
                    lastPos = pos;
                    lastrendertime = se_GameTime();
    ...
    }
    {
        ...
        if (sg_noclipActive && !isSpectator) {
            sg_ToggleNoclip(this);
        }

        int numkeys;
        const bool* keystate = SDL_GetKeyboardState(&numkeys);
        SDL_Scancode sc = sg_noclipKeyToggle > 0 ? SDL_GetScancodeFromKey((SDL_Keycode)sg_noclipKeyToggle) : SDL_SCANCODE_UNKNOWN;
        bool pressed = (keystate && sc >= 0 && sc < numkeys) ? keystate[sc] : false;
        
        bool safeToToggle = isSpectator;
        if ((lp && lp->netPlayer && lp->netPlayer->IsChatting()) || uMenu::MenuActive()) {
            safeToToggle = false;
        }

        if (pressed && !sg_noclipToggleLast && safeToToggle) {
            sg_ToggleNoclip(this);
        } else if (pressed && !sg_noclipToggleLast && !isSpectator && !((lp && lp->netPlayer && lp->netPlayer->IsChatting()) || uMenu::MenuActive())) {
            tColoredString m;
        ...
        sg_noclipToggleLast = pressed;

        bool cinKey = (keystate && sg_noclipKeyCinematic > 0 && sg_noclipKeyCinematic < SDLK_LAST) ? keystate[sg_noclipKeyCinematic] : false;
        bool safeToToggleCin = true;
        if ((lp && lp->netPlayer && lp->netPlayer->IsChatting()) || uMenu::MenuActive()) {
            safeToToggleCin = false;
        }
        if (cinKey && !sg_noclipCinematicLast && safeToToggleCin) {
            sg_noclipCinematic = !sg_noclipCinematic;
            tString s; s << "Cinematic: " << (sg_noclipCinematic ? "ON" : "OFF");
            sr_con.DoCenterDisplay(s, 2.0, 1.0, 0.85, 0.3);
        }
        sg_noclipCinematicLast = cinKey;

        // Sync g_NoclipMode with sg_noclipActive bi-directionally
        extern bool g_NoclipMode;
        static bool lastNoclipActive = false;
        static bool lastModMenuNoclip = false;
        
        if (g_NoclipMode != lastModMenuNoclip) {
            if (g_NoclipMode && !isSpectator) {
                g_NoclipMode = false;
                lastModMenuNoclip = false;
                tColoredString m;
                m << tColoredString::ColorString(1.0, 0.3, 0.3)
                  << "[NOCLIP] " << tColoredString::ColorString(1,1,1)
                  << "Noclip is only available when spectating.\n";
                con << m;
            } else if (g_NoclipMode != sg_noclipActive) {
                sg_ToggleNoclip(this);
            }
            lastModMenuNoclip = g_NoclipMode;
            lastNoclipActive = sg_noclipActive;
        } else if (sg_noclipActive != lastNoclipActive) {
            g_NoclipMode = sg_noclipActive;
            lastModMenuNoclip = g_NoclipMode;
            lastNoclipActive = sg_noclipActive;
        }
    }
