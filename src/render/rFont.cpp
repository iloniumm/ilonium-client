...
// Whether dark text gets a bright plate behind it. Off by default: it singles
// out one player with a grey box while everyone else is drawn plainly.
bool sr_darkTextPlate = false;
static tConfItem<bool> sr_darkTextPlateConf( "TEXT_DARK_BACKGROUND", sr_darkTextPlate );
#include <ctype.h>
...
rFont rFont::s_defaultFontSmall("textures/font_s.png",32,5/128.0,9/128.0,1/128.0, -1, &sr_lowerPartFont);
...
void rFont::ProcessImage( SDL_Surface * surface )
{
    ...
    else if (SDL_BYTESPERPIXEL(surface->format) == 2)
    {
        for (int i=surface->w*surface->h-1;i>=0;i--){
...
void rFont::Render(unsigned char c,REAL left,REAL top,REAL right,REAL bot){
    ...
    // if(31<c && 256>c && sr_glOut)
    {
        ...
        if (rTextureGroups::TextureMode[rTextureGroups::TEX_FONT] != GL_NEAREST && rTextureGroups::TextureMode[rTextureGroups::TEX_FONT] != GL_NEAREST_MIPMAP_NEAREST)
            pix = select->onepixel * .5;

        REAL ttop=y*select->cheight+pix;
        REAL tbot=(y+1)*select->cheight-pix;
        REAL tleft=x*select->cwidth+select->borderExtension*pix;
        REAL tright=(x+1)*select->cwidth-pix;
...
void rTextField::FlushLine(int len,bool newline){
    ...
    if (sr_glOut)
    {
        // Dark text needs help to stay readable. The classic answer is a bright
        // plate behind it, which is loud next to players who get none; the
        // alternative simply lifts the glyph colour instead.
        if ( color_.IsDark() )
        {
            RenderEnd(true);
            glDisable(GL_TEXTURE_2D);
            if ( sr_alphaBlend && sr_darkTextPlate )
            {
            ...
            {
                // Raise the glyph itself to a readable level, keeping whatever
                // hue is there so the player is still recognisable.
                REAL peak = r;
                if ( g > peak ) peak = g;
                if ( b > peak ) peak = b;
