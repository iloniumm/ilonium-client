...
// The helper writes where it has always written: a fixed set of places under
// the home directory, chosen so that both the plain build and the one Steam
// unpacks into its own tree find it. A build started from its source tree has
// none of those as its own var directory, so every one of them is looked at
// and the freshest wins.
static void MediaBridgePaths(std::vector<std::string> & out, char const * name) {
    // Where the bridge is told to write now, and where older builds left it.
    // Both are looked at: a helper started by a previous version is still
    // writing to the old place until it is restarted.
    out.push_back((char const *)rc_InternalPath(name));
    out.push_back((char const *)tDirectories::Var().GetWritePath(name));

#if defined(_WIN32) || defined(WIN32)
    // Beside the game itself, which is where the companion also leaves a copy.
    // Neither side has to guess where the other thinks data belongs.
    char self[MAX_PATH] = { 0 };
    if (GetModuleFileNameA(NULL, self, sizeof(self) - 1)) {
        std::string beside = self;
        size_t cut = beside.find_last_of("\\/");
        if (cut != std::string::npos)
            out.push_back(beside.substr(0, cut + 1) + name);
    }
#endif
    ...
    static char const * const known[] = {
        "/.Retrocycles/var/",
        "/.armagetronad/var/",
        "/snap/steam/common/.Retrocycles/var/",
        "/snap/steam/common/.armagetronad/var/"
    };

    for (size_t i = 0; i < sizeof(known)/sizeof(known[0]); ++i)
        out.push_back(std::string(home) + known[i] + name);
}

//! The newest of them, since a stale one left over from another build would
//! otherwise freeze the display on a track that stopped hours ago.
extern bool rc_mediaReport;

static FILE * OpenFreshestBridgeFile(char const * name) {
    std::vector<std::string> paths;
    MediaBridgePaths(paths, name);

    std::string best;
    time_t newest = 0;
    for (size_t i = 0; i < paths.size(); ++i) {
        struct stat info;
        if (stat(paths[i].c_str(), &info) == 0 && info.st_mtime >= newest) {
            newest = info.st_mtime;
            best = paths[i];
        }
    }

    // A file nobody writes any more is worse than no file at all: it would
    // pin the display to a track that stopped hours ago. The helpers rewrite
    // theirs several times a second, so anything older than this is dead.
    if (!best.empty() && time(NULL) - newest > 10)
        best.clear();

    return best.empty() ? NULL : fopen(best.c_str(), "r");
}

//! Orders go to every place the helper watches; it polls all of them.
static bool TellBridge(char const * text) {
    std::vector<std::string> paths;
    MediaBridgePaths(paths, "retrocycles_media_cmd.txt");

    bool told = false;
    for (size_t i = 0; i < paths.size(); ++i) {
        std::string dir = paths[i].substr(0, paths[i].find_last_of('/'));
        struct stat info;
...
void SendMediaCommand(int cmd) {
    // Reading was made direct but this was left writing to a file only the
    // bridge script reads, so with no script running the buttons did nothing.
    // Tell the player the same way we ask it.
    if (rc_TellPlayer(cmd))
        return;

#if defined(_WIN32) || defined(WIN32)
    // The helper this talks to elsewhere is a shell script driving playerctl,
    // and neither exists here, so the buttons had nothing on the other end.
    // Windows has no single place to ask about playback either, but every
    // player listens for the keyboard's transport keys, so press those.
    BYTE key;
    switch (cmd) {
        case -1: key = VK_MEDIA_PREV_TRACK; break;
        case  0: key = VK_MEDIA_PLAY_PAUSE; break;
...
#else
    switch (cmd) {
        case -1: TellBridge("prev"); break;
        case  0: TellBridge("play-pause"); break;
        case  1: TellBridge("next"); break;
    }
#endif
}

// Where a seek was asked to land, and when it was asked. Kept out here because
// the asking and the believing happen in different places.
static float sg_seekTo = -1.0f;
static REAL  sg_seekAt = 0.0f;
...
//! The bridge that reads what Windows is playing. It sits beside the game
//! rather than inside it, and it is a script rather than a program: the same
//! questions asked from hand written interface declarations crashed on the
//! first call, in process and out of it alike.
static HANDLE sg_mediaHelper = NULL;

static void StartMediaHelper() {
    if (sg_mediaHelper) return;

    // Only for somebody who asked for it.
    //
    // This starts PowerShell, hidden, with the execution policy waived - the
    // exact shape of the thing every antivirus on Windows is built to catch,
    // and it used to happen the moment the game opened whether the media
    // widget was switched on or not. A game that does that on startup gets
    // quarantined, and the person who downloaded it is left deciding whether
    // they have been sent a miner.
    //
    // Nobody who has not turned the widget on will ever see it now.
    if (!sg_modMediaWidgetEnabled) return;

    extern bool rc_mediaReport;

    char self[MAX_PATH] = { 0 };
    if (!GetModuleFileNameA(NULL, self, sizeof(self) - 1)) return;

    std::string beside = self;
    size_t cut = beside.find_last_of("\\/");
    if (cut == std::string::npos) return;
    beside = beside.substr(0, cut + 1) + "retrocycles_media_bridge.ps1";

    // Said out loud rather than under a switch: a bridge that is not in the
    // package looks from the outside exactly like one that is there and
    // silent, and the difference is the whole diagnosis.
    if (GetFileAttributesA(beside.c_str()) == INVALID_FILE_ATTRIBUTES) {
        con << "media bridge not found: " << beside.c_str() << "\n";
        return;
    }

    std::string folder = (char const *)rc_InternalPath("retrocycles_media_state.txt");
    size_t end = folder.find_last_of("\\/");
    folder = (end == std::string::npos) ? std::string(".") : folder.substr(0, end);

    // Named in full rather than left to the search path, which a game has no
    // business trusting, and the older of the two shells on purpose: it is the
    // one that can reach the system's media session.
    char shell[MAX_PATH] = { 0 };
    UINT room = GetSystemDirectoryA(shell, sizeof(shell) - 1);
    if (!room || room >= sizeof(shell)) return;

    std::string line = std::string("\"") + shell +
        "\\WindowsPowerShell\\v1.0\\powershell.exe\" -NoProfile -NonInteractive"
        // The window is already suppressed by CREATE_NO_WINDOW below, so the
        // flag that says so again is dropped: it buys nothing and it is one of
        // the two words a scanner looks for. The other one has to stay - a
        // script nobody paid to sign will not run without it.
        " -ExecutionPolicy Bypass -File \"" + beside + "\" \"" + folder + "\" ";

    char id[32];
    snprintf(id, sizeof(id), "%lu", (unsigned long)GetCurrentProcessId());
    line += id;
    ...
    ZeroMemory(&made, sizeof(made));

    std::vector<char> writable(line.begin(), line.end());
    writable.push_back('\0');

    bool started = CreateProcessA(NULL, &writable[0], NULL, NULL, FALSE,
                                  CREATE_NO_WINDOW, NULL, NULL, &start, &made) != 0;
    if (started) {
        CloseHandle(made.hThread);
        sg_mediaHelper = made.hProcess;
    } else {
        con << "media bridge would not start: " << beside.c_str() << "\n";
    }

    if (started && rc_mediaReport)
        con << "MEDIA bridge [" << beside.c_str() << "] folder [" << folder.c_str() << "]\n";
}

static void StopMediaHelper() {
    if (!sg_mediaHelper) return;
    TerminateProcess(sg_mediaHelper, 0);
    CloseHandle(sg_mediaHelper);
    sg_mediaHelper = NULL;
}
...
void MediaWidget::PollingThreadFunc() {
    ...
    while (!m_ThreadShouldExit) {
        // Asked here rather than once at the start: the bridge only runs for
        // somebody who has switched the widget on, and switching it on happens
        // long after this thread began. It returns at once when there is
        // nothing to do.
        StartMediaHelper();
        ...
        // A helper that is running is the better source and on Windows the
        // only one that knows a position, so it is read first. Asking the
        // system directly is what everybody without a helper gets, and on
        // desktops that refuse the game an answer it is what nobody gets.
        FILE* f = OpenFreshestBridgeFile("retrocycles_media_state.txt");

        rc_Playing live;
        bool answered = false;
        bool fromSystem = !f && rc_AskNowPlaying(live);
        if (fromSystem) {
            answered = true;
            ...
            length    = live.length;
            if (!live.title.empty()) {
                strncpy(title, live.title.c_str(), sizeof(title) - 1);
                title[sizeof(title) - 1] = '\0';
            }
            strncpy(artist, live.artist.c_str(), sizeof(artist) - 1);
            artist[sizeof(artist) - 1] = '\0';
            strncpy(album, live.album.c_str(), sizeof(album) - 1);
        ...
        // A single poll that learns nothing is not news: the file is rewritten
        // several times a second and a read can land between two of those
        // writes. Blanking the display for that one round is what made the
        // track flicker out for a moment. What was last known stands until
        // there has been nothing for long enough to mean it.
        static REAL quietSince = 0.0f;
        REAL rightNow = tSysTimeFloat();

        if (answered)
            quietSince = rightNow;

        bool holdOn = !answered && ( rightNow - quietSince < 5.0f );
...
void MediaWidget::Update(float dt) {
    ...
    //
    // A player goes on reporting where it was for a moment after being told to
    // move, and the bridge only asks a couple of times a second on top of
    // that. Taking those first answers at face value is what made the marker
    // snap back to where it came from and then jump forward again. Until the
    // source agrees it has arrived, the marker stays where it was sent.
    if (sg_seekTo >= 0.0f) {
        REAL waited = tSysTimeFloat() - sg_seekAt;
        bool arrived = fabsf(threadPos - sg_seekTo) < 2.5f;

        if (arrived || waited > 4.0f) {
            sg_seekTo = -1.0f;
        } else {
            m_LastPolledPos = threadPos;
            m_TimeSinceLastPosUpdate = 0.0f;
            m_CurrentPosition = sg_seekTo + (m_IsPlaying ? (float)waited : 0.0f);
    ...
    if (sg_seekTo >= 0.0f) {
        // held above; nothing else to work out this frame
    } else if (threadPos != m_LastPolledPos) {
        m_LastPolledPos = threadPos;
...
void MediaWidget::Draw(ImDrawList* dl, ImVec2 pos, float alphaMult) {
    ...
    // Dragging the marker rather than only clicking it, and on every system
    // now that there is something listening everywhere. The order goes once,
    // when it is let go: everything before that was the hand on its way there.
    static bool  dragging = false;
    static float dragTo   = 0.0f;

    if (isEditing || m_TotalLength <= 0.0f) {
        dragging = false;
    } else {
        if (pbHovered && io.MouseClicked[0])
            dragging = true;

        if (dragging) {
            dragTo = (io.MousePos.x - pbX0) / (pbX1 - pbX0);
            dragTo = std::max(0.0f, std::min(1.0f, dragTo));

            // the marker follows the hand while it is held
            progressNorm = dragTo;

            if (!io.MouseDown[0]) {
                dragging = false;

                float seekSec = dragTo * m_TotalLength;

                bool moved = rc_SeekPlayer(seekSec);
                if (!moved) {
                    char order[64];
                    snprintf(order, sizeof(order), "seek%.3f", seekSec);
                    moved = TellBridge(order);
                }

                // only claim it moved if something was actually told to
                if (moved) {
                    m_CurrentPosition = seekSec;
                    sg_seekTo = seekSec;
                    sg_seekAt = tSysTimeFloat();
                }
            }
        }
    }
