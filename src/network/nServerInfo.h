...
public:
    ...
    //! Authenticated names, one per line, lining up with UserNames().
    //!
    //! A blank line means that player is not logged in. This is the only place
    //! a client is told who is logged in as what - the player sync carries no
    //! such field - and it is what the server browser has always shown.
