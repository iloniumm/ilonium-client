...
SDL_Window *sr_screen=NULL; // our window
...
#else
rScreenSettings currentScreensetting(sr_DesktopScreensizeSupported() ? ArmageTron_Desktop : ArmageTron_800_600, false);
#endif
...
// A display can pack more pixels than the size it reports - every Retina Mac
// does - and drawing into all of them means four times the work for the
// resolution the player picked, with nothing in the menu to say so. Off is what
// the game has always done; on trades frame rate for a sharper picture.
bool sr_highDPI = false;
static tConfItem<bool> sr_highDPIConf( "HIGH_DPI", sr_highDPI );
...
bool cfg_MSAA = false;
static tConfItem<bool> msaa_ci("ANTI_ALIASING_MSAA", cfg_MSAA);
...
// kept so the context can be taken down with the window it belongs to
static SDL_GLContext sr_glContext = NULL;
...
char const * sr_ClientName()
{
    return "Ilonium Client";
}

char const * sr_ClientId()
{
    return "com.ilonium.client";
}
...
static bool lowlevel_sr_InitDisplay(){
    ...
    if ( sr_desktopWidth == 0 && !sr_screen )
    {
        ...
        // SDL3: use SDL_GetCurrentDisplayMode to get desktop resolution and color depth
        SDL_DisplayID displayID = SDL_GetPrimaryDisplay();
        const SDL_DisplayMode* dm = SDL_GetCurrentDisplayMode( displayID );
        if( dm )
        {
            // SDL3 always uses 32-bit RGBA internally; map to legacy depth vars
            desktopCD    = 32;
            desktopCD_R  = 8;
            desktopCD_G  = 8;
            desktopCD_B  = 8;
    ...
    if (!sr_screen)
    {
        // SDL3: window title is set on the SDL_Window after creation;
        // SDL_EnableUNICODE removed (text input handled by SDL_EVENT_TEXT_INPUT).
        ...
        */
        ...
        {
            // Have the screen reinited. Dropping the pointer on its own leaves
            // the window standing, which is the same way the game ended up
            // showing two of itself; take it down before asking for a new one.
            if ( sr_screen )
            {
                if ( sr_glContext )
                {
        ...
        if ( !sr_screen )
        {
            SDL_SetHint("SDL_MOUSE_RAW_INPUT", "1");

#if defined(WIN32) || defined(_WIN32)
            // On Windows a fullscreen window drops to the taskbar the moment
            // it loses focus, so anything that takes focus for an instant
            // throws the player out of the match. Elsewhere the default is
            // left alone: macOS gives a fullscreen window its own space, and
            // holding on to it through a task switch strands the desktop
            // behind a window that has stopped drawing.
            ...
            if ( !sr_screen )
            {
                // Try without fullscreen as fallback
                wflags &= ~SDL_WINDOW_FULLSCREEN;
                sr_screen = SDL_CreateWindow(
                    sr_ClientName(),
                    sr_screenWidth, sr_screenHeight,
                ...
                {
                    lastError.Clear();
                    lastError << "Couldn't create SDL3 window: ";
            ...
            if ( sr_glContext )
            {
                // nothing should get here holding one, but leaking it would be
                // silent and this costs nothing
                SDL_GL_DestroyContext( sr_glContext );
                sr_glContext = NULL;
            }

            SDL_GLContext glctx = SDL_GL_CreateContext(sr_screen);
            sr_glContext = glctx;
            if ( !glctx )
            {
                lastError.Clear();
                lastError << "Couldn't create OpenGL context: ";
                lastError << SDL_GetError();
                std::cerr << lastError << '\n';
                SDL_DestroyWindow(sr_screen);
                sr_screen = NULL;
            ...
            SDL_GL_MakeCurrent(sr_screen, glctx);

            // Set vSync / swap interval in SDL3
            ...
            SDL_SetWindowTitle( sr_screen, sr_ClientName() );
        }
    }

    // the numbers above are only what we asked for. A fullscreen window gets
    // whatever the desktop runs at, and a high density display hands back a
    // buffer larger than the size it was created with, so the viewport has to
    // follow the buffer we actually got or it covers part of the screen and
    // leaves the rest black.
    if ( sr_screen )
    {
        // asking for fullscreen at creation time is a request the window
        // manager may quietly serve as an ordinary window sized to the work
        // area; state it again afterwards and wait for the answer, otherwise
        // the panels stay carved out of the picture
        SDL_SetWindowFullscreen( sr_screen, currentScreensetting.fullscreen );
        SDL_SyncWindow( sr_screen );

        int pixelWidth = 0, pixelHeight = 0;
        if ( SDL_GetWindowSizeInPixels( sr_screen, &pixelWidth, &pixelHeight ) &&
             pixelWidth > 0 && pixelHeight > 0 )
        {
            sr_screenWidth  = pixelWidth;
            sr_screenHeight = pixelHeight;
        }
    }
...
void sr_ExitDisplay(){
    ...
    if (sr_screen){
        ...
        // This used to only drop the pointer, on the old advice that the
        // screen surface must not be freed. That was true when it was a
        // surface. It is a window now, and dropping the pointer leaves it open
        // - so changing the display left the old window standing beside the new
        // one, looking for all the world like a second copy of the game, with
        // the input going to whichever one the system felt like.
        if ( sr_glContext )
        {
            SDL_GL_DestroyContext( sr_glContext );
            sr_glContext = NULL;
