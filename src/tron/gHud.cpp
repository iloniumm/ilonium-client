...
// The warning that a teammate has gone.
//
// The first question decides everything else: is there a teammate at all? In a
// duel, in a free for all and in single player every rider is a team of one,
// and on those this has to be silent for ever - not filtered afterwards, not
// shown for an instant and taken back. Nothing below runs until that is
// answered, which is why the answer is the first thing here.
//
// What "alive" means is taken from where the score table takes it, because
// that one is never wrong: the player's own object, asked at this moment. A
// player with no object at all is not dead, only out of sight for a frame
// while the game rebuilds it, and nothing is said about them.

//! Who we have already said something about. Emptied every round, so nothing
//! is carried between two of them and there is no state to go stale.
//! Alive, as the score table on Tab says it. Declared rather than included:
//! this file has no other business with the widget code.
bool rc_PlayerIsAlive( ePlayerNetID const * p );

static std::vector< ePlayerNetID const * > sg_mourned;

std::vector< tString > sg_teammateDown;   //!< names waiting to be shown, oldest first
REAL sg_teammateDeathFlashTime = -1000.0f;
tString sg_deadTeammateName;

//! Drops the pending warning. Anything that ends the situation it described.
void rc_ForgetTeammateWarning()
{
    sg_teammateDeathFlashTime = -1000.0f;
    sg_deadTeammateName = tString();
    sg_teammateDown.clear();
    sg_mourned.clear();
}

namespace
{
bool Mourned( ePlayerNetID const * who )
{
    for ( size_t i = 0; i < sg_mourned.size(); ++i )
        if ( sg_mourned[i] == who )
            return true;
    return false;
}
...
{
    if (!sg_modTeammateDeathWarning || !me)
    {
        rc_ForgetTeammateWarning();
        return;
    }

    // 1. Somebody to be warned about, or nothing at all.
    eTeam * team = me->CurrentTeam();
    int mates = 0;
    ...
    if ( team )
    {
        ...
        {
            ePlayerNetID * mate = team->Player( i );
            if ( mate && mate != me )
                ++mates;
        }
    }
    ...
    // 2. Between two rounds the grid is taken apart and built again: everyone
    //    stops being alive, the cycles go, and then everyone is alive again.
    //    Every one of those steps is a death to anything that only reads a
    //    flag, which is where the warnings for nobody came from.
    if ( se_GameTime() < 0.5f )
    {
        sg_mourned.clear();
        return;
    ...
    // 3. Warnings are for somebody who is still riding.
    if ( !rc_PlayerIsAlive( me ) )
        return;

    // 4. A round that has been decided is not an emergency. If nobody but us
    //    is left standing there is nothing to warn about, and the deaths that
    //    end a round would otherwise fire this every single time.
    int othersAlive = 0;
    for ( int i = se_PlayerNetIDs.Len() - 1; i >= 0; --i )
    {
        ePlayerNetID * p = se_PlayerNetIDs( i );
        if ( p && p != me && rc_PlayerIsAlive( p ) )
            ++othersAlive;
    }
    ...
    // Somebody who left the game is no longer somebody we are keeping quiet
    // about, and their record would otherwise sit here until the round ended -
    // pointing at memory the game has given back.
    for ( size_t k = sg_mourned.size(); k-- > 0; )
    {
        bool stillHere = false;
        for ( int i = team->NumPlayers() - 1; i >= 0 && !stillHere; --i )
            stillHere = ( team->Player( i ) == sg_mourned[k] );

        if ( !stillHere )
            sg_mourned.erase( sg_mourned.begin() + k );
    }

    // 5. Whoever on this team has a body and is not moving in it.
    for ( int i = team->NumPlayers() - 1; i >= 0; --i )
    {
        ePlayerNetID * mate = team->Player( i );
        if ( !mate || mate == me )
        ...
        eGameObject * body = mate->Object();
        if ( !body )                 // no body to read yet: no news either
            continue;

        if ( rc_PlayerIsAlive( mate ) )
        {
            // Back on a bike. Servers that respawn people mid round would
            // otherwise get one warning per player per round and no more.
            for ( size_t k = sg_mourned.size(); k-- > 0; )
                if ( sg_mourned[k] == mate )
                    sg_mourned.erase( sg_mourned.begin() + k );
            continue;
        }
        ...
        // Only somebody who has just gone down. This runs from the drawing
        // pass, so it stops while a menu is open; without this the first frame
        // after closing one would announce a death from a minute ago as if it
        // had just happened.
        REAL const died = body->DeathTime();
        if ( died <= 0.0f || se_GameTime() - died > 2.0f )
            continue;

        sg_mourned.push_back( mate );

        // The list holds this warning and nothing older. A name from a round
        // ago is not news, and left in it would be shown again beside the new
        // one the next time anybody went down.
        REAL const now = (REAL)tRealSysTimeFloat();
        if ( now - sg_teammateDeathFlashTime >= 3.0f )
            sg_teammateDown.clear();

        // Two going down together is the ordinary case in fortress, and one
        // slot meant the second wrote over the first before anybody saw it.
        sg_teammateDown.push_back( mate->GetName() );
        if ( sg_teammateDown.size() > 3 )
            sg_teammateDown.erase( sg_teammateDown.begin() );

        sg_deadTeammateName = mate->GetName();
        sg_teammateDeathFlashTime = now;
    }
}
