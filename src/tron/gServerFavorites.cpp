...
int gServerFavorites::GetNumFavorites()
{
    return NUM_FAVORITES;
}

bool gServerFavorites::GetFavoriteInfo(int index, tString &name, tString &address, int &port)
{
    if (index < 0 || index >= NUM_FAVORITES)
        return false;
    
    gServerFavorite &fav = sg_favoriteHolder.GetFavorite(index);
    if (fav.name_ == "" || fav.name_ == "Empty")
        return false;
        
    name = fav.name_;
    address = fav.address_;
    port = fav.port_;
    return true;
}

void gServerFavorites::RemoveFavoriteByIndex(int index)
{
    if (index >= 0 && index < NUM_FAVORITES)
    {
        gServerFavorite &fav = sg_favoriteHolder.GetFavorite(index);
