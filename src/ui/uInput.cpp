...
uAction * su_ActionList( uActionScope scope ){
    uAction ** list = NULL;
    switch ( scope ){
    case uACTION_PLAYER: list = &s_playerActions; break;
    case uACTION_CAMERA: list = &s_cameraActions; break;
    case uACTION_GLOBAL: list = &s_globalActions; break;
    }

    if ( !list || !*list )
        return NULL;

    uAction::Sort<Input_Comparator>( *list );
    return *list;
}

int su_BindSlot( uActionScope scope, int localPlayer ){
    return scope == uACTION_GLOBAL ? 0 : localPlayer + 1;
}
