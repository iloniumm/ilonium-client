...
void sr_SetFastForward(double targetTime) {
    s_fastForwardTo = targetTime;
    s_fastForward = true;
}

bool sr_IsFastForwarding() {
    return s_fastForward;
}

double sr_GetFastForwardTarget() {
    return s_fastForwardTo;
}

void sr_CancelFastForward() {
    s_fastForward = false;
}
...
static const double sr_sleepMarginMin = 0.00025;
static const double sr_sleepMarginMax = 0.00500;
...
void sr_LimitFPS()
{
    ...
    static double sleepMargin = 0.0012;
    ...
    if (target_now_time - now_time > SPF * 4.0)
    {
        last_time = now_time;
        return;
    }

    const double coarse = target_now_time - now_time - sleepMargin;
    if (coarse > 0.0)
    {
        const double entered = tRealSysTimeFloat();
        SDL_Delay((Uint32)(coarse * 1000.0));
        const double overshoot = (tRealSysTimeFloat() - entered) - coarse;

        if (overshoot > 0.0)
            sleepMargin += (overshoot + sleepMargin * 0.25 - sleepMargin) * 0.125;
        else
            sleepMargin -= sleepMargin * 0.002;

        if (sleepMargin < sr_sleepMarginMin) sleepMargin = sr_sleepMarginMin;
        if (sleepMargin > sr_sleepMarginMax) sleepMargin = sr_sleepMarginMax;
    }
...
//! Checks the playback device is still alive; declared rather than included,
//! the renderer has no business pulling in the sound engine's header.
void se_SoundKeepAlive();

void rSysDep::SwapGL(){
    // Every loop in the client comes through here, including the ones that run
    // while the window is away, which is exactly when a playback device is
    // most likely to be taken from us.
    se_SoundKeepAlive();
    ...
    // [MOD] FBO unbind/quad are now inside gameplay only (gGame.cpp Render).
    // SwapGL is called for all screens including menus — don't post-process here.

    // Render overlay (visual menu) on top of resolved screen quad
