    ...
    if (sn_GetNetState()!=nCLIENT)
    {
        ...
        for(int t=eTeam::teams.Len()-1;t>=0;t--)
        {
            ...
            for (int p = 0; p<numPlayers; ++p)
            {
                ...
                if (sn_GetNetState()!=nCLIENT
#ifndef DEDICATED
                    && !(DemoPlayerManager::Instance().IsViewerActive() && DemoPlayerManager::Instance().IsLoaded())
#endif
                ){
#ifdef DEBUG
...
#ifndef DEDICATED
    if (DemoRecorder::Instance().IsRecording()) {
        std::vector<DemoCycleState> cycleStates;
        if (grid) {
            const tList<eGameObject>& gameObjects = grid->GameObjects();
            for (int i = gameObjects.Len() - 1; i >= 0; --i) {
                eGameObject* e = gameObjects(i);
                if (e) {
                    gCycle* cycle = dynamic_cast<gCycle*>(e);
                    if (cycle) {
                        DemoCycleState cs;
                        cs.playerSlot = (cycle->Player() ? cycle->Player()->pID : 0);
                        cs.posX = cycle->Position().x;
                        cs.posY = cycle->Position().y;
                        cs.dirX = cycle->Direction().x;
                        cs.dirY = cycle->Direction().y;
                        cs.speed = cycle->Speed();
                        cs.rubber = cycle->GetRubber();
                        cs.alive = cycle->Alive();
                        cycleStates.push_back(cs);
                    }
                }
            }
        }
        ...
        double curTime = time;
        if (lastLoggedTime < 0.0 || curTime < lastLoggedTime) {
            lastLoggedTime = curTime;
            recordTick = 0;
        }
        float dt = curTime - lastLoggedTime;
        DemoRecorder::Instance().LogFrameData(curTime, dt, recordTick++, cycleStates);
        lastLoggedTime = curTime;
    }
#endif
}
...
void RenderAllViewports(eGrid *grid){
    ...
    if(sr_glOut){
        ...
        if (g_CustomFog) {
            ...
            glFogf( GL_FOG_DENSITY, CUSTOM_FOG_DENSITY/gArena::SizeMultiplier() );
            GLfloat fogColor[3]={CUSTOM_FOG_R, CUSTOM_FOG_G, CUSTOM_FOG_B};
    ...
    // [MOD] Hide console/scores in cinematic mode if requested
...
#ifndef DEDICATED
            DemoRecorder::Instance().LogRoundBegin(se_GameTime());
#endif
...
#ifndef DEDICATED
        DemoRecorder::Instance().LogRoundEnd(se_GameTime());
#endif
    }
...
#ifndef DEDICATED
        if (DemoPlayerManager::Instance().IsViewerActive() && DemoPlayerManager::Instance().IsLoaded()) {
            REAL dt = se_mainGameTimer ? se_mainGameTimer->FrameTime() : (1.0f / 60.0f);
            if (dt > 0.1f) dt = 0.1f;
            DemoPlayerManager::Instance().Tick(dt);
