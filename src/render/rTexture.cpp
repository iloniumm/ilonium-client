...
void rSurface::Create( SDL_Surface * surface )
{
    ...
    if ( surface_ )
    {
        switch (SDL_BYTESPERPIXEL(surface_->format)){
        case 1:
        ...
        default:
            {
                ...
                SDL_Surface *dummy =
                    SDL_CreateSurface(1, 1, SDL_PIXELFORMAT_ARGB8888);

                SDL_Surface *convtex =
                    SDL_ConvertSurface(surface_, dummy->format);
...
rISurfaceTexture::rISurfaceTexture( int group, bool repx, bool repy, bool storeAlpha )
        : group_( group ), textureModeLast_( -1), tint_( 0 ), repx_( repx ), repy_( repy ), storeAlpha_( storeAlpha ), uploaded_( false )
{
}
...
#define GL_CLAMP_TO_EDGE GL_CLAMP
    ...
    bool texalpha=SDL_ISPIXELFORMAT_ALPHA(tex->format);
    ...
    // Grown on the way past, if the setting asks for it and the picture is
    // small enough to be worth it. The card is told about the bigger one and
    // never learns there was another; the copy dies with this call.
    GLint biggest = 0;
    glGetIntegerv( GL_MAX_TEXTURE_SIZE, &biggest );

    SDL_Surface * grown = WantsUpscale() ? rc_UpscaleSurface( tex, biggest ) : 0;
    SDL_Surface * use = grown ? grown : tex;

    gluBuild2DMipmaps(GL_TEXTURE_2D,format,use->w,use->h,
                      texformat,GL_UNSIGNED_BYTE,use->pixels);
