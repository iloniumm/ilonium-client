...
public:
    ...
    //! The description this item was declared with. Read only, and only so
    //! that something can list them; nothing here can change it.
    tOutput const & GetHelp() const {
        return help;
    }
    ...
    static bool OpenFile( std::ifstream & s, tString const & filename, tSearchPath path ); //! opens a file stream for configuration reading
